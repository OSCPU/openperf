...
static inline int ReadData(int vfile, FrameFormat *source, unsigned char *buf) {
  ...
  for (i = 0; i < source->height[0]; i++) {
    if (read(vfile, cur_buf, read_size) != read_size) {
      printf("read_one_frame: cannot read %d bytes from input file, unexpected "
             "EOF!\n",
             source->width[0]);
      return 0;
    }
  ...
  if (source->yuv_format != YUV400) {
    ...
    for (j = 0; j < 2; j++) {
      for (i = 0; i < source->height[1]; i++) {
        if (read(vfile, cur_buf, read_size) != read_size) {
          printf("read_one_frame: cannot read %d bytes from input file, "
                 "unexpected EOF!\n",
                 source->width[1]);
          return 0;
        }
...
#else
static inline int ReadData(int vfile, int framesize_in_bytes,
                           unsigned char *buf) {
 ...
 ************************************************************************
 */
int ReadFrameConcatenated(InputParameters *p_Inp, VideoDataFile *input_file,
  ...
  if (lseek(vfile,
            HeaderSize +
                framesize_in_bytes * (FrameNoInFile + p_Inp->start_frame),
            SEEK_SET) == -1) {
    snprintf(errortext, ET_SIZE,
             "read_one_frame: cannot advance file pointer in input file beyond "
             "frame %d\n",
  ...
  if ((source->pic_unit_size_on_disk & 0x07) == 0) {
    ...
    printf("read_one_frame (NOT IMPLEMENTED): pic unit size on disk must be "
           "divided by 8");
    exit(-1);
  }
  return file_read;
 ...
 ************************************************************************
 */
int ReadFrameSeparate(InputParameters *p_Inp, VideoDataFile *input_file,
