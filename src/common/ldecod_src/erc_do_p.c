...
static int concealByTrial(frame *recfr, imgpel *predMB, int currMBNum,
                          objectBuffer_t *object_list, int predBlocks[],
                          int picSizeX, int picSizeY, signed char *yCondition);
static int edgeDistortion(int predBlocks[], int currYBlockNum, imgpel *predMB,
                          imgpel *recY, int picSizeX, int regionSize);
static void copyBetweenFrames(frame *recfr, int currYBlockNum, int picSizeX,
                              int regionSize);
static void buildPredRegionYUV(VideoParameters *p_Vid, int *mv, int x, int y,
                               imgpel *predMB);

// picture error concealment
static void buildPredblockRegionYUV(VideoParameters *p_Vid, int *mv, int x,
                                    int y, imgpel *predMB, int list, int mb);
...
static const int uv_div[2][4] = {{0, 1, 1, 0},
                                 {0, 1, 0, 0}}; //[x/y][yuv_format]
 ...
 *be used 1, otherwise (even if none of the blocks were concealed) \param recfr
  ...
  if (errorVar && errorVar->concealment) {
    ...
    if (errorVar->nOfCorruptedSegments) {
      if (chroma_format_idc != YUV400)
        predMB = (imgpel *)malloc(
            (256 + (p_Vid->mb_cr_size_x * p_Vid->mb_cr_size_y) * 2) *
            sizeof(imgpel));
      else
      ...
      for (columnInd = 0; columnInd < lastColumn; columnInd++) {
        ...
        column = ((columnInd % 2) ? (lastColumn - columnInd / 2 - 1)
                                  : (columnInd / 2));

        for (row = 0; row < lastRow; row++) {

          if (errorVar->yCondition[MBxy2YBlock(column, row, 0, picSizeX)] <=
              ERC_BLOCK_CORRUPTED) { // ERC_BLOCK_CORRUPTED (1) or
                                     // ERC_BLOCK_EMPTY (0)
            firstCorruptedRow = row;
            /* find the last row which has corrupted blocks (in same continuous
             * area) */
            for (lastCorruptedRow = row + 1; lastCorruptedRow < lastRow;
                 lastCorruptedRow++) {
              /* check blocks in the current column */
              if (errorVar->yCondition[MBxy2YBlock(column, lastCorruptedRow, 0,
                                                   picSizeX)] >
                  ERC_BLOCK_CORRUPTED) {
            ...
            if (lastCorruptedRow >= lastRow) {
              ...
              for (currRow = firstCorruptedRow; currRow < lastRow; currRow++) {
                ...
                ercCollect8PredBlocks(predBlocks, (currRow << 1), (column << 1),
                                      errorVar->yCondition, (lastRow << 1),
                                      (lastColumn << 1), 2, 0);

                if (p_Vid->erc_mvperMB >= MVPERMB_THR)
                  concealByTrial(recfr, predMB, currRow * lastColumn + column,
                                 object_list, predBlocks, picSizeX, picSizeY,
                                 errorVar->yCondition);
                else
                  concealByCopy(recfr, currRow * lastColumn + column,
                                object_list, picSizeX);

                ercMarkCurrMBConcealed(currRow * lastColumn + column, -1,
                                       picSizeX, errorVar);
              }
              row = lastRow;
              ...
              /*
               *  Conceal the corrupted area switching between the up and the
               * bottom rows
               */
 ...
 ************************************************************************
 * \brief
 *      It conceals a given MB by simply copying the pixel area from the
 *reference image that is at the same location as the macroblock in the current
 *image. This correcponds to COPY MBs. \return Always zero (0). \param recfr
 *      Reconstructed frame buffer
 * \param currMBNum
 *      current MB index
 * \param object_list
 ...
 ************************************************************************
 * \brief
 *      Copies the co-located pixel values from the reference to the current
 *frame. Used by concealByCopy \param recfr Reconstructed frame buffer \param
 *currYBlockNum index of the block (8x8) in the Y plane \param picSizeX Width of
 *the frame in pixels \param regionSize can be 16 or 8 to tell the dimension of
 *the region to copy
 ************************************************************************
 */
static void copyBetweenFrames(frame *recfr, int currYBlockNum, int picSizeX,
  ...
  for (j = ymin >> uv_div[1][dec_picture->chroma_format_idc];
       j < (ymin + regionSize) >> uv_div[1][dec_picture->chroma_format_idc];
       j++)
    for (k = xmin >> uv_div[0][dec_picture->chroma_format_idc];
         k < (xmin + regionSize) >> uv_div[0][dec_picture->chroma_format_idc];
         k++) {
      //        location = j * picSizeX / 2 + k;
      location =
          ((j * picSizeX) >> uv_div[0][dec_picture->chroma_format_idc]) + k;
 ...
 ************************************************************************
 * \brief
 *      It conceals a given MB by using the motion vectors of one reliable
 *neighbor. That MV of a neighbor is selected wich gives the lowest pixel
 *difference at the edges of the MB (see function edgeDistortion). This
 *corresponds to a spatial smoothness criteria. \return Always zero (0). \param
 *recfr Reconstructed frame buffer \param predMB memory area for storing
 *temporary pixel values for a macroblock the Y,U,V planes are concatenated y =
 *predMB, u = predMB+256, v = predMB+320 \param currMBNum current MB index
 * \param object_list
 *      array of region structures storing region mode and mv for each region
 * \param predBlocks
 *      status array of the neighboring blocks (if they are OK, concealed or
 *lost) \param picSizeX Width of the frame in pixels \param picSizeY Height of
 *the frame in pixels \param yCondition array for conditions of Y blocks from
 *ercVariables_t
 ************************************************************************
 */
static int concealByTrial(frame *recfr, imgpel *predMB, int currMBNum,
  ...
  int predMBNum = 0, numMBPerLine, compSplit1 = 0, compSplit2 = 0, compLeft = 1,
      comp = 0, compPred, order = 1, fInterNeighborExists, numIntraNeighbours,
      fZeroMotionChecked, predSplitted = 0, threshold = ERC_BLOCK_OK, minDist,
      currDist, i, k, bestDir;
  int regionSize;
  objectBuffer_t *currRegion;
  ...
  do { /* 4 blocks loop */
    ...
    currRegion->xMin =
        (xPosYBlock(MBNum2YBlock(currMBNum, comp, picSizeX), picSizeX) << 3);
    currRegion->yMin =
        (yPosYBlock(MBNum2YBlock(currMBNum, comp, picSizeX), picSizeX) << 3);
        ...
        if (predBlocks[i] >= threshold) {
          ...
          if (isBlock(object_list, predMBNum, compSplit1, INTRA) ||
            ...
            for (predSplitted = isSplitted(object_list, predMBNum),
                compPred = compSplit1;
                 predSplitted >= 0; compPred = compSplit2,
                predSplitted -= ((compSplit1 == compSplit2) ? 2 : 1)) {

              /* if Zero Motion Block, do the copying. This option is tried only
               * once */
              if (isBlock(object_list, predMBNum, compPred, INTER_COPY)) {
                ...
                if (fZeroMotionChecked) {
                  ...
                  buildPredRegionYUV(p_Vid->erc_img, mvPred, currRegion->xMin,
                                     currRegion->yMin, predMB);
                }
              }
              ...
              currDist = edgeDistortion(
                  predBlocks, MBNum2YBlock(currMBNum, comp, picSizeX), predMB,
                  recfr->yptr, picSizeX, regionSize);
              ...
              if (currDist < minDist || !fInterNeighborExists) {
                ...
                currRegion->regionMode =
                    (isBlock(object_list, predMBNum, compPred, INTER_COPY))
                        ? ((regionSize == 16) ? REGMODE_INTER_COPY
                                              : REGMODE_INTER_COPY_8x8)
                        : ((regionSize == 16) ? REGMODE_INTER_PRED
                                              : REGMODE_INTER_PRED_8x8);

                copyPredMB(MBNum2YBlock(currMBNum, comp, picSizeX), predMB,
                           recfr, picSizeX, regionSize);
              }
    ...
    if (!fZeroMotionChecked) {
      ...
      currDist =
          edgeDistortion(predBlocks, MBNum2YBlock(currMBNum, comp, picSizeX),
...
************************************************************************
* \brief
*      Builds the motion prediction pixels from the given location (in 1/4 pixel
*units) of the reference frame. It not only copies the pixel values but builds
*the interpolation when the pixel positions to be copied from is not full pixel
*(any 1/4 pixel position). It copies the resulting pixel vlaues into predMB.
* \param p_Vid
...
*      the Y,U,V planes are concatenated y = predMB, u = predMB+256, v =
*predMB+320
************************************************************************
*/
static void buildPredRegionYUV(VideoParameters *p_Vid, int *mv, int x, int y,
  ...
  int ref_frame =
      imax(mv[2], 0); // !!KS: quick fix, we sometimes seem to get negative
                      // ref_pic here, so restrict to zero and above
  int mb_nr =
      y / 16 * (p_Vid->width / 16) + x / 16; /// currSlice->current_mb_nr;
  int **tmp_res = NULL;

  Macroblock *currMB =
      &p_Vid->mb_data[mb_nr]; // intialization code deleted, see below, StW
  ...
  for (j = 0; j < MB_BLOCK_SIZE / BLOCK_SIZE; j++) {
    ...
    for (i = 0; i < MB_BLOCK_SIZE / BLOCK_SIZE; i++) {
      ...
      get_block_luma(currSlice->listX[0][ref_frame], vec1_x, vec1_y, BLOCK_SIZE,
                     BLOCK_SIZE, tmp_block, dec_picture->iLumaStride,
                     dec_picture->size_x_m1,
                     (currMB->mb_field) ? (dec_picture->size_y >> 1) - 1
                                        : dec_picture->size_y_m1,
                     tmp_res, p_Vid->max_pel_value_comp[PLANE_Y],
                     (imgpel)p_Vid->dc_pred_value_comp[PLANE_Y], currMB);

      for (ii = 0; ii < BLOCK_SIZE; ii++)
        for (jj = 0; jj < MB_BLOCK_SIZE / BLOCK_SIZE; jj++)
          currSlice->mb_pred[LumaComp][jj + joff][ii + ioff] =
              tmp_block[jj][ii];
    }
  }
    ...
    for (uv = 0; uv < 2; uv++) {
      for (b8 = 0; b8 < (p_Vid->num_uv_blocks); b8++) {
        for (b4 = 0; b4 < 4; b4++) {
          ...
          for (jj = 0; jj < 4; jj++) {
            jf = (j4 + jj) /
                 (p_Vid->mb_cr_size_y / 4); // jf  = Subblock_y-coordinate
            for (ii = 0; ii < 4; ii++) {
              ifx = (i4 + ii) /
                    (p_Vid->mb_cr_size_x / 4); // ifx = Subblock_x-coordinate
              ...
              currSlice->mb_pred[uv + 1][jj + joff][ii + ioff] =
                  (imgpel)((if0 * jf0 *
                                currSlice->listX[0][ref_frame]
                                    ->imgUV[uv][jj0][ii0] +
                            if1 * jf0 *
                                currSlice->listX[0][ref_frame]
                                    ->imgUV[uv][jj0][ii1] +
 ...
 ************************************************************************
 * \brief
 *      Copies pixel values between a YUV frame and the temporary pixel value
 *storage place. This is used to save some pixel values temporarily before
 *overwriting it, or to copy back to a given location in a frame the saved pixel
 *values. \param currYBlockNum index of the block (8x8) in the Y plane \param
 *predMB memory area where the temporary pixel values are stored the Y,U,V
 *planes are concatenated y = predMB, u = predMB+256, v = predMB+320 \param
 *recfr pointer to a YUV frame \param picSizeX picture width in pixels \param
 ...
 ************************************************************************
 * \brief
 *      Calculates a weighted pixel difference between edge Y pixels of the
 *macroblock stored in predMB and the pixels in the given Y plane of a frame
 *(recY) that would become neighbor pixels if predMB was placed at currYBlockNum
 *block position into the frame. This "edge distortion" value is used to
 *determine how well the given macroblock in predMB would fit into the frame
 *when considering spatial smoothness. If there are correctly received neighbor
 *blocks (status stored in predBlocks) only they are used in calculating the
 *edge distorion; otherwise also the already concealed neighbor blocks can also
 *be used. \return The calculated weighted pixel difference at the edges of the
 *MB. \param predBlocks status array of the neighboring blocks (if they are OK,
 *concealed or lost) \param currYBlockNum index of the block (8x8) in the Y
 *plane \param predMB memory area where the temporary pixel values are stored
 *      the Y,U,V planes are concatenated y = predMB, u = predMB+256, v =
 *predMB+320 \param recY pointer to a Y plane of a YUV frame \param picSizeX
...
static int edgeDistortion(int predBlocks[], int currYBlockNum, imgpel *predMB,
  ...
  currBlock = recY + (yPosYBlock(currYBlockNum, picSizeX) << 3) * picSizeX +
    ...
    for (j = 4; j < 8; j++) {
      ...
      if (predBlocks[j] >= threshold) {
        ...
        case 6:
          ...
          for (i = 0; i < regionSize; i++) {
            distortion +=
                iabs((int)(predMB[i + currBlockOffset] - neighbor[i]));
          }
          break;
        case 7:
          ...
          for (i = 0; i < regionSize; i++) {
            distortion += iabs((int)(predMB[i * 16 + currBlockOffset] -
                                     neighbor[i * picSizeX]));
          }
          break;
        }
...
static void buildPredblockRegionYUV(VideoParameters *p_Vid, int *mv, int x,
  ...
  get_block_luma(currSlice->listX[list][ref_frame], vec1_x, vec1_y, BLOCK_SIZE,
                 BLOCK_SIZE, tmp_block, dec_picture->iLumaStride,
                 dec_picture->size_x_m1,
                 (currMB->mb_field) ? (dec_picture->size_y >> 1) - 1
                                    : dec_picture->size_y_m1,
                 currSlice->tmp_res, p_Vid->max_pel_value_comp[PLANE_Y],
                 (imgpel)p_Vid->dc_pred_value_comp[PLANE_Y], currMB);
    ...
    for (uv = 0; uv < 2; uv++) {
      ...
      for (jj = 0; jj < 2; jj++) {
        ...
        for (ii = 0; ii < 2; ii++) {
          ...
          currSlice->mb_pred[uv + 1][jj][ii] =
              (imgpel)((if0 * jf0 *
                            currSlice->listX[list][ref_frame]
                                ->imgUV[uv][jj0][ii0] +
                        if1 * jf0 *
                            currSlice->listX[list][ref_frame]
                                ->imgUV[uv][jj0][ii1] +
...
static void copy_to_conceal(StorablePicture *src, StorablePicture *dst,
  ...
  if (p_Vid->conceal_mode == 1) {
    ...
    CopyImgData(src->imgY, src->imgUV, dst->imgY, dst->imgUV, p_Vid->width,
  ...
  if (p_Vid->conceal_mode == 2) {
    if (dec_picture->chroma_format_idc != YUV400) {
      storeYUV = (imgpel *)malloc(
          (16 + (p_Vid->mb_cr_size_x * p_Vid->mb_cr_size_y) * 2 / 16) *
          sizeof(imgpel));
    } else {
    ...
    if (p_Vid->conceal_slice_type == B_SLICE)
      init_lists_for_non_reference_loss(p_Vid->p_Dpb, dst->slice_type,
                                        p_Vid->ppSliceList[0]->structure);
    else
    ...
    for (i = 0; i < mb_height * 4; i++) {
      ...
      for (j = 0; j < mb_width * 4; j++) {
        ...
        dst->mv_info[i][j].ref_idx[LIST_0] = (signed char)mv[2];
        ...
        for (ii = 0; ii < multiplier; ii++) {
          for (jj = 0; jj < multiplier; jj++) {
            dst->imgY[i * multiplier + ii][j * multiplier + jj] =
          ...
          for (uv = 0; uv < 2; uv++) {
            for (ii = 0; ii < (multiplier / 2); ii++) {
              for (jj = 0; jj < (multiplier / 2); jj++) {
                dst->imgUV[uv][i * multiplier / 2 + ii]
                          [j * multiplier / 2 + jj] =
...
*    to be concealed. The function initialize currSlice->listX[0] and list 1
*depending on current picture type
*
************************************************************************
*/
void init_lists_for_non_reference_loss(DecodedPictureBuffer *p_Dpb,
                                       int currSliceType,
      ...
      if (p_Dpb->fs[i]->concealment_reference == 1) {
        if (p_Dpb->fs[i]->frame_num > p_Vid->frame_to_conceal)
          p_Dpb->fs_ref[i]->frame_num_wrap =
              p_Dpb->fs[i]->frame_num - MaxFrameNum;
        else
  ...
  if (currSliceType == P_SLICE) {
    ...
    if (currPicStructure == FRAME) {
      ...
      qsort((void *)p_Vid->ppSliceList[0]->listX[0], list0idx,
            sizeof(StorablePicture *), compare_pic_by_pic_num_desc);
      p_Vid->ppSliceList[0]->listXsize[0] = (signed char)list0idx;
    }
  }
  ...
  if (currSliceType == B_SLICE) {
    if (currPicStructure == FRAME) {
      ...
      qsort((void *)p_Vid->ppSliceList[0]->listX[0], list0idx,
            sizeof(StorablePicture *), compare_pic_by_poc_desc);
      list0idx_1 = list0idx;
      ...
      qsort((void *)&p_Vid->ppSliceList[0]->listX[0][list0idx_1],
            list0idx - list0idx_1, sizeof(StorablePicture *),
            compare_pic_by_poc_asc);

      for (j = 0; j < list0idx_1; j++) {
        p_Vid->ppSliceList[0]->listX[1][list0idx - list0idx_1 + j] =
            p_Vid->ppSliceList[0]->listX[0][j];
      }
      for (j = list0idx_1; j < list0idx; j++) {
        p_Vid->ppSliceList[0]->listX[1][j - list0idx_1] =
            p_Vid->ppSliceList[0]->listX[0][j];
      }

      p_Vid->ppSliceList[0]->listXsize[0] =
          p_Vid->ppSliceList[0]->listXsize[1] = (signed char)list0idx;

      qsort((void *)&p_Vid->ppSliceList[0]
                ->listX[0][(short)p_Vid->ppSliceList[0]->listXsize[0]],
            list0idx - p_Vid->ppSliceList[0]->listXsize[0],
            sizeof(StorablePicture *), compare_pic_by_lt_pic_num_asc);
      qsort((void *)&p_Vid->ppSliceList[0]
                ->listX[1][(short)p_Vid->ppSliceList[0]->listXsize[0]],
            list0idx - p_Vid->ppSliceList[0]->listXsize[0],
            sizeof(StorablePicture *), compare_pic_by_lt_pic_num_asc);
      p_Vid->ppSliceList[0]->listXsize[0] =
          p_Vid->ppSliceList[0]->listXsize[1] = (signed char)list0idx;
  ...
  if ((p_Vid->ppSliceList[0]->listXsize[0] ==
       p_Vid->ppSliceList[0]->listXsize[1]) &&
      (p_Vid->ppSliceList[0]->listXsize[0] > 1)) {
    // check if lists are identical, if yes swap first two elements of listX[1]
    diff = 0;
    for (j = 0; j < p_Vid->ppSliceList[0]->listXsize[0]; j++) {
      if (p_Vid->ppSliceList[0]->listX[0][j] !=
  ...
  // set max size
  p_Vid->ppSliceList[0]->listXsize[0] = (signed char)imin(
      p_Vid->ppSliceList[0]->listXsize[0], (int)active_sps->num_ref_frames);
  p_Vid->ppSliceList[0]->listXsize[1] = (signed char)imin(
      p_Vid->ppSliceList[0]->listXsize[1], (int)active_sps->num_ref_frames);
...
StorablePicture *get_pic_from_dpb(DecodedPictureBuffer *p_Dpb, int missingpoc,
                                  unsigned int *pos) {
  VideoParameters *p_Vid = p_Dpb->p_Vid;
  int used_size = p_Dpb->used_size - 1;
  int i, concealfrom = 0;
...
static void add_node(VideoParameters *p_Vid,
                     struct concealment_node *concealment_new) {
