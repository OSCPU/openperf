...
// #define PRINT_BUFFERING_PERIOD_INFO    // uncomment to print buffering period
// SEI info #define PRINT_PCITURE_TIMING_INFO      // uncomment to print picture
// timing SEI info #define WRITE_MAP_IMAGE                // uncomment to write
// spare picture map #define PRINT_SUBSEQUENCE_INFO         // uncomment to
// print sub-sequence SEI info #define PRINT_SUBSEQUENCE_LAYER_CHAR   //
// uncomment to print sub-sequence layer characteristics SEI info #define
// PRINT_SUBSEQUENCE_CHAR         // uncomment to print sub-sequence
// characteristics SEI info #define PRINT_SCENE_INFORMATION        // uncomment
// to print scene information SEI info #define PRINT_PAN_SCAN_RECT            //
// uncomment to print pan-scan rectangle SEI info #define PRINT_RECOVERY_POINT
// // uncomment to print random access point SEI info #define
// PRINT_FILLER_PAYLOAD_INFO      // uncomment to print filler payload SEI info
// #define PRINT_DEC_REF_PIC_MARKING      // uncomment to print decoded picture
// buffer management repetition SEI info #define PRINT_RESERVED_INFO //
// uncomment to print reserved SEI info #define
// PRINT_USER_DATA_UNREGISTERED_INFO          // uncomment to print unregistered
// user data SEI info #define PRINT_USER_DATA_REGISTERED_ITU_T_T35_INFO  //
// uncomment to print ITU-T T.35 user data SEI info #define
// PRINT_FULL_FRAME_FREEZE_INFO               // uncomment to print full-frame
// freeze SEI info #define PRINT_FULL_FRAME_FREEZE_RELEASE_INFO       //
// uncomment to print full-frame freeze release SEI info #define
// PRINT_FULL_FRAME_SNAPSHOT_INFO             // uncomment to print full-frame
// snapshot SEI info #define PRINT_PROGRESSIVE_REFINEMENT_END_INFO      //
// uncomment to print Progressive refinement segment start SEI info #define
// PRINT_PROGRESSIVE_REFINEMENT_END_INFO      // uncomment to print Progressive
// refinement segment end SEI info #define
// PRINT_MOTION_CONST_SLICE_GROUP_SET_INFO    // uncomment to print
// Motion-constrained slice group set SEI info #define
// PRINT_FILM_GRAIN_CHARACTERISTICS_INFO      // uncomment to print Film grain
// characteristics SEI info #define
// PRINT_DEBLOCKING_FILTER_DISPLAY_PREFERENCE_INFO // uncomment to print
// deblocking filter display preference SEI info #define
// PRINT_STEREO_VIDEO_INFO_INFO               // uncomment to print stereo video
// SEI info #define PRINT_TONE_MAPPING                         // uncomment to
// print tone-mapping SEI info #define PRINT_POST_FILTER_HINT_INFO // uncomment
// to print post-filter hint SEI info #define
// PRINT_FRAME_PACKING_ARRANGEMENT_INFO       // uncomment to print frame
// packing arrangement SEI info
/*!
 ************************************************************************
 *  \brief
...
void InterpretSEIMessage(byte *msg, int size, VideoParameters *p_Vid,
  ...
  do {
    ...
    case SEI_DEC_REF_PIC_MARKING_REPETITION:
      interpret_dec_ref_pic_marking_repetition_info(msg + offset, payload_size,
                                                    p_Vid, pSlice);
      break;
    case SEI_SPARE_PIC:
    ...
    case SEI_SUB_SEQ_LAYER_CHARACTERISTICS:
      interpret_subsequence_layer_characteristics_info(msg + offset,
                                                       payload_size, p_Vid);
      break;
    case SEI_SUB_SEQ_CHARACTERISTICS:
      interpret_subsequence_characteristics_info(msg + offset, payload_size,
                                                 p_Vid);
      break;
    ...
    case SEI_PROGRESSIVE_REFINEMENT_SEGMENT_START:
      interpret_progressive_refinement_start_info(msg + offset, payload_size,
                                                  p_Vid);
      break;
    case SEI_PROGRESSIVE_REFINEMENT_SEGMENT_END:
      interpret_progressive_refinement_end_info(msg + offset, payload_size,
                                                p_Vid);
      break;
    case SEI_MOTION_CONSTRAINED_SLICE_GROUP_SET:
      interpret_motion_constrained_slice_group_set_info(msg + offset,
                                                        payload_size, p_Vid);
    case SEI_FILM_GRAIN_CHARACTERISTICS:
      interpret_film_grain_characteristics_info(msg + offset, payload_size,
                                                p_Vid);
      break;
    case SEI_DEBLOCKING_FILTER_DISPLAY_PREFERENCE:
      interpret_deblocking_filter_display_preference_info(msg + offset,
                                                          payload_size, p_Vid);
      break;
    ...
    case SEI_FRAME_PACKING_ARRANGEMENT:
      interpret_frame_packing_arrangement_info(msg + offset, payload_size,
                                               p_Vid);
      break;
    default:
...
void interpret_spare_pic(byte *payload, int size, VideoParameters *p_Vid) {
  ...
  signed char filename[20] = "map_dec.yuv";
  FILE *fp;
  imgpel **Y;
  static int old_pn = -1;
  static int first = 1;
  ...
  for (i = 0; i < num_spare_pics; i++) {
    ...
    case 2: // The map is compressed
            //! KS: could not check this function, description is unclear (as
            //! stated in Ed. Note)
      bit0 = 0;
      bit1 = 1;
      bitc = bit0;
      ...
      for (k = 0; k < 2; k++)
        for (i = 0; i < p_Vid->height >> 1; i++)
          for (j = 0; j < p_Vid->width >> 1; j++)
            fwrite(&(p_Vid->dc_pred_value_comp[1]), symbol_size_in_bytes, 1,
                   p_out);
    }
    fclose(fp);
    free_mem2Dpel(Y);
...
void interpret_subsequence_info(byte *payload, int size,
  ...
  Bitstream *buf;
  int sub_seq_layer_num, sub_seq_id, first_ref_pic_flag,
      leading_non_ref_pic_flag, last_pic_flag, sub_seq_frame_num_flag,
      sub_seq_frame_num;
...
void interpret_subsequence_layer_characteristics_info(byte *payload, int size,
                                                      VideoParameters *p_Vid) {
  Bitstream *buf;
  long num_sub_layers, accurate_statistics_flag, average_bit_rate,
      average_frame_rate;
  int i;
...
void interpret_subsequence_characteristics_info(byte *payload, int size,
  ...
  Bitstream *buf;
  int i;
  int sub_seq_layer_num, sub_seq_id, duration_flag, average_rate_flag,
      accurate_statistics_flag;
  unsigned long sub_seq_duration, average_bit_rate, average_frame_rate;
  int num_referenced_subseqs, ref_sub_seq_layer_num, ref_sub_seq_id,
...
void interpret_user_data_registered_itu_t_t35_info(byte *payload, int size,
                                                   VideoParameters *p_Vid) {
  int offset = 0;
  byte itu_t_t35_country_code, itu_t_t35_country_code_extension_byte,
      payload_byte;
...
void interpret_recovery_point_info(byte *payload, int size,
                                   VideoParameters *p_Vid) {
  int recovery_frame_cnt, exact_match_flag, broken_link_flag,
...
void interpret_dec_ref_pic_marking_repetition_info(byte *payload, int size,
  ...
  int old_idr_flag, old_no_output_of_prior_pics_flag,
      old_long_term_reference_flag, old_adaptive_ref_pic_buffering_flag;
  ...
  // we need to save everything that is probably overwritten in
  // dec_ref_pic_marking()
  old_drpm = pSlice->dec_ref_pic_marking_buffer;
  old_idr_flag = pSlice->idr_flag; // p_Vid->idr_flag;

  old_no_output_of_prior_pics_flag =
      pSlice
          ->no_output_of_prior_pics_flag; // p_Vid->no_output_of_prior_pics_flag;
  ...
  if (p_Vid->idr_flag) {
    ...
    if (p_Vid->adaptive_ref_pic_buffering_flag) {
      ...
      while (tmp_drpm != NULL) {
        ...
        if ((tmp_drpm->memory_management_control_operation == 1) ||
            (tmp_drpm->memory_management_control_operation == 3)) {
        ...
        if ((tmp_drpm->memory_management_control_operation == 3) ||
            (tmp_drpm->memory_management_control_operation == 6)) {
...
void interpret_motion_constrained_slice_group_set_info(byte *payload, int size,
                                                       VideoParameters *p_Vid) {
  int num_slice_groups_minus1, slice_group_id, exact_match_flag,
      pan_scan_rect_flag, pan_scan_rect_id;
  int i;
  int sliceGroupSize;
...
void interpret_film_grain_characteristics_info(byte *payload, int size,
  ...
  int film_grain_bit_depth_luma_minus8, film_grain_bit_depth_chroma_minus8,
      film_grain_full_range_flag, film_grain_colour_primaries,
      film_grain_transfer_characteristics, film_grain_matrix_coefficients;
  ...
  film_grain_characteristics_cancel_flag =
      u_1("SEI: film_grain_characteristics_cancel_flag", buf);
    ...
    for (c = 0; c < 3; c++)
      if (comp_model_present_flag[c]) {
        num_intensity_intervals_minus1 =
            u_v(8, "SEI: num_intensity_intervals_minus1", buf);
...
void interpret_deblocking_filter_display_preference_info(
  ...
  int display_prior_to_deblocking_preferred_flag,
  ...
  deblocking_display_preference_cancel_flag =
      u_1("SEI: deblocking_display_preference_cancel_flag", buf);
...
void interpret_stereo_video_info_info(byte *payload, int size,
  ...
  int top_field_is_left_view_flag, current_frame_is_left_view_flag,
...
void interpret_buffering_period_info(byte *payload, int size,
                                     VideoParameters *p_Vid) {
  int seq_parameter_set_id, initial_cpb_removal_delay,
  ...
  // Note: NalHrdBpPresentFlag and CpbDpbDelaysPresentFlag can also be set "by
  // some means not specified in this Recommendation | International Standard"
  if (sps->vui_parameters_present_flag) {

    if (sps->vui_seq_parameters.nal_hrd_parameters_present_flag) {
      for (k = 0;
           k < sps->vui_seq_parameters.nal_hrd_parameters.cpb_cnt_minus1 + 1;
           k++) {
        initial_cpb_removal_delay =
            u_v(sps->vui_seq_parameters.nal_hrd_parameters
    ...
    if (sps->vui_seq_parameters.vcl_hrd_parameters_present_flag) {
      for (k = 0;
           k < sps->vui_seq_parameters.vcl_hrd_parameters.cpb_cnt_minus1 + 1;
           k++) {
        initial_cpb_removal_delay =
            u_v(sps->vui_seq_parameters.vcl_hrd_parameters
                        .initial_cpb_removal_delay_length_minus1 +
                    1,
                "SEI: initial_cpb_removal_delay", buf);
        initial_cpb_removal_delay_offset =
...
void interpret_picture_timing_info(byte *payload, int size,
  ...
  int cpb_removal_delay, dpb_output_delay, picture_structure_present_flag,
      picture_structure;
  int clock_time_stamp_flag;
  int ct_type, nuit_field_based_flag, counting_type, full_timestamp_flag,
      discontinuity_flag, cnt_dropped_flag, nframes;
  int seconds_value, minutes_value, hours_value, seconds_flag, minutes_flag,
      hours_flag, time_offset;
  int NumClockTs = 0;
  int i;
  ...
  // CpbDpbDelaysPresentFlag can also be set "by some means not specified in
  // this Recommendation | International Standard"
  CpbDpbDelaysPresentFlag =
      (Boolean)(active_sps->vui_parameters_present_flag &&
                ((active_sps->vui_seq_parameters
  ...
  if (CpbDpbDelaysPresentFlag) {
    if (active_sps->vui_parameters_present_flag) {
      if (active_sps->vui_seq_parameters.nal_hrd_parameters_present_flag) {
        cpb_removal_len = active_sps->vui_seq_parameters.nal_hrd_parameters
                              .cpb_removal_delay_length_minus1 +
                          1;
        dpb_output_len = active_sps->vui_seq_parameters.nal_hrd_parameters
                             .dpb_output_delay_length_minus1 +
                         1;
      } else if (active_sps->vui_seq_parameters
                     .vcl_hrd_parameters_present_flag) {
        cpb_removal_len = active_sps->vui_seq_parameters.vcl_hrd_parameters
                              .cpb_removal_delay_length_minus1 +
                          1;
        dpb_output_len = active_sps->vui_seq_parameters.vcl_hrd_parameters
                             .dpb_output_delay_length_minus1 +
                         1;
      }
    }
  ...
  if (!active_sps->vui_parameters_present_flag) {
    ...
    picture_structure_present_flag =
        active_sps->vui_seq_parameters.pic_struct_present_flag;
  }
  ...
  if (picture_structure_present_flag) {
    ...
    case 8:
      ...
      break;
    default:
      error("reserved picture_structure used (can't determine NumClockTs)",
            500);
    }
      ...
      if (clock_time_stamp_flag) {
        ...
        }
        {
          ...
          if (active_sps->vui_seq_parameters.vcl_hrd_parameters_present_flag)
            time_offset_length = active_sps->vui_seq_parameters
                                     .vcl_hrd_parameters.time_offset_length;
          else if (active_sps->vui_seq_parameters
                       .nal_hrd_parameters_present_flag)
            time_offset_length = active_sps->vui_seq_parameters
                                     .nal_hrd_parameters.time_offset_length;
          else
...
void interpret_frame_packing_arrangement_info(byte *payload, int size,
  ...
  seiFramePackingArrangement.frame_packing_arrangement_id =
      (unsigned int)ue_v("SEI: frame_packing_arrangement_id", buf);
  seiFramePackingArrangement.frame_packing_arrangement_cancel_flag =
      u_1("SEI: frame_packing_arrangement_cancel_flag", buf);
#ifdef PRINT_FRAME_PACKING_ARRANGEMENT_INFO
  ...
  if (seiFramePackingArrangement.frame_packing_arrangement_cancel_flag ==
    ...
    if (seiFramePackingArrangement.quincunx_sampling_flag == FALSE &&
      ...
      seiFramePackingArrangement.frame0_grid_position_x =
          (unsigned char)u_v(4, "SEI: frame0_grid_position_x", buf);
      seiFramePackingArrangement.frame0_grid_position_y =
          (unsigned char)u_v(4, "SEI: frame0_grid_position_y", buf);
      seiFramePackingArrangement.frame1_grid_position_x =
          (unsigned char)u_v(4, "SEI: frame1_grid_position_x", buf);
      seiFramePackingArrangement.frame1_grid_position_y =
          (unsigned char)u_v(4, "SEI: frame1_grid_position_y", buf);
#ifdef PRINT_FRAME_PACKING_ARRANGEMENT_INFO
    ...
    seiFramePackingArrangement.frame_packing_arrangement_repetition_period =
        (unsigned int)ue_v("SEI: frame_packing_arrangement_repetition_period",
                           buf);
#ifdef PRINT_FRAME_PACKING_ARRANGEMENT_INFO
    printf("frame_packing_arrangement_reserved_byte          = %d\n",
           seiFramePackingArrangement.frame_packing_arrangement_reserved_byte);
    printf(
        "frame_packing_arrangement_repetition_period      = %d\n",
        seiFramePackingArrangement.frame_packing_arrangement_repetition_period);
#endif
  }
  seiFramePackingArrangement.frame_packing_arrangement_extension_flag =
      u_1("SEI: frame_packing_arrangement_extension_flag", buf);
#ifdef PRINT_FRAME_PACKING_ARRANGEMENT_INFO
...
void interpret_tone_mapping(byte *payload, int size, VideoParameters *p_Vid) {
  ...
  seiToneMappingTmp.tone_map_cancel_flag =
      (unsigned char)u_1("SEI: tone_map_cancel_flag", buf);
  ...
  if (seiToneMappingTmp.tone_map_id != 0)
    printf("WARNING! Tone_map_id != 0, print the SEI message info only. The "
           "tone mapping is actually applied only when Tone_map_id==0\n\n");
  printf("tone_map_cancel_flag = %d\n", seiToneMappingTmp.tone_map_cancel_flag);
#endif

  if (!seiToneMappingTmp.tone_map_cancel_flag) {
    seiToneMappingTmp.tone_map_repetition_period =
        ue_v("SEI: tone_map_repetition_period", buf);
    seiToneMappingTmp.coded_data_bit_depth =
        (unsigned char)u_v(8, "SEI: coded_data_bit_depth", buf);
    seiToneMappingTmp.sei_bit_depth =
        (unsigned char)u_v(8, "SEI: sei_bit_depth", buf);
    ...
    if (seiToneMappingTmp.model_id == 0) { // linear mapping with clipping
...
#endif
    } else if (seiToneMappingTmp.model_id == 1) { // sigmoidal mapping
      seiToneMappingTmp.sigmoid_midpoint =
          u_v(32, "SEI: sigmoid_midpoint", buf);
      seiToneMappingTmp.sigmoid_width = u_v(32, "SEI: sigmoid_width", buf);
#ifdef PRINT_TONE_MAPPING
      printf("sigmoid_midpoint = %d, sigmoid_width = %d\n",
             seiToneMappingTmp.sigmoid_midpoint,
             seiToneMappingTmp.sigmoid_width);
#endif
    } else if (seiToneMappingTmp.model_id == 2) { // user defined table mapping
      for (i = 0; i < max_output_num; i++) {
        seiToneMappingTmp.start_of_coded_interval[i] =
            u_v((((seiToneMappingTmp.coded_data_bit_depth + 7) >> 3) << 3),
                "SEI: start_of_coded_interval", buf);
#ifdef PRINT_TONE_MAPPING // too long to print
                          // printf("start_of_coded_interval[%d] = %d\n", i,
        // seiToneMappingTmp.start_of_coded_interval[i]);
#endif
      }
    } else if (seiToneMappingTmp.model_id == 3) { // piece-wise linear mapping
      ...
      seiToneMappingTmp.coded_pivot_value[seiToneMappingTmp.num_pivots + 1] =
          max_coded_num - 1;
      seiToneMappingTmp.sei_pivot_value[seiToneMappingTmp.num_pivots + 1] =
          max_output_num - 1;

      for (i = 1; i < seiToneMappingTmp.num_pivots + 1; i++) {
        seiToneMappingTmp.coded_pivot_value[i] =
            u_v((((seiToneMappingTmp.coded_data_bit_depth + 7) >> 3) << 3),
                "SEI: coded_pivot_value", buf);
        seiToneMappingTmp.sei_pivot_value[i] =
            u_v((((seiToneMappingTmp.sei_bit_depth + 7) >> 3) << 3),
                "SEI: sei_pivot_value", buf);
#ifdef PRINT_TONE_MAPPING
        printf("coded_pivot_value[%d] = %d, sei_pivot_value[%d] = %d\n", i,
               seiToneMappingTmp.coded_pivot_value[i], i,
               seiToneMappingTmp.sei_pivot_value[i]);
#endif
      }
    }
    ...
    // Currently, only when the map_id == 0, the tone-mapping is actually
    // applied.
    if (seiToneMappingTmp.tone_map_id == 0) {
      int j;
      p_Vid->seiToneMapping->seiHasTone_mapping = TRUE;
      p_Vid->seiToneMapping->tone_map_repetition_period =
          seiToneMappingTmp.tone_map_repetition_period;
      p_Vid->seiToneMapping->coded_data_bit_depth =
          seiToneMappingTmp.coded_data_bit_depth;
      ...
      case 0: // linear mapping with clipping
        ...
        for (i = seiToneMappingTmp.min_value + 1;
             i < seiToneMappingTmp.max_value; i++)
          p_Vid->seiToneMapping->lut[i] =
              (imgpel)((i - seiToneMappingTmp.min_value) *
                       (max_output_num - 1) /
                       (seiToneMappingTmp.max_value -
                        seiToneMappingTmp.min_value));
      ...
      case 1: // sigmoid mapping
        ...
        for (i = 0; i < max_coded_num; i++) {
          double tmp =
              1.0 + exp(-6 * (double)(i - seiToneMappingTmp.sigmoid_midpoint) /
                        seiToneMappingTmp.sigmoid_width);
          p_Vid->seiToneMapping->lut[i] =
              (imgpel)((double)(max_output_num - 1) / tmp + 0.5);
        }
        break;
      case 2: // user defined table
        if (0 < max_output_num - 1) {
          for (j = 0; j < max_output_num - 1; j++) {
            for (i = seiToneMappingTmp.start_of_coded_interval[j];
      ...
      case 3: // piecewise linear mapping
        for (j = 0; j < seiToneMappingTmp.num_pivots + 1; j++) {
          double slope = (double)(seiToneMappingTmp.sei_pivot_value[j + 1] -
                                  seiToneMappingTmp.sei_pivot_value[j]) /
                         (seiToneMappingTmp.coded_pivot_value[j + 1] -
                          seiToneMappingTmp.coded_pivot_value[j]);
          for (i = seiToneMappingTmp.coded_pivot_value[j];
               i <= seiToneMappingTmp.coded_pivot_value[j + 1]; i++) {
            p_Vid->seiToneMapping->lut[i] =
                (imgpel)(seiToneMappingTmp.sei_pivot_value[j] +
                         (int)(((i - seiToneMappingTmp.coded_pivot_value[j]) *
                                slope)));
          }
        }
        break;
...
#if (ENABLE_OUTPUT_TONEMAPPING)
// tone map using the look-up-table generated according to SEI tone mapping
// message
...
void interpret_post_filter_hints_info(byte *payload, int size,
                                      VideoParameters *p_Vid) {
  Bitstream *buf;
  unsigned int filter_hint_size_y, filter_hint_size_x, filter_hint_type,
      color_component, cx, cy, additional_extension_flag;
  int ***filter_hint;
  ...
  filter_hint_size_y = ue_v("SEI: filter_hint_size_y",
                            buf); // interpret post-filter hint SEI here
  filter_hint_size_x = ue_v("SEI: filter_hint_size_x",
                            buf); // interpret post-filter hint SEI here
  filter_hint_type = u_v(2, "SEI: filter_hint_type",
                         buf); // interpret post-filter hint SEI here
  ...
  for (color_component = 0; color_component < 3; color_component++)
    for (cy = 0; cy < filter_hint_size_y; cy++)
      for (cx = 0; cx < filter_hint_size_x; cx++)
        filter_hint[color_component][cy][cx] = se_v(
            "SEI: filter_hint", buf); // interpret post-filter hint SEI here

  additional_extension_flag = u_1("SEI: additional_extension_flag",
                                  buf); // interpret post-filter hint SEI here
    ...
    for (cy = 0; cy < filter_hint_size_y; cy++)
      for (cx = 0; cx < filter_hint_size_x; cx++)
        printf(" post_filter_hint[%d][%d][%d] %d \n", color_component, cy, cx,
               filter_hint[color_component][cy][cx]);
