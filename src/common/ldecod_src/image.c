...
static void init_picture(VideoParameters *p_Vid, Slice *currSlice,
  ...
  if (p_Vid->recovery_point)
    p_Vid->recovery_frame_num =
        (currSlice->frame_num + p_Vid->recovery_frame_cnt) % p_Vid->MaxFrameNum;
  ...
  if (p_Vid->recovery_point == 0 &&
    ...
    if (active_sps->gaps_in_frame_num_value_allowed_flag == 0) {
      ...
      if (p_Inp->conceal_mode != 0) {
        if ((currSlice->frame_num) <
            ((p_Vid->pre_frame_num + 1) % p_Vid->MaxFrameNum)) {
      ...
      } else { /* Advanced Error Concealment would be called here to combat
                  unintentional loss of pictures. */
  ...
  if (p_Vid->recovery_frame_num == (int)currSlice->frame_num &&
...
#endif /* !SPEC */
  dec_picture = p_Vid->dec_picture =
      alloc_storable_picture(p_Vid, currSlice->structure, p_Vid->width,
                             p_Vid->height, p_Vid->width_cr, p_Vid->height_cr);
  ...
  dec_picture->chroma_qp_offset[1] =
      p_Vid->active_pps->second_chroma_qp_index_offset;
  dec_picture->iCodingType =
      currSlice->structure == FRAME
          ? (currSlice->mb_aff_frame_flag ? FRAME_MB_PAIR_CODING : FRAME_CODING)
          : FIELD_CODING; // currSlice->slice_type;
  ...
  // reset all variables of the error concealment instance before decoding of
  // every frame. here the third parameter should, if perfectly, be equal to the
  // number of slices per frame. using little value is ok, the code will
  // allocate more memory if the slice number is larger
  ercReset(p_Vid->erc_errorVar, p_Vid->PicSizeInMbs, p_Vid->PicSizeInMbs,
           dec_picture->size_x);
  p_Vid->erc_mvperMB = 0;
  ...
  if (p_Vid->active_pps->entropy_coding_mode_flag == CAVLC) {
    memset(p_Vid->nz_coeff[0][0][0], -1,
           p_Vid->PicSizeInMbs * 48 * sizeof(byte)); // 3 * 4 * 4
  }

  // Set the slice_nr member of each MB to -1, to ensure correct when packet
  // loss occurs TO set Macroblock Map (mark all MBs as 'have to be concealed')
  if ((p_Vid->separate_colour_plane_flag != 0)) {
    for (nplane = 0; nplane < MAX_PLANE; ++nplane) {
      Macroblock *currMB = p_Vid->mb_data_JV[nplane];
      signed char *intra_block = p_Vid->intra_block_JV[nplane];
      for (i = 0; i < (int)p_Vid->PicSizeInMbs; ++i) {
        reset_mbs(currMB++);
      }
      fast_memset(p_Vid->ipredmode_JV[nplane][0], DC_PRED,
                  16 * p_Vid->FrameHeightInMbs * p_Vid->PicWidthInMbs *
                      sizeof(char));
...
#if 0 && (defined(_OPENMP) || defined(SPEC_OPENMP)) &&                         \
...
#pragma omp parallel for
    ...
    fast_memset(p_Vid->ipredmode[0], DC_PRED,
                16 * p_Vid->FrameHeightInMbs * p_Vid->PicWidthInMbs *
                    sizeof(char));
  }
  ...
  dec_picture->dec_ref_pic_marking_buffer =
      currSlice->dec_ref_pic_marking_buffer;
  ...
  p_Vid->get_mb_block_pos = dec_picture->mb_aff_frame_flag
                                ? get_mb_block_pos_mbaff
                                : get_mb_block_pos_normal;
  p_Vid->getNeighbour =
      dec_picture->mb_aff_frame_flag ? getAffNeighbour : getNonAffNeighbour;

  dec_picture->pic_num = currSlice->frame_num;
  dec_picture->frame_num = currSlice->frame_num;

  dec_picture->recovery_frame =
      (unsigned int)((int)currSlice->frame_num == p_Vid->recovery_frame_num);
  ...
  if (dec_picture->frame_cropping_flag) {
    dec_picture->frame_cropping_rect_left_offset =
        active_sps->frame_cropping_rect_left_offset;
    dec_picture->frame_cropping_rect_right_offset =
        active_sps->frame_cropping_rect_right_offset;
    dec_picture->frame_cropping_rect_top_offset =
        active_sps->frame_cropping_rect_top_offset;
    dec_picture->frame_cropping_rect_bottom_offset =
        active_sps->frame_cropping_rect_bottom_offset;
  }
  ...
  if (p_Vid->seiToneMapping->seiHasTone_mapping) {
    ...
    memcpy(dec_picture->tone_mapping_lut, p_Vid->seiToneMapping->lut,
           sizeof(imgpel) * coded_data_bit_max);
  ...
  if ((p_Vid->separate_colour_plane_flag != 0)) {
    ...
    p_Vid->dec_picture_JV[1] = alloc_storable_picture(
        p_Vid, (PictureStructure)currSlice->structure, p_Vid->width,
        p_Vid->height, p_Vid->width_cr, p_Vid->height_cr);
    copy_dec_picture_JV(p_Vid, p_Vid->dec_picture_JV[1],
                        p_Vid->dec_picture_JV[0]);
    p_Vid->dec_picture_JV[2] = alloc_storable_picture(
        p_Vid, (PictureStructure)currSlice->structure, p_Vid->width,
        p_Vid->height, p_Vid->width_cr, p_Vid->height_cr);
    copy_dec_picture_JV(p_Vid, p_Vid->dec_picture_JV[2],
                        p_Vid->dec_picture_JV[0]);
  }
}

static void update_mbaff_macroblock_data(imgpel **cur_img, imgpel (*temp)[16],
                                         int x0, int width, int height) {
...
static void MbAffPostProc(VideoParameters *p_Vid) {
  ...
  for (i = 0; i < (int)dec_picture->PicSizeInMbs; i += 2) {
    if (dec_picture->motion.mb_field[i]) {
      ...
      update_mbaff_macroblock_data(imgY + y0, temp_buffer, x0, MB_BLOCK_SIZE,
                                   MB_BLOCK_SIZE);
      ...
      if (dec_picture->chroma_format_idc != YUV400) {
        ...
        update_mbaff_macroblock_data(imgUV[0] + y0, temp_buffer, x0,
                                     p_Vid->mb_cr_size_x, p_Vid->mb_cr_size_y);
        update_mbaff_macroblock_data(imgUV[1] + y0, temp_buffer, x0,
                                     p_Vid->mb_cr_size_x, p_Vid->mb_cr_size_y);
      }
    }
  }
    ...
    if (currSlice->active_pps->weighted_bipred_idc == 2) {
      ...
      for (i = 0; i < MAX_REFERENCE_PICTURES; ++i) {
        for (comp = 0; comp < 3; ++comp) {
          log_weight_denom = (comp == 0) ? currSlice->luma_log2_weight_denom
        ...
        for (comp = 0; comp < 3; ++comp) {
          ...
          if (currSlice->active_pps->weighted_bipred_idc == 1) {
            currSlice->wbp_weight[0][i][j][comp] =
                currSlice->wp_weight[0][i][comp];
            currSlice->wbp_weight[1][i][j][comp] =
                currSlice->wp_weight[1][j][comp];
          } else if (currSlice->active_pps->weighted_bipred_idc == 2) {
            td = iClip3(-128, 127,
                        currSlice->listX[LIST_1][j]->poc -
                            currSlice->listX[LIST_0][i]->poc);
            if (td == 0 || currSlice->listX[LIST_1][j]->is_long_term ||
              ...
              tb =
                  iClip3(-128, 127,
                         currSlice->ThisPOC - currSlice->listX[LIST_0][i]->poc);
              ...
              if (currSlice->wbp_weight[1][i][j][comp] < -64 ||
                  currSlice->wbp_weight[1][i][j][comp] > 128) {
                currSlice->wbp_weight[0][i][j][comp] = 32;
                currSlice->wbp_weight[1][i][j][comp] = 32;
                currSlice->wp_offset[0][i][comp] = 0;
    ...
    if (currSlice->mb_aff_frame_flag) {
      for (i = 0; i < 2 * max_l0_ref; ++i) {
        for (j = 0; j < 2 * max_l1_ref; ++j) {
          for (comp = 0; comp < 3; ++comp) {
            for (k = 2; k < 6; k += 2) {
              currSlice->wp_offset[k + 0][i][comp] =
                  currSlice->wp_offset[0][i >> 1][comp];
              currSlice->wp_offset[k + 1][j][comp] =
                  currSlice->wp_offset[1][j >> 1][comp];

              log_weight_denom = (comp == 0)
                                     ? currSlice->luma_log2_weight_denom
                                     : currSlice->chroma_log2_weight_denom;
              if (currSlice->active_pps->weighted_bipred_idc == 1) {
                currSlice->wbp_weight[k + 0][i][j][comp] =
                    currSlice->wp_weight[0][i >> 1][comp];
                currSlice->wbp_weight[k + 1][i][j][comp] =
                    currSlice->wp_weight[1][j >> 1][comp];
              } else if (currSlice->active_pps->weighted_bipred_idc == 2) {
                td = iClip3(-128, 127,
                            currSlice->listX[k + LIST_1][j]->poc -
                                currSlice->listX[k + LIST_0][i]->poc);
                if (td == 0 || currSlice->listX[k + LIST_1][j]->is_long_term ||
                  ...
                  tb = iClip3(
                      -128, 127,
                      ((k == 2) ? currSlice->toppoc : currSlice->bottompoc) -
                          currSlice->listX[k + LIST_0][i]->poc);
...
static void init_picture_decoding(VideoParameters *p_Vid) {
  ...
  if (p_Vid->iSliceNumOfCurrPic >= MAX_NUM_SLICES) {
    error("Maximum number of supported slices exceeded. \nPlease recompile "
          "with increased value for MAX_NUM_SLICES",
          200);
  }

  if (p_Vid->pNextPPS->Valid && (int)p_Vid->pNextPPS->pic_parameter_set_id ==
                                    pSlice->pic_parameter_set_id) {
    ...
    MakePPSavailable(p_Vid, p_Vid->pNextPPS->pic_parameter_set_id,
                     p_Vid->pNextPPS);
  ...
  p_Vid->PicHeightInMbs =
      p_Vid->FrameHeightInMbs / (1 + pSlice->field_pic_flag);
  ...
  if (p_Vid->iDeblockMode == 1) {
    for (j = 0; j < p_Vid->iSliceNumOfCurrPic; j++) {
      ...
      for (i = pSlice->start_mb_nr * (1 + pSlice->mb_aff_frame_flag);
           i < pSlice->end_mb_nr_plus1 * (1 + pSlice->mb_aff_frame_flag); i++)
...
void init_slice(VideoParameters *p_Vid, Slice *currSlice) {
  ...
  if (!(currSlice->redundant_pic_cnt != 0 &&
        p_Vid->previous_frame_num == currSlice->frame_num)) {
    for (i = 16; i > 0; i--) {
      currSlice->ref_flag[i] = currSlice->ref_flag[i - 1];
    }
  }
  currSlice->ref_flag[0] = currSlice->redundant_pic_cnt == 0
                               ? p_Vid->Is_primary_correct
...
void decode_slice(Slice *currSlice, int current_header) {
  ...
  if (p_Vid->iDeblockMode &&
      (p_Vid->bDeblockEnable &
       (1 << (p_Vid->dec_picture->used_for_reference)))) {
    if ((p_Vid->separate_colour_plane_flag != 0)) {
      change_plane_JV(p_Vid, currSlice->colour_plane_id, currSlice);
      DeblockPicturePartially(p_Vid, currSlice->dec_picture,
                              currSlice->start_mb_nr * iScale,
                              currSlice->end_mb_nr_plus1 * iScale);
    } else {
      DeblockPicturePartially(p_Vid, currSlice->dec_picture,
                              currSlice->start_mb_nr * iScale,
                              currSlice->end_mb_nr_plus1 * iScale);
...
static void Error_tracking(VideoParameters *p_Vid, Slice *currSlice) {
  ...
  if (currSlice->redundant_pic_cnt == 0 && p_Vid->type != I_SLICE) {
    for (i = 0; i < currSlice->num_ref_idx_active[LIST_0]; ++i) {
      if (currSlice->ref_flag[i] ==
          0) // any reference of primary slice is incorrect
      {
        p_Vid->Is_primary_correct = 0; // primary slice is incorrect
      }
    }
  } else if (currSlice->redundant_pic_cnt != 0 && p_Vid->type != I_SLICE) {
    if (currSlice->ref_flag[currSlice->redundant_slice_ref_idx] ==
        0) // reference of redundant slice is incorrect
    {
...
int decode_one_frame(DecoderParams *pDecoder) {
  ...
  if (p_Vid->newframe) {
    if (p_Vid->pNextPPS->Valid && (int)p_Vid->pNextPPS->pic_parameter_set_id ==
                                      p_Vid->pNextSlice->pic_parameter_set_id) {
      MakePPSavailable(p_Vid, p_Vid->pNextPPS->pic_parameter_set_id,
                       p_Vid->pNextPPS);
  ...
  while (current_header != SOP && current_header != EOS) {
    ...
    // If primary and redundant are received and primary is correct, discard the
    // redundant else, primary slice will be replaced with redundant slice.
    if (currSlice->frame_num == p_Vid->previous_frame_num &&
        currSlice->redundant_pic_cnt != 0 && p_Vid->Is_primary_correct != 0 &&
        current_header != EOS) {
      continue;
    }
    ...
    if ((current_header != SOP && current_header != EOS) ||
      ...
      p_Vid->dec_picture->max_slice_id = (short)imax(
          currSlice->current_slice_nr, p_Vid->dec_picture->max_slice_id);
      if (p_Vid->iSliceNumOfCurrPic > 0) {
        CopyPOC(*ppSliceList, currSlice);
        ppSliceList[p_Vid->iSliceNumOfCurrPic - 1]->end_mb_nr_plus1 =
      ...
      if (p_Vid->iSliceNumOfCurrPic >= p_Vid->iNumOfSlicesAllocated) {
        ...
        if (!tmpSliceList) {
          tmpSliceList =
              calloc((p_Vid->iNumOfSlicesAllocated + MAX_NUM_DECSLICES),
                     sizeof(Slice *));
          memcpy(tmpSliceList, p_Vid->ppSliceList,
                 p_Vid->iSliceNumOfCurrPic * sizeof(Slice *));
          // free;
          ...
          ppSliceList = p_Vid->ppSliceList = tmpSliceList;
          memset(p_Vid->ppSliceList + p_Vid->iSliceNumOfCurrPic, 0,
                 sizeof(Slice *) * MAX_NUM_DECSLICES);
        }
        p_Vid->iNumOfSlicesAllocated += MAX_NUM_DECSLICES;
      }
      current_header = SOS;
      ...
      if (ppSliceList[p_Vid->iSliceNumOfCurrPic - 1]->mb_aff_frame_flag)
        ppSliceList[p_Vid->iSliceNumOfCurrPic - 1]->end_mb_nr_plus1 =
            p_Vid->FrameSizeInMbs / 2;
      else
        ppSliceList[p_Vid->iSliceNumOfCurrPic - 1]->end_mb_nr_plus1 =
            p_Vid->FrameSizeInMbs /
            (1 + ppSliceList[p_Vid->iSliceNumOfCurrPic - 1]->field_pic_flag);
      p_Vid->newframe = 1;
 ...
 ************************************************************************
 */
void buffer2img(imgpel **imgX, unsigned char *buf, int size_x, int size_y,
  ...
  if ((sizeof(char) == sizeof(imgpel)) &&
      (sizeof(char) == symbol_size_in_bytes)) {
    ...
    if (testEndian()) {
      ...
      if (symbol_size_in_bytes == 1) {
        ...
        for (j = 0; j < size_y; ++j) {
          ...
          for (i = 0; i < size_x; ++i) {
            imgX[j][i] = 0;
            memcpy(&(imgX[j][i]), buf + ((i + jpos) * symbol_size_in_bytes),
 ...
 ***********************************************************************
 */
int64 compute_SSE(imgpel **imgRef, imgpel **imgSrc, int xRef, int xSrc,
                  int ySize, int xSize) {
  int i, j;
...
void calculate_frame_no(VideoParameters *p_Vid, StorablePicture *p) {
  ...
  if (psnrPOC == 0) // && p_Vid->psnr_number)
  {
    p_Vid->idr_psnr_number =
        p_Vid->number * p_Vid->ref_poc_gap / (p_Inp->poc_scale);
  }
  p_Vid->psnr_number =
      imax(p_Vid->psnr_number, p_Vid->idr_psnr_number + psnrPOC);
...
void find_snr(VideoParameters *p_Vid, StorablePicture *p, int *p_ref) {
  ...
  Boolean rgb_output =
      (Boolean)(p_Vid->active_sps->vui_seq_parameters.matrix_coefficients == 0);
  unsigned char *buf;
  imgpel **cur_ref[3];
  imgpel **cur_comp[3];
  // picture error concealment
  ...
  framesize_in_bytes = (((int64)comp_size_x[0] * comp_size_y[0]) +
                        ((int64)comp_size_x[1] * comp_size_y[1]) * 2) *
                       symbol_size_in_bytes;

  // KS: this buffer should actually be allocated only once, but this is still
  ...
  if (status == -1) {
    fprintf(stderr,
            "Warning: Could not seek to frame number %d in reference file. "
            "Shown PSNR might be wrong.\n",
            p_Vid->frame_no);
    free(buf);
    return;
  }
  ...
  for (k = 0; k < ((p->chroma_format_idc != YUV400) ? 3 : 1); ++k) {
    ...
    ret = read(*p_ref, buf,
               comp_size_x[k] * comp_size_y[k] * symbol_size_in_bytes);
    ...
    buffer2img(cur_ref[k], buf, comp_size_x[k], comp_size_y[k],
               symbol_size_in_bytes);

    // Compute SSE
    diff_comp[k] = compute_SSE(cur_ref[k], cur_comp[k], 0, 0, comp_size_y[k],
                               comp_size_x[k]);

    // Collecting SNR statistics
    snr->snr[k] = psnr(max_pix_value_sqd[k], comp_size_x[k] * comp_size_y[k],
                       (float)diff_comp[k]);
    ...
    if (snr->frame_ctr == 0) // first
    {
      ...
      snr->snra[k] = (float)(snr->snra[k] * (snr->frame_ctr) + snr->snr[k]) /
                     (snr->frame_ctr + 1); // average snr chroma for all frames
    }
  }
  ...
  if (p->concealed_pic) {
    fprintf(stdout, "%04d(P)  %8d %5d %5d %7.4f %7.4f %7.4f  %s %5d\n",
            p_Vid->frame_no, p->frame_poc, p->pic_num, p->qp, snr->snr[0],
            snr->snr[1], snr->snr[2], yuv_types[p->chroma_format_idc], 0);
  }
}
...
void reorder_lists(Slice *currSlice) {
  ...
  if ((currSlice->slice_type != I_SLICE) &&
    ...
    if (p_Vid->no_reference_picture ==
        currSlice->listX[0][currSlice->num_ref_idx_active[LIST_0] - 1]) {
      if (p_Vid->non_conforming_stream)
        printf("RefPicList0[ num_ref_idx_l0_active_minus1 ] is equal to 'no "
               "reference picture'\n");
      else
        error("RefPicList0[ num_ref_idx_l0_active_minus1 ] is equal to 'no "
              "reference picture', invalid bitstream",
              500);
    }
    // that's a definition
    currSlice->listXsize[0] =
        (signed char)currSlice->num_ref_idx_active[LIST_0];
  }
  if (currSlice->slice_type == B_SLICE) {
    ...
    if (p_Vid->no_reference_picture ==
        currSlice->listX[1][currSlice->num_ref_idx_active[LIST_1] - 1]) {
      if (p_Vid->non_conforming_stream)
        printf("RefPicList1[ num_ref_idx_l1_active_minus1 ] is equal to 'no "
               "reference picture'\n");
      else
        error("RefPicList1[ num_ref_idx_l1_active_minus1 ] is equal to 'no "
              "reference picture', invalid bitstream",
              500);
    }
    // that's a definition
    currSlice->listXsize[1] =
        (signed char)currSlice->num_ref_idx_active[LIST_1];
  }
    ...
    if ((p_Vid->profile_idc == MVC_HIGH || p_Vid->profile_idc == STEREO_HIGH) &&
      ...
      if (currSlice->listXsize[0] > 0) {
        ...
        printf(" ** (CurViewID:%d) %s Ref Pic List 0 ****\n",
               ...
               currSlice->structure == FRAME
                   ? "FRM"
                   : (currSlice->structure == TOP_FIELD ? "TOP" : "BOT"));
        for (i = 0; i < (unsigned int)(currSlice->listXsize[0]);
             i++) // ref list 0
        {
      ...
      if (currSlice->listXsize[1] > 0) {
        printf(" ** (CurViewID:%d) %s Ref Pic List 1 ****\n",
               ...
               currSlice->structure == FRAME
                   ? "FRM"
                   : (currSlice->structure == TOP_FIELD ? "TOP" : "BOT"));
        for (i = 0; i < (unsigned int)(currSlice->listXsize[1]);
             i++) // ref list 1
        {
          printf("   %2d -> POC: %4d PicNum: %4d ViewID: %d\n", i,
...
int read_new_slice(Slice *currSlice) {
  ...
  for (;;) {
    ...
    if (p_Inp->DecodeAllLayers == 1 &&
      ...
      if (currSlice->svc_extension_flag) {
        ...
        currSlice->NaluHeaderMVCExt.iPrefixNALU =
            (nalu->nal_unit_type == NALU_TYPE_PREFIX);
      }
      ...
      if (nalu->nal_unit_type == NALU_TYPE_SLC_EXT) {
        if (currSlice->svc_extension_flag) {
          ...
          nalu->nal_unit_type = currSlice->NaluHeaderMVCExt.non_idr_flag == 0
                                    ? NALU_TYPE_IDR
                                    : NALU_TYPE_SLICE;
        }
      }
  ...
  process_nalu:
    ...
    case NALU_TYPE_IDR:
      ...
      // Some syntax of the Slice Header depends on the parameter set, which
      // depends on the parameter set ID of the SLice header.  Hence, read the
      // pic_parameter_set_id of the slice header first, then setup the active
      // parameter sets, and then read the rest of the slice header
      ...
      currSlice->is_not_independent =
          (p_Vid->active_sps->chroma_format_idc == YUV444) &&
          ((p_Vid->separate_colour_plane_flag == 0));

      BitsUsedByHeader += RestOfSliceHeader(currSlice);

#if (MVC_EXTENSION_ENABLE)
      if (currSlice->svc_extension_flag == 0) { // MVC
        currSlice->view_id = currSlice->NaluHeaderMVCExt.view_id;
        currSlice->inter_view_flag =
            currSlice->NaluHeaderMVCExt.inter_view_flag;
        currSlice->anchor_pic_flag =
            currSlice->NaluHeaderMVCExt.anchor_pic_flag;
      ...
      // if primary slice is replaced with redundant slice, set the correct
      // image type
      if (currSlice->redundant_pic_cnt && p_Vid->Is_primary_correct == 0 &&
          p_Vid->Is_redundant_correct) {
      ...
      // From here on, p_Vid->active_sps, p_Vid->active_pps and the slice header
      // are valid
      if (currSlice->mb_aff_frame_flag)
      ...
      if (p_Vid->active_pps->entropy_coding_mode_flag) {
        ...
        arideco_start_decoding(&currSlice->partArr[0].de_cabac,
    ...
    case NALU_TYPE_DPA:
      ...
      if (NALU_TYPE_DPB == nalu->nal_unit_type) {
        ...
        if ((slice_id_b != slice_id_a) || (nalu->lost_packets)) {
          printf("Waning: got a data partition B which does not match DP_A (DP "
                 "loss!)\n");
          currSlice->dpB_NotPresent = 1;
          currSlice->dpC_NotPresent = 1;
        } else {
      ...
      if (NALU_TYPE_DPC == nalu->nal_unit_type) {
        ...
        if ((slice_id_c != slice_id_a) || (nalu->lost_packets)) {
          printf("Warning: got a data partition C which does not match DP_A(DP "
                 "loss!)\n");
          // currSlice->dpB_NotPresent =1;
          currSlice->dpC_NotPresent = 1;
        }
      ...
      if ((nalu->nal_unit_type != NALU_TYPE_DPB) &&
          (nalu->nal_unit_type != NALU_TYPE_DPC)) {
        // we have a NALI that we can't process here, so restart processing
        goto process_nalu;
        // yes, "goto" should not be used, but it's really the best way here
        // before we restructure the decoding loop (which should be taken care
        // of anyway)
      }
    ...
    case NALU_TYPE_AUD:
      ...
      //         printf ("read_new_slice: Found 'Access Unit Delimiter' NAL
      //         unit, len %d, ignored\n", nalu->len);
      break;
    case NALU_TYPE_EOSEQ:
      //        printf ("read_new_slice: Found 'End of Sequence' NAL unit, len
      //        %d, ignored\n", nalu->len);
      break;
    case NALU_TYPE_EOSTREAM:
      //        printf ("read_new_slice: Found 'End of Stream' NAL unit, len %d,
      //        ignored\n", nalu->len);
      break;
    case NALU_TYPE_FILL:
      if (p_Inp->silent == FALSE) {
    ...
    case NALU_TYPE_VDRD:
      ...
      //         printf ("read_new_slice: Found 'View and Dependency
      //         Representation Delimiter' NAL unit, len %d, ignored\n",
      //         nalu->len);
      break;
...
void pad_buf(imgpel *pImgBuf, int iWidth, int iHeight, int iStride, int iPadX,
             int iPadY) {
  int j;
  ...
  fast_memset(pImgBuf + iWidth, *(pImgBuf + iWidth - 1),
              iPadX * sizeof(imgpel));
  ...
  for (j = 1; j < iHeight; j++) {
    ...
    fast_memset(pLine + pad_width, *(pLine + pad_width - 1),
                iPadX * sizeof(imgpel));
  }
...
void exit_picture(VideoParameters *p_Vid, StorablePicture **dec_picture) {
  ...
  if (!(*dec_picture)->mb_aff_frame_flag) {
    ...
    for (i = 1; i < (*dec_picture)->PicSizeInMbs; ++i) {
      if (p_Vid->mb_data[i].ei_flag != p_Vid->mb_data[i - 1].ei_flag) {
        ercStopSegment(i - 1, ercSegment, 0,
                       p_Vid->erc_errorVar); //! stop current segment
        ...
        ercStartSegment(i, ercSegment, 0,
                        p_Vid->erc_errorVar); //! start new segment
        ercStartMB = i;                       //! save start MB for this segment
      }
    }
    //! mark end of the last segment
    ercStopSegment((*dec_picture)->PicSizeInMbs - 1, ercSegment, 0,
                   p_Vid->erc_errorVar);
    if (p_Vid->mb_data[i - 1].ei_flag)
      ercMarkCurrSegmentLost((*dec_picture)->size_x, p_Vid->erc_errorVar);
    else
    ...
    if ((*dec_picture)->slice_type == I_SLICE ||
        (*dec_picture)->slice_type == SI_SLICE) // I-frame
      ercConcealIntraFrame(p_Vid, &recfr, (*dec_picture)->size_x,
                           (*dec_picture)->size_y, p_Vid->erc_errorVar);
    else
      ercConcealInterFrame(&recfr, p_Vid->erc_object_list,
                           (*dec_picture)->size_x, (*dec_picture)->size_y,
                           p_Vid->erc_errorVar,
  ...
  *dec_picture = NULL;
    ...
    if (p_Inp->silent == FALSE) {
      ...
      if (p_Vid->p_ref != -1)
        fprintf(stdout, "%05d(%s%5d %5d %5d %8.4f %8.4f %8.4f  %s %7d\n",
                p_Vid->frame_no, p_Vid->cslice_type, frame_poc, pic_num, qp,
                snr->snr[0], snr->snr[1], snr->snr[2], yuvFormat,
                (int)tmp_time);
      else
        fprintf(stdout,
                "%05d(%s%5d %5d %5d                             %s %7d\n",
                p_Vid->frame_no, p_Vid->cslice_type, frame_poc, pic_num, qp,
                yuvFormat, (int)tmp_time);
    } else
  ...
  // p_Vid->currentSlice->current_mb_nr = -4712;   // impossible value for
  // debugging, StW p_Vid->currentSlice->current_slice_nr = 0;
}
...
void ercWriteMBMODEandMV(Macroblock *currMB) {
  ...
  int i, ii, jj,
      currMBNum = currMB->mbAddrX; // p_Vid->currentSlice->current_mb_nr;
  StorablePicture *dec_picture = p_Vid->dec_picture;
  int mbx = xPosMB(currMBNum, dec_picture->size_x),
      mby = yPosMB(currMBNum, dec_picture->size_x);
  ...
  if (p_Vid->type != B_SLICE) // non-B frame
  {
    for (i = 0; i < 4; ++i) {
      ...
      if (currMB->b8mode[i] == 0 ||
          currMB->b8mode[i] == IBLOCK) // INTRA OR COPY
      {
        pRegion->mv[0] = 0;
        pRegion->mv[1] = 0;
        pRegion->mv[2] = 0;
        ...
        {
          pRegion->mv[0] =
              (dec_picture->mv_info[jj][ii].mv[LIST_0].mv_x +
               dec_picture->mv_info[jj][ii + 1].mv[LIST_0].mv_x +
               dec_picture->mv_info[jj + 1][ii].mv[LIST_0].mv_x +
               dec_picture->mv_info[jj + 1][ii + 1].mv[LIST_0].mv_x + 2) /
              4;
          pRegion->mv[1] =
              (dec_picture->mv_info[jj][ii].mv[LIST_0].mv_y +
               dec_picture->mv_info[jj][ii + 1].mv[LIST_0].mv_y +
               dec_picture->mv_info[jj + 1][ii].mv[LIST_0].mv_y +
               dec_picture->mv_info[jj + 1][ii + 1].mv[LIST_0].mv_y + 2) /
              4;
        } else // 16x16, 16x8, 8x16, 8x8
        {
          ...
          //          pRegion->mv[0]  =
          //          dec_picture->motion.mv[LIST_0][4*mby+(i/2)*2][4*mbx+(i%2)*2+BLOCK_SIZE][0];
          //          pRegion->mv[1]  =
          //          dec_picture->motion.mv[LIST_0][4*mby+(i/2)*2][4*mbx+(i%2)*2+BLOCK_SIZE][1];
        }
        currMB->p_Slice->erc_mvperMB +=
            iabs(pRegion->mv[0]) + iabs(pRegion->mv[1]);
  ...
  } else // B-frame
  {
    for (i = 0; i < 4; ++i) {
      ...
      pRegion = currRegion + i;
      pRegion->regionMode =
          (currMB->mb_type == I16MB      ? REGMODE_INTRA
           : currMB->b8mode[i] == IBLOCK ? REGMODE_INTRA_8x8
                                         : REGMODE_INTER_PRED_8x8);
      if (currMB->mb_type == I16MB || currMB->b8mode[i] == IBLOCK) // INTRA
      {
        ...
        //        int idx = (currMB->b8mode[i]==0 && currMB->b8pdir[i]==2 ?
        //        LIST_0 : currMB->b8pdir[i]==1 ? LIST_1 : LIST_0); int idx =
        //        currMB->b8pdir[i]==0 ? LIST_0 : LIST_1;
        pRegion->mv[0] =
            (dec_picture->mv_info[jj][ii].mv[idx].mv_x +
             dec_picture->mv_info[jj][ii + 1].mv[idx].mv_x +
             dec_picture->mv_info[jj + 1][ii].mv[idx].mv_x +
             dec_picture->mv_info[jj + 1][ii + 1].mv[idx].mv_x + 2) /
            4;
        pRegion->mv[1] =
            (dec_picture->mv_info[jj][ii].mv[idx].mv_y +
             dec_picture->mv_info[jj][ii + 1].mv[idx].mv_y +
             dec_picture->mv_info[jj + 1][ii].mv[idx].mv_y +
             dec_picture->mv_info[jj + 1][ii + 1].mv[idx].mv_y + 2) /
            4;
        currMB->p_Slice->erc_mvperMB +=
            iabs(pRegion->mv[0]) + iabs(pRegion->mv[1]);
        ...
        /*
        if (currMB->b8pdir[i]==0 || (currMB->b8pdir[i]==2 &&
        currMB->b8mode[i]!=0)) // forward or bidirect
        {
...
void CopySliceInfo(Slice *currSlice, OldSliceParams *p_old_slice) {
  ...
  p_old_slice->field_pic_flag =
      currSlice->field_pic_flag; // p_Vid->field_pic_flag;
 ...
 ************************************************************************
 */
int is_new_picture(StorablePicture *dec_picture, Slice *currSlice,
  ...
  if (p_Vid->active_sps->pic_order_cnt_type == 0) {
    ...
    if (p_Vid->active_pps->bottom_field_pic_order_in_frame_present_flag == 1 &&
  ...
  if (p_Vid->active_sps->pic_order_cnt_type == 1) {
    if (!p_Vid->active_sps->delta_pic_order_always_zero_flag) {
      ...
      if (p_Vid->active_pps->bottom_field_pic_order_in_frame_present_flag ==
              1 &&
          !currSlice->field_pic_flag) {
        result |= (p_old_slice->delta_pic_order_cnt[1] !=
 ...
 ************************************************************************
 */
void field_postprocessing(VideoParameters *p_Vid) { p_Vid->number /= 2; }
 ...
 ************************************************************************
 */
void copy_dec_picture_JV(VideoParameters *p_Vid, StorablePicture *dst,
                         StorablePicture *src) {
  ...
  dst->frame_cropping_rect_bottom_offset =
      src->frame_cropping_rect_bottom_offset;
  ...
  if (src->tone_mapping_lut) {
    ...
    memcpy(dst->tone_mapping_lut, src->tone_mapping_lut,
           sizeof(imgpel) * coded_data_bit_max);
  }
#endif
}

// this is intended to make get_block_luma faster by doing this at a more
// appropriate level i.e. per slice rather than per MB
static void init_cur_imgy(Slice *currSlice, VideoParameters *p_Vid) {
  ...
  if ((p_Vid->separate_colour_plane_flag != 0)) {
    ...
    StorablePicture *vidref = p_Vid->no_reference_picture;
    int noref = (currSlice->framepoc < p_Vid->recovery_poc);
    int total_lists = currSlice->mb_aff_frame_flag
                          ? 6
                          : (currSlice->slice_type == B_SLICE ? 2 : 1);
    //    for (j = 0; j < 6; j++) {  //for (j = 0; j <
    //    (currSlice->slice_type==B_SLICE?2:1); j++) {
    for (j = 0; j < total_lists; j++) {
      // note that if we always set this to MAX_LIST_SIZE, we avoid crashes with
      // invalid ref_idx being set since currently this is done at the slice
      // level, it seems safe to do so. Note for some reason I get now a
      // mismatch between version 12 and this one in cabac. I wonder why.
...
void decode_one_slice(Slice *currSlice) {
  ...
  while (end_of_slice == FALSE) // loop over macroblocks
  {
    ...
    fprintf(p_Dec->p_trace,
            ...
            currSlice->ThisPOC, currSlice->current_mb_nr,
            currSlice->current_slice_nr, currSlice->slice_type);
#endif
    ...
    end_of_slice = exit_macroblock(currSlice, (!currSlice->mb_aff_frame_flag ||
                                               currSlice->current_mb_nr % 2));
  }
...
int GetVOIdx(VideoParameters *p_Vid, int iViewId) {
  ...
  if (p_Vid->active_subset_sps) {
    ...
    for (iVOIdx = p_Vid->active_subset_sps->num_views_minus1; iVOIdx >= 0;
         iVOIdx--)
...
int get_maxViewIdx(VideoParameters *p_Vid, int view_id, int anchor_pic_flag,
                   int listidx) {
  int VOIdx;
  int maxViewIdx = 0;
  ...
  if (VOIdx >= 0) {
    if (anchor_pic_flag)
      maxViewIdx = listidx
                       ...
                       : p_Vid->active_subset_sps->num_anchor_refs_l0[VOIdx];
    else
      maxViewIdx =
          listidx ? p_Vid->active_subset_sps->num_non_anchor_refs_l1[VOIdx]
                  : p_Vid->active_subset_sps->num_non_anchor_refs_l0[VOIdx];
  }
