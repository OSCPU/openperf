...
typedef enum TiffType {
  ...
  T_ASCII = 2, //!< 8-bit byte that contains a 7-bit ASCII code; the last byte
               //!< must be NUL (binary zero).
  T_SHORT = 3, //!< 16-bit (2-byte) unsigned integer.
  T_LONG = 4,  //!< 32-bit (4-byte) unsigned integer.
  T_RATIONAL = 5,   //!< Two LONGs:  the first represents the numerator of a
                    //!< fraction; the second, the denominator.
  T_SBYTE = 6,      //!< An 8-bit signed (twos-complement) integer.
  T_UNDEFINED = 7,  //!< An 8-bit byte that may contain anything, depending on
                    //!< the definition of the field.
  T_SSHORT = 8,     //!< A 16-bit (2-byte) signed (twos-complement) integer.
  T_SLONG = 9,      //!< A 32-bit (4-byte) signed (twos-complement) integer.
  T_SRATIONAL = 10, //!< Two SLONG’s:  the first represents the numerator of a
                    //!< fraction, the second the denominator.
...
typedef struct Tiff {
  ...
  uint16 Orientation;
  uint32 BitsPerSample[3], RowsPerStrip, ImageLength, ImageWidth,
      StripByteCounts[YRES], StripOffsets[YRES], XResolution[2], YResolution[2];
...
static const double Coef[VC_MAX + 1][3] = {
    ...
    {0.212639, 0.715169,
     0.072192} //  8  SMPTE RP-177 & 274M, ITU-R Rec. 709 exact
};
...
typedef struct RGB_YUV {
  uint16 pixMax; //!< Maximum pixel value.
  int stdRange,  //!< 0 = full range, 1 = std range
      y, u, v, r, g, b, yr, yg, yb, ur, ug, ub, vr, vg, vb, offy, offuv;
  double sy, suv;
} RGB_YUV;
 ...
 ************************************************************************
 * \brief
 *   Initialize pointer variables, since they are maintained across multiple
 *calls to ReadTIFFImage
 *
 ************************************************************************
 */
 ...
 ************************************************************************
 */
static void getIntArray(Tiff *t, uint32 offset, TiffType type, uint32 a[],
                        int n) {
  int i;
...
static int readDirectoryEntry(Tiff *t) {
  ...
  case 257: // ImageLength    SHORT or LONG
    ...
    if (offset > YRES) {
      fprintf(stderr,
              "readDirectoryEntry:  ImageLength (%d) exceeds builtin maximum "
              "of %d\n",
              offset, YRES);
      return 1;
    }
  ...
  case 258: // BitsPerSample  SHORT 8,8,8
    ...
    // t->BitsPerSample[0], t->BitsPerSample[1], t->BitsPerSample[2]);
    if (t->BitsPerSample[0] != t->BitsPerSample[1] ||
        t->BitsPerSample[0] != t->BitsPerSample[2]) {
...
static int readFileIntoMemory(Tiff *t, const char *path) {
  ...
  uint16 byteOrder;
  int endian = 1,
      machineLittleEndian = (*((signed char *)(&endian)) == 1) ? 1 : 0, fd;
  ...
  cnt = (long)lseek(fd, 0,
                    SEEK_END); // TIFF files by definition cannot exceed 2^32
  if (cnt == -1L)
    return 1;
...
RGB_YUV_initialize(RGB_YUV *T,
  ...
  if (stdRange) {
    T->offy = (int)(INTEGER_SCALE *
                    (pixScale * 16 / 256.0 + 0.5)); // setup + rounding
  ...
  T->yr = (int)(T->sy * CR + 0.5);
  T->yg = (int)(T->sy * CG + 0.5);
  T->yb = (int)(T->sy * CB + 0.5);
  T->ur = (int)(T->suv * CR * CU - 0.5);
  T->ug = (int)(T->suv * CG * CU - 0.5);
  T->ub = (int)(T->suv * 0.5 + 0.5);
  T->vr = (int)(T->suv * 0.5 + 0.5);
  T->vg = (int)(T->suv * CG * CV - 0.5);
  T->vb = (int)(T->suv * CB * CV - 0.5);
 ...
 *****************************************************************************
 * \brief
 *    Convert interleaved/planar RGB components to interleaved/planar YUV
 *components
 *
 *****************************************************************************
 */
static void
RGB_YUV_rgb_to_yuv(RGB_YUV *T,
  ...
  if (T->stdRange) // clipping not needed
  {
    for (i = 0; i < count; ++i) {
      ...
      bp += rgb_stride;
      // convert to YUV
      *yp = (uint16)((T->yr * T->r + T->yg * T->g + T->yb * T->b + T->offy) >>
                     INTEGER_SHIFT);
      yp += yuv_stride;
      *up = (uint16)((T->ur * T->r + T->ug * T->g + T->ub * T->b + T->offuv) >>
                     INTEGER_SHIFT);
      up += yuv_stride;
      *vp = (uint16)((T->vr * T->r + T->vg * T->g + T->vb * T->b + T->offuv) >>
                     INTEGER_SHIFT);
      vp += yuv_stride;
    }
  } else // full range -- need to clip
  {
    for (i = 0; i < count; ++i) {
      ...
      T->y = (T->yr * T->r + T->yg * T->g + T->yb * T->b + T->offy) >>
             INTEGER_SHIFT;
      if (T->y < 0)
        T->y = 0;
      ...
      *yp = (uint16)T->y;
      ...
      T->u = (T->ur * T->r + T->ug * T->g + T->ub * T->b + T->offuv) >>
             INTEGER_SHIFT;
      if (T->u < 0)
        T->u = 0;
      ...
      *up = (uint16)T->u;
      ...
      T->v = (T->vr * T->r + T->vg * T->g + T->vb * T->b + T->offuv) >>
             INTEGER_SHIFT;
      if (T->v < 0)
        T->v = 0;
 ...
 *    number of taps in decimal   17   (normalized & rounded in
 ...
 *****************************************************************************
 */
void horizontal_half_1chan_cosite(uint16 *srcPtr, int srcXres, int yres,
                                  int srcZres, uint16 *dstPtr, int dstZres,
  ...
  for (y = 0; y < yres; y++) {
    for (x = 0; x < 8; x += 2) {
      ...
      n7 = (x >= 7) ? 7 : x;
      result =
          (2048 * *(src) + 1228 * (*(src - n1 * srcZres) + *(src + srcZres)) -
           262 * (*(src - n3 * srcZres) + *(src + 3 * srcZres)) +
           47 * (*(src - n5 * srcZres) + *(src + 5 * srcZres)) +
           11 * (*(src - n7 * srcZres) + *(src + 7 * srcZres)) + 2048) /
          4096;
      if (result < 0)
        result = 0;
    ...
    limit = srcXres - 8;
    for (x = 8; x < limit; x += 2) {
      result = (2048 * *(src) + 1228 * (*(src - srcZres) + *(src + srcZres)) -
                262 * (*(src - 3 * srcZres) + *(src + 3 * srcZres)) +
                47 * (*(src - 5 * srcZres) + *(src + 5 * srcZres)) +
                11 * (*(src - 7 * srcZres) + *(src + 7 * srcZres)) + 2048) /
               4096;
      if (result < 0)
        result = 0;
      else if (result > pixMax)
    ...
    for (x = x; x < limit; x += 2) {
      ...
      result =
          (2048 * *(src) + 1228 * (*(src - srcZres) + *(src + n1 * srcZres)) -
           262 * (*(src - 3 * srcZres) + *(src + n3 * srcZres)) +
           47 * (*(src - 5 * srcZres) + *(src + n5 * srcZres)) +
           11 * (*(src - 7 * srcZres) + *(src + n7 * srcZres)) + 2048) /
          4096;
      if (result < 0)
        result = 0;
      else if (result > pixMax)
 ...
 *****************************************************************************
 * \brief
 *    FIR vertical filter and 2:1 subsampling with 0.5 sample interval phase
 *shift see vertical_half_1chan.pdf for response
 *****************************************************************************
 */
void vertical_half_1chan(uint16 *srcPtr, int xres, int srcYres, int srcZres,
                         uint16 *dstPtr, int dstZres,
                         int pixMax //!< needed for clipping
) {
    ...
    for (x = 0; x < xres; x++) {
      result =
          (225 * (*(src) + *(src + srcRowCount)) +
           ...
           2 * (*(src - y * srcRowCount) + *(src + 6 * srcRowCount)) + 256) /
          512;
      if (result < 0)
        result = 0;
      else if (result > pixMax)
    ...
    for (x = 0; x < xres; x++) {
      result =
          (225 * (*(src) + *(src + srcRowCount)) +
           ...
           2 * (*(src - 5 * srcRowCount) + *(src + 6 * srcRowCount)) + 256) /
          512;
      if (result < 0)
        result = 0;
      else if (result > pixMax)
    ...
    for (x = 0; x < xres; x++) {
      result =
          (225 * (*(src) + *(src + n1 * srcRowCount)) +
           ...
           2 * (*(src - 5 * srcRowCount) + *(src + n6 * srcRowCount)) + 256) /
          512;
      if (result < 0)
        result = 0;
      else if (result > pixMax)
 ...
 *****************************************************************************
 */
int ReadTIFFImage(InputParameters *p_Inp, VideoDataFile *input_file,
                  int FrameNoInFile, FrameFormat *source, unsigned char *buf) {
  static Tiff
      t; // Declaring it static allows it to "remember" memory allocations;
         // ATOUR: Not a good idea since this affects reentrancy
  char path[FILE_NAME_SIZE];
  int frameNumberInFile, n, nComponents, height, i, width, y;
  RGB_YUV rgb_yuv;
  uint16 *img, *temp = NULL, *temp2 = NULL, *p, *yp, *up, *vp;
  ...
  if (source->color_model == CM_RGB && !input_file->is_interleaved) {
    fprintf(stderr, "ReadTIFFImage:  RGB input file has not been declared as "
                    "interleaved but only interleaved is supported\n");
    goto Error;
  }
  ...
  if (input_file->num_digits > 0) {
    if (input_file->zero_pad) {
      n = snprintf(path, sizeof(path), "%s%0*d%s", input_file->fhead,
                   input_file->num_digits, frameNumberInFile,
                   input_file->ftail);
    } else {
      n = snprintf(path, sizeof(path), "%s%*d%s", input_file->fhead,
                   input_file->num_digits, frameNumberInFile,
                   input_file->ftail);
    }
  ...
  if ((int)t.ImageLength != height) {
    fprintf(stderr,
            "ReadTIFFImage:  Tiff height (%u) different from encoder input "
            "height (%d) . Exiting...\n",
  ...
  if ((int)t.ImageWidth != width) {
    fprintf(stderr,
            "ReadTIFFImage:  Tiff width (%u) different from encoder input "
            "width (%d) . Exiting...\n",
            t.ImageWidth, width);
    goto Error;
  }

  if (source->pic_unit_size_on_disk != 8 &&
      source->pic_unit_size_on_disk != 16) {
    fprintf(
        stderr,
        "ReadTIFFImage only supports pic_unit_size_on_disk of 8 or 16 not %d\n",
  ...
  nComponents =
      width * height * 3; // for RGB, which will be overridden by YUV420 etc.

  if (source->color_model == CM_YUV) {
    if (RGB_YUV_initialize(&rgb_yuv, (VideoCode)p_Inp->videoCode,
                           p_Inp->stdRange, 65535))
      goto Error;
    RGB_YUV_rgb_to_yuv(&rgb_yuv, img, img + 1, img + 2, width, height, 3, img,
                       img + 1, img + 2, 3);
    ...
    case YUV422:
      ...
      // U & V
      for (y = 0; y < height; ++y) {
        horizontal_half_1chan_cosite(img + 1 + y * width * 3, width, 1, 3,
                                     up + y * width / 2, 1, 65535);
        horizontal_half_1chan_cosite(img + 2 + y * width * 3, width, 1, 3,
                                     vp + y * width / 2, 1, 65535);
      }
      img = yp; // img points at result
      break;
    case YUV444:
  ...
  default:
    fprintf(
        stderr,
        "ReadTIFFImage only supports pic_unit_size_shift3 of 1 or 2 not %d\n",
