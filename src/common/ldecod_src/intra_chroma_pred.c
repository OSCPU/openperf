...
static void intra_chroma_DC_single_mbaff(imgpel **curr_img, int up_avail,
                                         int left_avail, PixelPos up,
                                         PixelPos left[17], int blk_x,
                                         int blk_y, int *pred, int direction) {
  int i;
  int s0 = 0;
...
static void intra_chroma_DC_all_mbaff(imgpel **curr_img, int up_avail,
                                      int left_avail, PixelPos up,
                                      PixelPos left[17], int blk_x, int blk_y,
                                      int *pred) {
  int i;
  int s0 = 0, s1 = 0;
...
static void intra_chroma_DC_single(imgpel **curr_img, int up_avail,
                                   int left_avail, PixelPos up, PixelPos left,
                                   int blk_x, int blk_y, int *pred,
                                   int direction) {
  int i;
  int s0 = 0;
...
static void intra_chroma_DC_all(imgpel **curr_img, int up_avail, int left_avail,
                                PixelPos up, PixelPos left, int blk_x,
                                int blk_y, int *pred) {
  int i;
  int s0 = 0, s1 = 0;
...
static void intrapred_chroma_dc(Macroblock *currMB) {
  ...
  if (!p_Vid->active_pps->constrained_intra_pred_flag) {
    ...
    left_up_avail =
        up_left.available ? currSlice->intra_block[up_left.mb_addr] : 0;
  }

  // DC prediction
  // Note that unlike what is stated in many presentations and papers, this mode
  // does not operate the same way as I_16x16 DC prediction.
  for (b8 = 0; b8 < (p_Vid->num_uv_blocks); ++b8) {
    for (b4 = 0; b4 < 4; ++b4) {
      ...
      case 0: //===== TOP LEFT =====
        intra_chroma_DC_all(imgUV0, up_avail, left_avail, up, left, blk_x,
                            blk_y + 1, &pred);
        intra_chroma_DC_all(imgUV1, up_avail, left_avail, up, left, blk_x,
                            blk_y + 1, &pred1);
        break;
      case 1: //===== TOP RIGHT =====
        intra_chroma_DC_single(imgUV0, up_avail, left_avail, up, left, blk_x,
                               blk_y + 1, &pred, 1);
        intra_chroma_DC_single(imgUV1, up_avail, left_avail, up, left, blk_x,
                               blk_y + 1, &pred1, 1);
        break;
      case 2: //===== BOTTOM LEFT =====
        intra_chroma_DC_single(imgUV0, up_avail, left_avail, up, left, blk_x,
...
static void intrapred_chroma_plane(Macroblock *currMB) {
  ...
  if (!left_up_avail || !left_avail || !up_avail)
    ...
    for (uv = 0; uv < 2; uv++) {
      ...
      iv = cr_MB_y2 * (imgUV[left.pos_y + cr_MB_y - 1][pos_x] -
                       imgUV[up_left.pos_y][pos_x]);

      for (i = 0; i < cr_MB_y2 - 1; ++i) {
        iv += (i + 1) * (*(imgUV[pos_y1++] + pos_x) - *((*predU2--) + pos_x));
      }

      ib = ((cr_MB_x == 8 ? 17 : 5) * ih + 2 * cr_MB_x) >>
           (cr_MB_x == 8 ? 5 : 6);
      ic = ((cr_MB_y == 8 ? 17 : 5) * iv + 2 * cr_MB_y) >>
           (cr_MB_y == 8 ? 5 : 6);
      ...
      for (j = 0; j < cr_MB_y; ++j) {
        ...
        for (i = 0; i < cr_MB_x; ++i)
          mb_pred[j][i] =
              (imgpel)iClip1(max_imgpel_value, ((i * ib + plane) >> 5));
      }
    }
  }
}
 ...
 ************************************************************************
 */
void intrapred_chroma_mbaff(Macroblock *currMB) {
  Slice *currSlice = currMB->p_Slice;
  VideoParameters *p_Vid = currMB->p_Vid;
  int i, j, ii, jj;
  StorablePicture *dec_picture = currSlice->dec_picture;
  ...
  case DC_PRED_8: {
    ...
    if (!p_Vid->active_pps->constrained_intra_pred_flag) {
      ...
      for (i = 0, left_avail[0] = 1; i < cr_MB_y2; ++i)
        left_avail[0] &= left[i + 1].available
                             ? currSlice->intra_block[left[i + 1].mb_addr]
                             : 0;

      for (i = cr_MB_y2, left_avail[1] = 1; i < cr_MB_y; ++i)
        left_avail[1] &= left[i + 1].available
                             ? currSlice->intra_block[left[i + 1].mb_addr]
                             : 0;
    ...
    // DC prediction
    // Note that unlike what is stated in many presentations and papers, this
    // mode does not operate the same way as I_16x16 DC prediction.
    {
      int pred1;
      imgpel **imgUV0 = dec_picture->imgUV[0];
      imgpel **imgUV1 = dec_picture->imgUV[1];
      ...
      for (b8 = 0; b8 < (p_Vid->num_uv_blocks); ++b8) {
        for (b4 = 0; b4 < 4; ++b4) {
          ...
          switch (block_pos[yuv][b8][b4]) {
          case 0: //===== TOP LEFT =====
            intra_chroma_DC_all_mbaff(imgUV0, up_avail, left_avail[0], up, left,
                                      blk_x, blk_y + 1, &pred);
            intra_chroma_DC_all_mbaff(imgUV1, up_avail, left_avail[0], up, left,
                                      blk_x, blk_y + 1, &pred1);
            break;
          case 1: //===== TOP RIGHT =====
            intra_chroma_DC_single_mbaff(imgUV0, up_avail, left_avail[0], up,
                                         left, blk_x, blk_y + 1, &pred, 1);
            intra_chroma_DC_single_mbaff(imgUV1, up_avail, left_avail[0], up,
                                         left, blk_x, blk_y + 1, &pred1, 1);
            break;
          case 2: //===== BOTTOM LEFT =====
            intra_chroma_DC_single_mbaff(imgUV0, up_avail, left_avail[1], up,
                                         left, blk_x, blk_y + 1, &pred, 0);
            intra_chroma_DC_single_mbaff(imgUV1, up_avail, left_avail[1], up,
                                         left, blk_x, blk_y + 1, &pred1, 0);
            break;
          case 3: //===== BOTTOM RIGHT =====
            intra_chroma_DC_all_mbaff(imgUV0, up_avail, left_avail[1], up, left,
                                      blk_x, blk_y + 1, &pred);
            intra_chroma_DC_all_mbaff(imgUV1, up_avail, left_avail[1], up, left,
                                      blk_x, blk_y + 1, &pred1);
            break;
          }
  ...
  case PLANE_8: {
    ...
    for (i = 0; i < cr_MB_y + 1; ++i)
      p_Vid->getNeighbour(currMB, -1, i - 1, p_Vid->mb_size[IS_CHROMA],
                          &left[i]);
    ...
    if (!left_up_avail || !left_avail[0] || !left_avail[1] || !up_avail)
      ...
      for (uv = 0; uv < 2; uv++) {
        ...
        ih = cr_MB_x2 *
             (upPred[cr_MB_x - 1] - imgUV[left[0].pos_y][left[0].pos_x]);
        for (i = 0; i < cr_MB_x2 - 1; ++i)
          ih += (i + 1) * (upPred[cr_MB_x2 + i] - upPred[cr_MB_x2 - 2 - i]);

        iv = cr_MB_y2 * (imgUV[left[cr_MB_y].pos_y][left[cr_MB_y].pos_x] -
                         imgUV[left[0].pos_y][left[0].pos_x]);
        for (i = 0; i < cr_MB_y2 - 1; ++i)
          iv += (i + 1) * (imgUV[left[cr_MB_y2 + 1 + i].pos_y]
                                [left[cr_MB_y2 + 1 + i].pos_x] -
                           imgUV[left[cr_MB_y2 - 1 - i].pos_y]
                                [left[cr_MB_y2 - 1 - i].pos_x]);
        ...
        iaa = 16 * (imgUV[left[cr_MB_y].pos_y][left[cr_MB_y].pos_x] +
                    upPred[cr_MB_x - 1]);
