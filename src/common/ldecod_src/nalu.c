...
int read_next_nalu(VideoParameters *p_Vid, NALU_t *nalu) {
  ...
  if (ret < 0) {
    snprintf(errortext, ET_SIZE,
             "Error while getting the NALU in file format %s, exit\n",
  ...
  // we can't tell whether it is the first VCL NALU at this point, so only
  // non-VCL NAL unit is checked here.
...
void CheckZeroByteNonVCL(VideoParameters *p_Vid, NALU_t *nalu) {
  ...
  if (nalu->nal_unit_type == NALU_TYPE_SPS ||
    ...
    CheckZeroByte = 1;
  // check the possibility of the current NALU to be the start of a new access
  // unit, according to 7.4.1.2.3
  if (nalu->nal_unit_type == NALU_TYPE_AUD ||
    ...
    if (p_Vid->LastAccessUnitExists) {
      p_Vid->LastAccessUnitExists = 0; // deliver the last access unit to
                                       // decoder
      p_Vid->NALUCount = 0;
    }
  }
...
void CheckZeroByteVCL(VideoParameters *p_Vid, NALU_t *nalu) {
  ...
  p_Vid->NALUCount++;
  // the first VCL NAL unit that is the first NAL unit after last VCL NAL unit
  // indicates the start of a new access unit and hence the first NAL unit of
  // the new access unit.           (sounds like a tongue twister :-)
  if (p_Vid->NALUCount == 1)
    CheckZeroByte = 1;
...
#if (MVC_EXTENSION_ENABLE)
void nal_unit_header_mvc_extension(NALUnitHeaderMVCExt_t *NaluHeaderMVCExt,
                                   Bitstream *s) {
  // to be implemented;
