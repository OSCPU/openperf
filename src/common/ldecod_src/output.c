...
static void write_out_picture(VideoParameters *p_Vid, StorablePicture *p, int p_out)
{
  ...
  {
    if((p_Vid->p_out_mvc[iViewIdx] == -1) && (STRCMP(p_Inp->outfile, "\"\"")!=0) && (strlen(p_Inp->outfile)>0))
    {
  ...
  { //Normal AVC
    if((p_Vid->p_out_mvc[0] == -1) && (STRCMP(p_Inp->outfile, "\"\"")!=0) && (strlen(p_Inp->outfile)>0))
    {
      if( (STRCMP(p_Inp->outfile, "\"\"")!=0) && ((p_Vid->p_out_mvc[0]=open(p_Inp->outfile, OPENFLAGS_WRITE, OPEN_PERMISSIONS))==-1) )
      {
