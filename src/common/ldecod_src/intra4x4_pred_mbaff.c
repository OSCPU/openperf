 ...
 ***********************************************************************
 */
static int intra4x4_dc_pred_mbaff(Macroblock *currMB, ColorPlane pl, int ioff,
                                  int joff) {
  ...
  for (i = 0; i < 4; ++i) {
    p_Vid->getNeighbour(currMB, ioff - 1, joff + i, p_Vid->mb_size[IS_LUMA],
                        &pix_a[i]);
  }
                                    ...
                                    int joff) //!< pixel offset Y within MB
{
  ...
  if (!block_available_up) {
    ...
    imgpel **mb_pred = currSlice->mb_pred[pl];
    imgpel *imgY =
        (pl) ? &currSlice->dec_picture->imgUV[pl - 1][pix_b.pos_y][pix_b.pos_x]
 ...
 ***********************************************************************
 */
static int intra4x4_hor_pred_mbaff(Macroblock *currMB, ColorPlane pl, int ioff,
                                   int joff) {
  VideoParameters *p_Vid = currMB->p_Vid;
  Slice *currSlice = currMB->p_Slice;
  ...
  if (!block_available_left)
    printf(
        "warning: Intra_4x4_Horizontal prediction mode not allowed at mb %d\n",
        (int)currSlice->current_mb_nr);
 ...
 */
static int
intra4x4_diag_down_right_pred_mbaff(Macroblock *currMB, //!< current macroblock
                                    ColorPlane pl,      //!< current image plane
                                    int ioff, //!< pixel offset X within MB
                                    int joff) //!< pixel offset Y within MB
{
  ...
  p_Vid->getNeighbour(currMB, ioff, joff - 1, p_Vid->mb_size[IS_LUMA], &pix_b);
  p_Vid->getNeighbour(currMB, ioff - 1, joff - 1, p_Vid->mb_size[IS_LUMA],
                      &pix_d);
  ...
  if ((!block_available_up) || (!block_available_left) ||
      (!block_available_up_left))
    printf("warning: Intra_4x4_Diagonal_Down_Right prediction mode not allowed "
           "at mb %d\n",
           (int)currSlice->current_mb_nr);
  else {
    imgpel PredPixel[7];
 ...
 */
static int
intra4x4_diag_down_left_pred_mbaff(Macroblock *currMB, //!< current macroblock
                                   ColorPlane pl,      //!< current image plane
                                   int ioff, //!< pixel offset X within MB
                                   int joff) //!< pixel offset Y within MB
{
  ...
  p_Vid->getNeighbour(currMB, ioff, joff - 1, p_Vid->mb_size[IS_LUMA], &pix_b);
  p_Vid->getNeighbour(currMB, ioff + 4, joff - 1, p_Vid->mb_size[IS_LUMA],
                      &pix_c);

  pix_c.available =
      pix_c.available && !((ioff == 4) && ((joff == 4) || (joff == 12)));
  ...
  if (!block_available_up)
    printf("warning: Intra_4x4_Diagonal_Down_Left prediction mode not allowed "
           "at mb %d\n",
           (int)currSlice->current_mb_nr);
  else {
    imgpel **imgY = (pl) ? currSlice->dec_picture->imgUV[pl - 1]
 ...
 */
static int
intra4x4_vert_right_pred_mbaff(Macroblock *currMB, //!< current macroblock
                               ColorPlane pl,      //!< current image plane
                               int ioff,           //!< pixel offset X within MB
                               int joff)           //!< pixel offset Y within MB
{
  ...
  if ((!block_available_up) || (!block_available_left) ||
      (!block_available_up_left))
    printf("warning: Intra_4x4_Vertical_Right prediction mode not allowed at "
           "mb %d\n",
           (int)currSlice->current_mb_nr);
  {
    imgpel PredPixel[10];
 ...
 */
static int
intra4x4_vert_left_pred_mbaff(Macroblock *currMB, //!< current macroblock
                              ColorPlane pl,      //!< current image plane
                              int ioff,           //!< pixel offset X within MB
                              int joff)           //!< pixel offset Y within MB
{
 ...
 */
static int
intra4x4_hor_up_pred_mbaff(Macroblock *currMB, //!< current macroblock
                           ColorPlane pl,      //!< current image plane
                           int ioff,           //!< pixel offset X within MB
                           int joff)           //!< pixel offset Y within MB
{
  ...
  if (!block_available_left)
    printf("warning: Intra_4x4_Horizontal_Up prediction mode not allowed at mb "
           "%d\n",
           (int)currSlice->current_mb_nr);
  else {
    imgpel PredPixel[10];
 ...
 */
static int
intra4x4_hor_down_pred_mbaff(Macroblock *currMB, //!< current macroblock
                             ColorPlane pl,      //!< current image plane
                             int ioff,           //!< pixel offset X within MB
                             int joff)           //!< pixel offset Y within MB
{
  ...
  if ((!block_available_up) || (!block_available_left) ||
      (!block_available_up_left))
    printf("warning: Intra_4x4_Horizontal_Down prediction mode not allowed at "
           "mb %d\n",
           (int)currSlice->current_mb_nr);
  else {
    imgpel PredPixel[10];
