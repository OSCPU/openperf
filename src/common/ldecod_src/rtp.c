...
/*!
  ...
  in the semantics of the p_Vid-> and p_Inp-> structure members must be
  represented in this function as well!
  ...
  header.  Decoding has to start at bitoffset 0 (CAVLC) or bytreoffset 0
  (CABAC).
...
int GetRTPNALU(VideoParameters *p_Vid, NALU_t *nalu) {
  static uint16 first_call =
      1; //!< triggers sequence number initialization on first call
  static uint16 old_seq =
      0; //!< store the last RTP sequence number for loss detection
...

{
  ...
  memcpy(&p->timestamp, &p->packet[4],
         4); // change to shifts for unified byte sex
  ...
  if ((p->v != 2) || (p->p != 0) || (p->x != 0) || (p->cc != 0)) {
    printf(
        "DecomposeRTPpacket, RTP header consistency problem, header follows\n");
    DumpRTPHeader(p);
    return -1;
  }
  ...
  return 0;
#else
  printf("ERROR: we're in DecompseRTPpacket which shouldn't be called in SPEC "
         "CPU\n");
  return 1;
#endif
}
...
int RTPReadPacket(RTPpacket_t *p, int bitstream) {
  ...
  if (DecomposeRTPpacket(p) < 0) {
    // this should never happen, hence exit() is ok.  We probably do not want to
    // attempt to decode a packet that obviously wasn't generated by RTP
