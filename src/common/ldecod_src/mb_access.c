...
Boolean mb_is_available(int mbAddr, Macroblock *currMB) {
  ...
  if ((mbAddr < 0) ||
      (mbAddr > ((int)currMB->p_Slice->dec_picture->PicSizeInMbs -
                 1))) // if ((mbAddr < 0) || (mbAddr >
                      // ((int)p_Vid->dec_picture->PicSizeInMbs - 1)))
    return FALSE;
...
void CheckAvailabilityOfNeighbors(Macroblock *currMB) {
  ...
  StorablePicture *dec_picture =
      currMB->p_Slice->dec_picture; // p_Vid->dec_picture;
  ...
  if (dec_picture->mb_aff_frame_flag) {
    ...
    currMB->mbAvailA = (Boolean)(mb_is_available(currMB->mbAddrA, currMB) &&
                                 ((PicPos[cur_mb_pair].x) != 0));
    currMB->mbAvailB = (Boolean)(mb_is_available(currMB->mbAddrB, currMB));
    currMB->mbAvailC = (Boolean)(mb_is_available(currMB->mbAddrC, currMB) &&
                                 ((PicPos[cur_mb_pair + 1].x) != 0));
    currMB->mbAvailD = (Boolean)(mb_is_available(currMB->mbAddrD, currMB) &&
                                 ((PicPos[cur_mb_pair].x) != 0));
  } else {
  ...
  currMB->mb_left =
      (currMB->mbAvailA) ? &(currMB->p_Slice->mb_data[currMB->mbAddrA]) : NULL;
  currMB->mb_up =
      (currMB->mbAvailB) ? &(currMB->p_Slice->mb_data[currMB->mbAddrB]) : NULL;
}
 ...
 ************************************************************************
 */
void get_mb_pos(VideoParameters *p_Vid, int mb_addr, int mb_size[2], short *x,
                short *y) {
 ...
 ************************************************************************
 */
void getNonAffNeighbour(Macroblock *currMB, int xN, int yN, int mb_size[2],
                        PixelPos *pix) {
 ...
 ************************************************************************
 */
void getAffNeighbour(Macroblock *currMB, int xN, int yN, int mb_size[2],
                     PixelPos *pix) {
  VideoParameters *p_Vid = currMB->p_Vid;
  int maxW, maxH;
  int yM = -1;
      ...
      if (yN < 0) {
        if (!currMB->mb_field) {
          ...
          if ((currMB->mbAddrX & 0x01) == 0) {
            // top
            pix->mb_addr = currMB->mbAddrB;
            // for the deblocker if the current MB is a frame and the one above
            // is a field then the neighbor is the top MB of the pair
            if (currMB->mbAvailB) {
              if (!(currMB->DeblockCall == 1 &&
                    (p_Vid->mb_data[currMB->mbAddrB]).mb_field))
                pix->mb_addr += 1;
            }
        ...
        // yN >=0
        // for the deblocker if this is the extra edge then do this special
        // stuff
 ...
 ************************************************************************
 */
void get4x4Neighbour(Macroblock *currMB, int block_x, int block_y,
 ...
 ************************************************************************
 */
void get4x4NeighbourBase(Macroblock *currMB, int block_x, int block_y,
                         int mb_size[2], PixelPos *pix) {
  currMB->p_Vid->getNeighbour(currMB, block_x, block_y, mb_size, pix);
