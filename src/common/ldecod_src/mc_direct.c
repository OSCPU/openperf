      ...
      if (currMB->b8mode[k] == 0) {
        ...
        for (j0 = 2 * (k >> 1); j0 < 2 * (k >> 1) + 2; j0 += step_v0) {
          for (i0 = currMB->block_x + 2 * (k & 0x01);
            ...
            if (currSlice->mb_aff_frame_flag) {
              ...
              if (!currMB->mb_field &&
                  ((currSlice->listX[LIST_1][0]->iCodingType ==
                   ...
                   (currSlice->listX[LIST_1][0]->iCodingType ==
                    FIELD_CODING))) {
                if (iabs(dec_picture->poc -
                         currSlice->listX[LIST_1 + 4][0]->poc) >
                    iabs(dec_picture->poc -
                         currSlice->listX[LIST_1 + 2][0]->poc)) {
                  colocated =
                      p_Vid->active_sps->direct_8x8_inference_flag
                          ? &currSlice->listX[LIST_1 + 2][0]
                                 ->mv_info[RSD(currMB->block_y_aff + j0) >> 1]
                                          [RSD(i0)]
                          : &currSlice->listX[LIST_1 + 2][0]
                                 ->mv_info[(currMB->block_y_aff + j0) >> 1][i0];
                } else {
                  colocated =
                      p_Vid->active_sps->direct_8x8_inference_flag
              ...
              if (iabs(dec_picture->poc - list1[0]->bottom_field->poc) >
                ...
                colocated =
                    p_Vid->active_sps->direct_8x8_inference_flag
                        ? &list1[0]->top_field->mv_info
                               [RSD(currMB->block_y_aff + j0) >> 1][RSD(i0)]
                        : &list1[0]
                               ->top_field
                               ->mv_info[(currMB->block_y_aff + j0) >> 1][i0];
              } else {
              ...
              if (currSlice->structure == TOP_FIELD) {
                colocated = p_Vid->active_sps->direct_8x8_inference_flag
                                ? &list1[0]->frame->top_field->mv_info[RSD(
                                      currMB->block_y_aff + j0)][RSD(i0)]
                                : &list1[0]
                                       ->frame->top_field
                                       ->mv_info[currMB->block_y_aff + j0][i0];
              } else {
                colocated = p_Vid->active_sps->direct_8x8_inference_flag
                                ? &list1[0]->frame->bottom_field->mv_info[RSD(
                                      currMB->block_y_aff + j0)][RSD(i0)]
                                : &list1[0]
                                       ->frame->bottom_field
                                       ->mv_info[currMB->block_y_aff + j0][i0];
            ...
            if (ref_idx == -1) {
              for (j4 = currMB->block_y + j0;
                   j4 < currMB->block_y + j0 + step_v0; ++j4) {
              ...
              if ((currSlice->mb_aff_frame_flag &&
                ...
                for (iref = 0;
                  ...
                  if (currSlice->listX[LIST_0 + list_offset][iref]->top_field ==
                      ...
                      currSlice->listX[LIST_0 + list_offset][iref]
                              ->bottom_field == colocated->ref_pic[refList] ||
                      currSlice->listX[LIST_0 + list_offset][iref]->frame ==
                          colocated->ref_pic[refList]) {
                    mapped_idx = iref;
                    break;
                  } else //! invalid index. Default to zero even though this
              ...
              if (mapped_idx != INVALIDINDEX) {
                for (j = j0; j < j0 + step_v0; ++j) {
                  ...
                  for (i4 = i0; i4 < i0 + step_h0; ++i4) {
                    ...
                    if(currSlice->mb_aff_frame_flag /*&& (!p_Vid->active_sps->frame_mbs_only_flag || p_Vid->active_sps->direct_8x8_inference_flag)*/)
                    {
                    ...
                    } else if (/*!currSlice->mb_aff_frame_flag &&*/ !p_Vid
                      ...
                      if (iabs(dec_picture->poc - list1[0]->bottom_field->poc) >
                        ...
                        colocated =
                            p_Vid->active_sps->direct_8x8_inference_flag
                                ? &list1[0]->top_field->mv_info[RSD(j6) >> 1]
                                                               [RSD(i4)]
                                : &list1[0]->top_field->mv_info[(j6) >> 1][i4];
                      } else {
                        colocated =
                            p_Vid->active_sps->direct_8x8_inference_flag
                                ? &list1[0]->bottom_field->mv_info[RSD(j6) >> 1]
                                                                  [RSD(i4)]
                                : &list1[0]
                                       ->bottom_field->mv_info[(j6) >> 1][i4];
                      }
                    } else if (!p_Vid->active_sps->frame_mbs_only_flag &&
                               currSlice->field_pic_flag &&
                               currSlice->structure != list1[0]->structure &&
                      ...
                      if (currSlice->structure == TOP_FIELD) {
                        colocated =
                            p_Vid->active_sps->direct_8x8_inference_flag
                                ? &list1[0]->frame->top_field->mv_info[RSD(j6)]
                                                                      [RSD(i4)]
                        ...
                        colocated =
                            p_Vid->active_sps->direct_8x8_inference_flag
                                ? &list1[0]->frame->bottom_field->mv_info[RSD(
                                      j6)][RSD(i4)]
                                : &list1[0]
                                       ->frame->bottom_field->mv_info[j6][i4];
                      }
                    }
                    ...
                    if ((currSlice->mb_aff_frame_flag && !currMB->mb_field &&
                        ...
                        (!currSlice->mb_aff_frame_flag &&
                         ...
                         colocated->ref_pic[refList]->structure != FRAME))
                      mv_y *= 2;
                    else if ((currSlice->mb_aff_frame_flag &&
                              currMB->mb_field &&
                              colocated->ref_pic[refList]->structure ==
                                  FRAME) ||
                             (!currSlice->mb_aff_frame_flag &&
                              currSlice->field_pic_flag == 1 &&
                              colocated->ref_pic[refList]->structure == FRAME))
                    ...
                    mv_scale =
                        currSlice->mvscale[LIST_0 + list_offset][mapped_idx];

                    mv_info->ref_idx[LIST_0] = (signed char)mapped_idx;
                    mv_info->ref_idx[LIST_1] = 0;
                    ...
                    if (mv_scale == 9999 ||
                      ...
                      mv_info->mv[LIST_0].mv_x =
                          (short)((mv_scale * colocated->mv[refList].mv_x +
                                   128) >>
                                  8);
                      ...
                      mv_info->mv[LIST_1].mv_x =
                          (short)(mv_info->mv[LIST_0].mv_x -
                                  colocated->mv[refList].mv_x);
...
*************************************************************************************
*/
int get_colocated_info_4x4(Macroblock *currMB, StorablePicture *list1, int i,
                           int j) {
  if (list1->is_long_term)
    ...
    int moving = !(
        (((fs->ref_idx[LIST_0] == 0) && (iabs(fs->mv[LIST_0].mv_x) >> 1 == 0) &&
...
*Will be replaced with function that will access directly motion information
*************************************************************************************
*/
int get_colocated_info_8x8(Macroblock *currMB, StorablePicture *list1, int i,
                           int j) {
  if (list1->is_long_term)
    return 1;
  else {
    ...
    VideoParameters *p_Vid = currMB->p_Vid;
    if ((currSlice->mb_aff_frame_flag) ||
        (!p_Vid->active_sps->frame_mbs_only_flag &&
         ((!currSlice->structure && list1->iCodingType == FIELD_CODING) ||
      ...
      if (currSlice->field_pic_flag &&
        ...
        if ((currSlice->mb_aff_frame_flag &&
             ((!currMB->mb_field && list1->motion.mb_field[currMB->mbAddrX]) ||
      ...
      int moving;
      if (currMB->p_Vid->separate_colour_plane_flag &&
          currMB->p_Vid->yuv_format == YUV444)
        fs =
            &list1->JVmv_info[currMB->p_Slice->colour_plane_id][RSD(j)][RSD(i)];
      moving = !((((fs->ref_idx[LIST_0] == 0) &&
                   (iabs(fs->mv[LIST_0].mv_x) >> 1 == 0) &&
                   (iabs(fs->mv[LIST_0].mv_y) >> 1 == 0))) ||
                 ((fs->ref_idx[LIST_0] == -1) && (fs->ref_idx[LIST_1] == 0) &&
...
void update_direct_types(Slice *currSlice) {
  if (currSlice->active_sps->direct_8x8_inference_flag)
    currSlice->update_direct_mv_info = currSlice->direct_spatial_mv_pred_flag
                                           ? update_direct_mv_info_spatial_8x8
                                           : update_direct_mv_info_temporal;
  else
    currSlice->update_direct_mv_info = currSlice->direct_spatial_mv_pred_flag
                                           ? update_direct_mv_info_spatial_4x4
                                           : update_direct_mv_info_temporal;
}
