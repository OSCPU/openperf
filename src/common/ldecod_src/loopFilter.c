...
#else
static void DeblockParallel(VideoParameters *p_Vid, StorablePicture *p,
                            unsigned int column, int block, int n_last) {
  int i, j;
...
void DeblockPicture(VideoParameters *p_Vid, StorablePicture *p) {
  ...
  for (i = 0; i < k; i++) {
    ...
    int n_start =
        (i < p->PicWidthInMbs) ? 0 : ((i - p->PicWidthInMbs) >> 1) + 1;
...
void DeblockPicturePartially(VideoParameters *p_Vid, StorablePicture *p,
                             int iStart, int iEnd) {
  int i;
...
static void DeblockMb(VideoParameters *p_Vid, StorablePicture *p, int MbQAddr) {
  ...
  if (MbQ->DFDisableIdc == 1) {
    ...
    byte Strength[16];
#else
    // avoid misalign core dumps at low opt: ask for 2x8 byte, not 16x1 byte
    int64 Str8[2];
    byte *Strength = (byte *)Str8;
#endif
    ...
    int mvlimit =
        ((p->structure != FRAME) || (p->mb_aff_frame_flag && MbQ->mb_field))
            ? 2
            : 4;
    ...
    filterNon8x8LumaEdgesFlag[1] = filterNon8x8LumaEdgesFlag[3] =
        !(MbQ->luma_transform_size_8x8_flag);
    ...
    if (MbQ->DFDisableIdc == 2) {
      ...
      // if this the bottom of a frame macroblock pair then always filter the
      // top edge
      filterTopMbEdgeFlag =
          (p->mb_aff_frame_flag && !MbQ->mb_field && (MbQAddr & 0x01))
              ? 1
              : MbQ->mbAvailB;
    }
    ...
    for (edge = 0; edge < 4; ++edge) {
      ...
      if (MbQ->cbp == 0) {
        if (filterNon8x8LumaEdgesFlag[edge] == 0 &&
            active_sps->chroma_format_idc != YUV444)
          continue;
        else if (edge > 0) {
          if (((MbQ->mb_type == PSKIP && currSlice->slice_type == P_SLICE) ||
               (MbQ->mb_type == P16x16) || (MbQ->mb_type == P16x8)))
            continue;
          else if ((edge & 0x01) && ((MbQ->mb_type == P8x16) ||
      ...
      if (edge || filterLeftMbEdgeFlag) {
        ...
        if ((p_Strength64[0]) ||
            (p_Strength64[1])) // only if one of the 16 Strength bytes is != 0
        {
          if (filterNon8x8LumaEdgesFlag[edge]) {
            p_Vid->EdgeLoopLumaVer(PLANE_Y, imgY, Strength, MbQ, edge << 2, p);
            if (currSlice->is_not_independent) {
              p_Vid->EdgeLoopLumaVer(PLANE_U, imgUV[0], Strength, MbQ,
                                     edge << 2, p);
              p_Vid->EdgeLoopLumaVer(PLANE_V, imgUV[1], Strength, MbQ,
                                     edge << 2, p);
            }
          }
          if (active_sps->chroma_format_idc == YUV420 ||
              active_sps->chroma_format_idc == YUV422) {
    ...
    for (edge = 0; edge < 4; ++edge) {
      ...
      if (MbQ->cbp == 0) {
        if (filterNon8x8LumaEdgesFlag[edge] == 0 &&
          ...
          if (((MbQ->mb_type == PSKIP && currSlice->slice_type == P_SLICE) ||
               (MbQ->mb_type == P16x16) || (MbQ->mb_type == P8x16)))
            continue;
          else if ((edge & 0x01) && ((MbQ->mb_type == P16x8) ||
                                     (currSlice->slice_type == B_SLICE &&
                                      MbQ->mb_type == BSKIP_DIRECT &&
                                      active_sps->direct_8x8_inference_flag)))
            continue;
      ...
      if (edge || filterTopMbEdgeFlag) {
            ...
            (p_Strength64[1])) // only if one of the 16 Strength bytes is != 0
        {
          if (filterNon8x8LumaEdgesFlag[edge]) {
            ...
            if (currSlice->is_not_independent) {
              p_Vid->EdgeLoopLumaHor(PLANE_U, imgUV[0], Strength, MbQ,
                                     edge << 2, p);
              p_Vid->EdgeLoopLumaHor(PLANE_V, imgUV[1], Strength, MbQ,
                                     edge << 2, p);
            }
          }
        ...
        if (!edge && !MbQ->mb_field &&
            MbQ->mixedModeEdgeFlag) // currSlice->mixedModeEdgeFlag)
        {
          // this is the extra horizontal edge between a frame macroblock pair
          // and a field above it
          MbQ->DeblockCall = 2;
          p_Vid->GetStrengthHor(Strength, MbQ, MB_BLOCK_SIZE, mvlimit,
                                p); // Strength for 4 blks in 1 stripe

          // if( *((int*)Strength) )                      // only if one of the
          // 4 Strength bytes is != 0
          {
            if (filterNon8x8LumaEdgesFlag[edge]) {

              p_Vid->EdgeLoopLumaHor(PLANE_Y, imgY, Strength, MbQ,
                                     MB_BLOCK_SIZE, p);
              if (currSlice->is_not_independent) {
                p_Vid->EdgeLoopLumaHor(PLANE_U, imgUV[0], Strength, MbQ,
                                       MB_BLOCK_SIZE, p);
            ...
            if (active_sps->chroma_format_idc == YUV420 ||
              ...
              edge_cr = chroma_edge[1][edge][p->chroma_format_idc];
              if ((imgUV != NULL) && (edge_cr >= 0)) {
                p_Vid->EdgeLoopChromaHor(imgUV[0], Strength, MbQ, MB_BLOCK_SIZE,
                                         0, p);
                p_Vid->EdgeLoopChromaHor(imgUV[1], Strength, MbQ, MB_BLOCK_SIZE,
                                         1, p);
              }
            }
          }
