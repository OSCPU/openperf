 ...
 *
 * \brief
 *   Calculate if decoder leaky bucket parameters meets HRD constraints
 *specified by encoder.
 *
 * \author
 ...
 ***********************************************************************
 * \brief
 *   Calculates if decoder leaky bucket parameters meets HRD constraints
 *specified by encoder. \param p_Inp Structure which contains decoder leaky
 *bucket parameters. \return None \par SideEffects None. \par Notes Failure if
 *LeakyBucketParam file is missing or if it does not have the correct number of
 *entries. \author Shankar Regunathan                   shanre@microsoft.com
 *  \date
 *      December 06, 2001.
 ***********************************************************************
 */
...
void calc_buffer(InputParameters *p_Inp) {
  ...
  if (iBucket > 0) {
    if (iBucket < NumberLeakyBuckets) {
      ...
      B_interp =
          (float)(Bmin[iBucket] * frac1 + Bmin[iBucket - 1] * frac2) / dnr;
      F_interp =
          (float)(Fmin[iBucket] * frac1 + Fmin[iBucket - 1] * frac2) / dnr;
    } else {
