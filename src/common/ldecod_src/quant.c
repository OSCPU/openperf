...
int quant_intra_default[16] = {6,  13, 20, 28, 13, 20, 28, 32,
                               20, 28, 32, 37, 28, 32, 37, 42};

int quant_inter_default[16] = {10, 14, 20, 24, 14, 20, 24, 27,
...
void init_qp_process(VideoParameters *p_Vid) {
  int bitdepth_qp_scale =
      imax(p_Vid->bitdepth_luma_qp_scale, p_Vid->bitdepth_chroma_qp_scale);
  int i;

  // We should allocate memory outside of this process since maybe we will have
  // a change of SPS and we may need to recreate these. Currently should only
  // support same bitdepth
  if (p_Vid->qp_per_matrix == NULL)
    if ((p_Vid->qp_per_matrix = (int *)malloc((MAX_QP + 1 + bitdepth_qp_scale) *
                                              sizeof(int))) == NULL)
      no_mem_exit("init_qp_process: p_Vid->qp_per_matrix");

  if (p_Vid->qp_rem_matrix == NULL)
    if ((p_Vid->qp_rem_matrix = (int *)malloc((MAX_QP + 1 + bitdepth_qp_scale) *
                                              sizeof(int))) == NULL)
 ...
 ************************************************************************
 * \brief
 *    For mapping the q-matrix to the active id and calculate quantisation
 *values
 *
 * \param currSlice
 *    Slice pointer
        ...
        if (i < 6) {
          if (!sps->seq_scaling_list_present_flag[i]) // fall-back rule A
          {
            ...
            if (sps->UseDefaultScalingMatrix4x4Flag[i])
              currSlice->qmatrix[i] =
                  (i < 3) ? quant_intra_default : quant_inter_default;
            else
          ...
          if (!sps->seq_scaling_list_present_flag[i]) // fall-back rule A
          {
            ...
            if (sps->UseDefaultScalingMatrix8x8Flag[i - 6])
              currSlice->qmatrix[i] = (i == 6 || i == 8 || i == 10)
...
static void set_dequant4x4(int (*InvLevelScale4x4)[4], const int (*dequant)[4],
                           int *qmatrix) {
  int j;
  for (j = 0; j < 4; j++) {
...
static void set_dequant8x8(int (*InvLevelScale8x8)[8], const int (*dequant)[8],
                           int *qmatrix) {
  int j;
  for (j = 0; j < 8; j++) {
...
void CalculateQuant4x4Param(Slice *currSlice) {
  ...
  for (k = 0; k < 6; k++) {
    set_dequant4x4(*InvLevelScale4x4_Intra_0++, *p_dequant_coef,
                   currSlice->qmatrix[0]);
    set_dequant4x4(*InvLevelScale4x4_Intra_1++, *p_dequant_coef,
                   currSlice->qmatrix[1]);
    set_dequant4x4(*InvLevelScale4x4_Intra_2++, *p_dequant_coef,
                   currSlice->qmatrix[2]);
    set_dequant4x4(*InvLevelScale4x4_Inter_0++, *p_dequant_coef,
                   currSlice->qmatrix[3]);
    set_dequant4x4(*InvLevelScale4x4_Inter_1++, *p_dequant_coef,
                   currSlice->qmatrix[4]);
    set_dequant4x4(*InvLevelScale4x4_Inter_2++, *p_dequant_coef++,
                   currSlice->qmatrix[5]);
  }
}
...
void CalculateQuant8x8Param(Slice *currSlice) {
  ...
  for (k = 0; k < 6; k++) {
    set_dequant8x8(*InvLevelScale8x8_Intra_0++, *p_dequant_coef,
                   currSlice->qmatrix[6]);
    set_dequant8x8(*InvLevelScale8x8_Inter_0++, *p_dequant_coef++,
                   currSlice->qmatrix[7]);
  }
  ...
  {
    for (k = 0; k < 6; k++) {
      set_dequant8x8(*InvLevelScale8x8_Intra_1++, *p_dequant_coef,
                     currSlice->qmatrix[8]);
      set_dequant8x8(*InvLevelScale8x8_Inter_1++, *p_dequant_coef,
                     currSlice->qmatrix[9]);
      set_dequant8x8(*InvLevelScale8x8_Intra_2++, *p_dequant_coef,
                     currSlice->qmatrix[10]);
      set_dequant8x8(*InvLevelScale8x8_Inter_2++, *p_dequant_coef++,
                     currSlice->qmatrix[11]);
    }
  }
}
