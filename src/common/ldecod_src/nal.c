...
*          position of the last byte pos. If the last-byte was entirely a
*stuffing byte, it is removed, and the last_byte_pos is updated.
*
************************************************************************/
...
int RBSPtoSODB(byte *streamBuffer, int last_byte_pos) {
  ...
  bitoffset = 0;
  // find trailing 1
  ctr_bit = (streamBuffer[last_byte_pos - 1] &
             (0x01 << bitoffset)); // set up control bit
...
int EBSPtoRBSP(byte *streamBuffer, int end_bytepos, int begin_bytepos) {
  ...
  for (i = begin_bytepos; i < end_bytepos;
       ++i) { // starting from begin_bytepos to avoid header information
