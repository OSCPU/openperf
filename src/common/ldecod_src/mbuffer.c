...
static void insert_picture_in_dpb(VideoParameters *p_Vid, FrameStore *fs,
                                  StorablePicture *p);
...
int getDpbSize(seq_parameter_set_rbsp_t *active_sps) {
  int pic_size = (active_sps->pic_width_in_mbs_minus1 + 1) *
  ...
  if (active_sps->vui_parameters_present_flag &&
      active_sps->vui_seq_parameters.bitstream_restriction_flag) {
    int size_vui;
...
#ifdef _DEBUG
    if (size_vui < size) {
      printf("Warning: max_dec_frame_buffering(%d) is less than DPB size(%d) "
             "calculated from Profile/Level.\n",
             size_vui, size);
    }
#endif
    size = size_vui;
...
void check_num_ref(DecodedPictureBuffer *p_Dpb) {
  if ((int)(p_Dpb->ltref_frames_in_buffer + p_Dpb->ref_frames_in_buffer) >
 ...
 ************************************************************************
 * \brief
 *    Allocate memory for decoded picture buffer and initialize with sane
 *values.
 *
 ************************************************************************
 */
void init_dpb(VideoParameters *p_Vid, DecodedPictureBuffer *p_Dpb) {
  ...
  if (active_sps->profile_idc == MVC_HIGH ||
      active_sps->profile_idc == STEREO_HIGH)
...
#endif
  {
    error("DPB size at specified level is smaller than the specified number of "
          "reference frames. This is not allowed.\n",
          1000);
  }
  ...
  for (i = 0; i < 6; i++)
  {
  currSlice->listX[i] = calloc(MAX_LIST_SIZE, sizeof (StorablePicture*)); // +1
  for reordering if (NULL==currSlice->listX[i]) no_mem_exit("init_dpb:
  currSlice->listX[i]");
  }
  */
  /* allocate a dummy storable picture */
  p_Vid->no_reference_picture =
      alloc_storable_picture(p_Vid, FRAME, p_Vid->width, p_Vid->height,
...
#endif
    {
      error("DPB size at specified level is smaller than the specified number "
            "of reference frames. This is not allowed.\n",
            1000);
    }
 ...
 ************************************************************************
 * \brief
 *    Allocate memory for decoded picture buffer frame stores and initialize
 *with sane values.
 *
 * \return
 ...
 ************************************************************************
 */
StorablePicture *alloc_storable_picture(VideoParameters *p_Vid,
                                        PictureStructure structure, int size_x,
                                        int size_y, int size_x_cr,
                                        int size_y_cr) {
  seq_parameter_set_rbsp_t *active_sps = p_Vid->active_sps;
  ...
  // == FRAME)?"FRAME":(type == TOP_FIELD)?"TOP_FIELD":"BOTTOM_FIELD", size_x,
  // size_y, size_x_cr, size_y_cr);
  ...
  get_mem2DpelWithPad(&(s->imgY), size_y, size_x, p_Vid->iLumaPadY,
                      p_Vid->iLumaPadX);
  s->iLumaStride = size_x + 2 * p_Vid->iLumaPadX;
  s->iLumaExpandedHeight = size_y + 2 * p_Vid->iLumaPadY;

  if (active_sps->chroma_format_idc != YUV400)
    get_mem3DpelWithPad(&(s->imgUV), 2, size_y_cr, size_x_cr,
                        p_Vid->iChromaPadY,
                        p_Vid->iChromaPadX); // get_mem3Dpel (&(s->imgUV), 2,
                                             // size_y_cr, size_x_cr);
  ...
  get_mem2Dshort(&(s->slice_id), size_y / MB_BLOCK_SIZE,
                 size_x / MB_BLOCK_SIZE);
  ...
  if ((p_Vid->separate_colour_plane_flag != 0)) {
    for (nplane = 0; nplane < MAX_PLANE; nplane++) {
      get_mem2Dmp(&s->JVmv_info[nplane], size_y / BLOCK_SIZE,
                  size_x / BLOCK_SIZE);
      alloc_pic_motion(&s->JVmotion[nplane], size_y / BLOCK_SIZE,
                       size_x / BLOCK_SIZE);
    }
  }
  ...
  if (!p_Vid->active_sps->frame_mbs_only_flag && structure != FRAME) {
    ...
    for (i = 0; i < 2; i++) {
      s->listX[i] =
          calloc(MAX_LIST_SIZE, sizeof(StorablePicture *)); // +1 for reordering
      if (NULL == s->listX[i])
 ...
 ************************************************************************
 * \brief
 *    compares two stored pictures by picture number for qsort in descending
 *order
 *
 ************************************************************************
 */
 ...
 ************************************************************************
 */
void gen_pic_list_from_frame_list(PictureStructure currStructure,
 ...
 ************************************************************************
 */
static void gen_pic_list_from_frame_interview_list(
    PictureStructure currStrcture, FrameStore **fs_list, int list_idx,
    StorablePicture **list, signed char *list_size) {
  int i;
  ...
  if (currSlice->structure == FRAME) {
    ...
    for (i = 0; i < p_Dpb->ltref_frames_in_buffer; i++) {
      if (p_Dpb->fs_ltref[i]->is_used == 3) {
        if (p_Dpb->fs_ltref[i]->frame->is_long_term) {
          p_Dpb->fs_ltref[i]->frame->long_term_pic_num =
              p_Dpb->fs_ltref[i]->frame->long_term_frame_idx;
        }
      }
    }
  } else {
      ...
      if (p_Dpb->fs_ref[i]->is_reference) {
        ...
        if (p_Dpb->fs_ref[i]->is_reference & 1) {
          p_Dpb->fs_ref[i]->top_field->pic_num =
              (2 * p_Dpb->fs_ref[i]->frame_num_wrap) + add_top;
        }
        if (p_Dpb->fs_ref[i]->is_reference & 2) {
          p_Dpb->fs_ref[i]->bottom_field->pic_num =
              (2 * p_Dpb->fs_ref[i]->frame_num_wrap) + add_bottom;
        }
      }
    }
    // update long_term_pic_num
    for (i = 0; i < p_Dpb->ltref_frames_in_buffer; i++) {
      if (p_Dpb->fs_ltref[i]->is_long_term & 1) {
        p_Dpb->fs_ltref[i]->top_field->long_term_pic_num =
            2 * p_Dpb->fs_ltref[i]->top_field->long_term_frame_idx + add_top;
      }
      if (p_Dpb->fs_ltref[i]->is_long_term & 2) {
        p_Dpb->fs_ltref[i]->bottom_field->long_term_pic_num =
            2 * p_Dpb->fs_ltref[i]->bottom_field->long_term_frame_idx +
            add_bottom;
      }
    }
  }
...
void init_lists_p_slice(Slice *currSlice) {
  ...
  if (currSlice->structure == FRAME) {
    for (i = 0; i < p_Dpb->ref_frames_in_buffer; i++) {
      if (p_Dpb->fs_ref[i]->is_used == 3) {
        ...
        if ((p_Dpb->fs_ref[i]->frame->used_for_reference) &&
            (!p_Dpb->fs_ref[i]->frame->is_long_term) &&
            (p_Dpb->fs_ref[i]->frame->view_id == curr_view_id))
#else
        if ((p_Dpb->fs_ref[i]->frame->used_for_reference) &&
            (!p_Dpb->fs_ref[i]->frame->is_long_term))
#endif
    ...
    qsort((void *)currSlice->listX[0], list0idx, sizeof(StorablePicture *),
          compare_pic_by_pic_num_desc);
    currSlice->listXsize[0] = (signed char)list0idx;
    // printf("listX[0] (PicNum): "); for (i=0; i<list0idx; i++){printf ("%d  ",
...
#if (MVC_EXTENSION_ENABLE)
        if (p_Dpb->fs_ltref[i]->frame->is_long_term &&
            (p_Dpb->fs_ltref[i]->frame->view_id == curr_view_id))
#else
        if (p_Dpb->fs_ltref[i]->frame->is_long_term)
#endif
        {
    ...
    qsort((void *)&currSlice->listX[0][(short)currSlice->listXsize[0]],
          list0idx - currSlice->listXsize[0], sizeof(StorablePicture *),
...
#if (MVC_EXTENSION_ENABLE)
      if (p_Dpb->fs_ref[i]->is_reference &&
          (p_Dpb->fs_ref[i]->view_id == curr_view_id))
#else
      if (p_Dpb->fs_ref[i]->is_reference)
#endif
      {
    ...
    qsort((void *)fs_list0, list0idx, sizeof(FrameStore *),
          compare_fs_by_frame_num_desc);

    // printf("fs_list0 (FrameNum): "); for (i=0; i<list0idx; i++){printf ("%d
    // ", fs_list0[i]->frame_num_wrap);} printf("\n");

    currSlice->listXsize[0] = 0;
    gen_pic_list_from_frame_list(currSlice->structure, fs_list0, list0idx,
                                 currSlice->listX[0], &currSlice->listXsize[0],
                                 0);

    // printf("listX[0] (PicNum): "); for (i=0; i < currSlice->listXsize[0];
    // i++){printf ("%d  ", currSlice->listX[0][i]->pic_num);} printf("\n");
    ...
    qsort((void *)fs_listlt, listltidx, sizeof(FrameStore *),
          compare_fs_by_lt_pic_idx_asc);

    gen_pic_list_from_frame_list(currSlice->structure, fs_listlt, listltidx,
                                 currSlice->listX[0], &currSlice->listXsize[0],
                                 1);
  ...
  if (currSlice->svc_extension_flag == 0) {
    ...
    if (currSlice->structure == FRAME) {
      append_interview_list(p_Dpb, 0, 0, currSlice->fs_listinterview0,
                            &currSlice->listinterviewidx0, currPOC,
                            curr_view_id, anchor_pic_flag);

      for (i = 0; i < (unsigned int)currSlice->listinterviewidx0; i++) {
        currSlice->listX[0][list0idx++] =
            currSlice->fs_listinterview0[i]->frame;
      }
      currSlice->listXsize[0] = (signed char)list0idx;
    } else {
      append_interview_list(p_Dpb, currSlice->structure, 0,
                            currSlice->fs_listinterview0,
                            &currSlice->listinterviewidx0, currPOC,
                            curr_view_id, anchor_pic_flag);
      gen_pic_list_from_frame_interview_list(
          currSlice->structure, currSlice->fs_listinterview0,
          currSlice->listinterviewidx0, currSlice->listX[0],
          &currSlice->listXsize[0]);
    }
  }
#endif
  // set max size
  currSlice->listXsize[0] = (signed char)imin(
      currSlice->listXsize[0], currSlice->num_ref_idx_active[LIST_0]);
  currSlice->listXsize[1] = (signed char)imin(
      currSlice->listXsize[1], currSlice->num_ref_idx_active[LIST_1]);
  ...
  currSlice->view_id, currSlice->structure==FRAME ?
  "FRM":(currSlice->structure==TOP_FIELD ? "TOP":"BOT")); for(i=0; i<(unsigned
  int)(currSlice->listXsize[0]); i++)	//ref list 0
        {
          printf("   %2d -> POC: %4d PicNum: %4d ViewID: %d\n", i,
  currSlice->listX[0][i]->poc, currSlice->listX[0][i]->pic_num,
...
void init_lists_b_slice(Slice *currSlice) {
...

  {
    ...
    if (currSlice->structure == FRAME) {
        ...
        if (p_Dpb->fs_ref[i]->is_used == 3) {
...
#endif
          {
            if (currSlice->framepoc >=
                p_Dpb->fs_ref[i]
                    ->frame->poc) //! KS use >= for error concealment
                                  //            if (p_Vid->framepoc >
      ...
      qsort((void *)&currSlice->listX[0][list0idx_1], list0idx - list0idx_1,
      ...
      currSlice->listXsize[0] = currSlice->listXsize[1] = (signed char)list0idx;

      //      printf("currSlice->listX[0] currPoc=%d (Poc): ", p_Vid->framepoc);
      //      for (i=0; i<currSlice->listXsize[0]; i++){printf ("%d  ",
      //      currSlice->listX[0][i]->poc);} printf("\n");
      //      printf("currSlice->listX[1] currPoc=%d (Poc): ", p_Vid->framepoc);
      //      for (i=0; i<currSlice->listXsize[1]; i++){printf ("%d  ",
      //      currSlice->listX[1][i]->poc);} printf("\n");
      ...
      qsort((void *)&currSlice->listX[0][(short)currSlice->listXsize[0]],
            list0idx - currSlice->listXsize[0], sizeof(StorablePicture *),
            compare_pic_by_lt_pic_num_asc);
      qsort((void *)&currSlice->listX[1][(short)currSlice->listXsize[0]],
            list0idx - currSlice->listXsize[0], sizeof(StorablePicture *),
            compare_pic_by_lt_pic_num_asc);
      currSlice->listXsize[0] = currSlice->listXsize[1] = (signed char)list0idx;
    } else {
      ...
      //      printf("fs_list0 currPoc=%d (Poc): ", currSlice->ThisPOC); for
      //      (i=0; i<list0idx; i++){printf ("%d  ", fs_list0[i]->poc);}
      //      printf("\n"); printf("fs_list1 currPoc=%d (Poc): ",
      //      currSlice->ThisPOC); for (i=0; i<list0idx; i++){printf ("%d  ",
      //      fs_list1[i]->poc);} printf("\n");
      ...
      gen_pic_list_from_frame_list(currSlice->structure, fs_list0, list0idx,
                                   currSlice->listX[0],
                                   &currSlice->listXsize[0], 0);
      gen_pic_list_from_frame_list(currSlice->structure, fs_list1, list0idx,
                                   currSlice->listX[1],
                                   &currSlice->listXsize[1], 0);
  ...
  if ((currSlice->listXsize[0] == currSlice->listXsize[1]) &&
      (currSlice->listXsize[0] > 1)) {
    // check if lists are identical, if yes swap first two elements of
    // currSlice->listX[1]
    int diff = 0;
  ...
  if (currSlice->svc_extension_flag == 0) {
    ...
    if (currSlice->structure == FRAME) {
      append_interview_list(p_Dpb, 0, 0, currSlice->fs_listinterview0,
                            &currSlice->listinterviewidx0, currPOC,
                            curr_view_id, anchor_pic_flag);
      append_interview_list(p_Dpb, 0, 1, currSlice->fs_listinterview1,
                            &currSlice->listinterviewidx1, currPOC,
                            curr_view_id, anchor_pic_flag);
      ...
      for (i = 0; i < (unsigned int)currSlice->listinterviewidx1; i++) {
        currSlice->listX[1][list0idx++] =
            currSlice->fs_listinterview1[i]->frame;
      }
      currSlice->listXsize[1] = (signed char)list0idx;
    } else {
      append_interview_list(p_Dpb, currSlice->structure, 0,
                            currSlice->fs_listinterview0,
                            &currSlice->listinterviewidx0, currPOC,
      ...
      gen_pic_list_from_frame_interview_list(
          ...
          currSlice->listinterviewidx0, currSlice->listX[0],
          &currSlice->listXsize[0]);
      append_interview_list(p_Dpb, currSlice->structure, 1,
                            currSlice->fs_listinterview1,
                            &currSlice->listinterviewidx1, currPOC,
                            curr_view_id, anchor_pic_flag);
      gen_pic_list_from_frame_interview_list(
          currSlice->structure, currSlice->fs_listinterview1,
          currSlice->listinterviewidx1, currSlice->listX[1],
  ...
  currSlice->view_id, currSlice->structure==FRAME ?
  "FRM":(currSlice->structure==TOP_FIELD ? "TOP":"BOT")); for(i=0; i<(unsigned
  int)(currSlice->listXsize[1]); i++)	//ref list 1
        {
          printf("   %2d -> POC: %4d PicNum: %4d ViewID: %d\n", i,
  currSlice->listX[1][i]->poc, currSlice->listX[1][i]->pic_num,
 ...
 ************************************************************************
 */
StorablePicture *get_long_term_pic(DecodedPictureBuffer *p_Dpb,
                                   int LongtermPicNum) {
  VideoParameters *p_Vid = p_Dpb->p_Vid;
  unsigned i;
 ...
 ************************************************************************
 */
static void reorder_short_term(Slice *currSlice, int cur_list,
 ...
 ************************************************************************
 */
static void reorder_long_term(Slice *currSlice, StorablePicture **RefPicListX,
  ...
  for (cIdx = *refIdxLX; cIdx <= num_ref_idx_lX_active_minus1 + 1; cIdx++) {
    if (RefPicListX[cIdx]) {
      if ((!RefPicListX[cIdx]->is_long_term) ||
          (RefPicListX[cIdx]->long_term_pic_num != LongTermPicNum))
        RefPicListX[nIdx++] = RefPicListX[cIdx];
    }
  }
}
...
void reorder_ref_pic_list(Slice *currSlice, int cur_list) {
  int *reordering_of_pic_nums_idc =
      currSlice->reordering_of_pic_nums_idc[cur_list];
  ...
  for (i = 0; reordering_of_pic_nums_idc[i] != 3; i++) {
    ...
    if (reordering_of_pic_nums_idc[i] < 2) {
      if (reordering_of_pic_nums_idc[i] == 0) {
        if (picNumLXPred - (abs_diff_pic_num_minus1[i] + 1) < 0)
          picNumLXNoWrap =
              picNumLXPred - (abs_diff_pic_num_minus1[i] + 1) + maxPicNum;
        else
...
#if (MVC_EXTENSION_ENABLE)
      reorder_short_term(currSlice, cur_list, num_ref_idx_lX_active_minus1,
                         picNumLX, &refIdxLX, -1);
#else
      reorder_short_term(currSlice, cur_list, num_ref_idx_lX_active_minus1,
                         picNumLX, &refIdxLX);
    ...
    } else //(remapping_of_pic_nums_idc[i] == 2)
    {
      ...
      reorder_long_term(currSlice, currSlice->listX[cur_list],
                        num_ref_idx_lX_active_minus1, long_term_pic_idx[i],
                        &refIdxLX, -1);
#else
      reorder_long_term(currSlice, currSlice->listX[cur_list],
                        num_ref_idx_lX_active_minus1, long_term_pic_idx[i],
  ...
  currSlice->listXsize[cur_list] =
      (signed char)(num_ref_idx_lX_active_minus1 + 1);
}
...
void update_ref_list(DecodedPictureBuffer *p_Dpb, int curr_view_id) {
  ...
  for (i = 0, j = 0; i < p_Dpb->used_size; i++) {
    if (is_short_term_reference(p_Dpb->fs[i]) &&
        p_Dpb->fs[i]->view_id == curr_view_id) {
...
static void idr_memory_management(DecodedPictureBuffer *p_Dpb,
  ...
  if (p->no_output_of_prior_pics_flag) {
    ...
    if (p_Vid->profile_idc == MVC_HIGH ||
        p_Vid->profile_idc == STEREO_HIGH) // if (svc_extension_flag == 0)
      flush_dpb(p_Dpb, -1);
    else
...
static void sliding_window_memory_management(DecodedPictureBuffer *p_Dpb,
  ...
  assert(!p->idr_flag);
  // if this is a reference pic with sliding sliding window, unmark first ref
  // frame
  if (p_Dpb->ref_frames_in_buffer ==
      p_Dpb->num_ref_frames - p_Dpb->ltref_frames_in_buffer) {
#if (MVC_EXTENSION_ENABLE)
    if (p_Dpb->p_Vid->profile_idc == MVC_HIGH ||
        p_Dpb->p_Vid->profile_idc == STEREO_HIGH) {
      for (i = 0; i < p_Dpb->used_size; i++) {
        if (p_Dpb->fs[i]->is_reference && (!(p_Dpb->fs[i]->is_long_term)) &&
            p_Dpb->fs[i]->view_id == p->view_id) {
 ...
 */
static void
mm_unmark_short_term_for_reference(DecodedPictureBuffer *p_Dpb,
                                   StorablePicture *p,
                                   int difference_of_pic_nums_minus1) {
  int picNumX;
    ...
    if (p_Dpb->fs_ltref[i]->view_id == p->view_id) {
      ...
      if (p->structure == FRAME) {
        if ((p_Dpb->fs_ltref[i]->is_reference == 3) &&
          ...
          if (p_Dpb->fs_ltref[i]->frame->long_term_pic_num ==
              long_term_pic_num) {
        ...
        if ((p_Dpb->fs_ltref[i]->is_reference & 1) &&
          ...
          if (p_Dpb->fs_ltref[i]->top_field->long_term_pic_num ==
              long_term_pic_num) {
        ...
        if ((p_Dpb->fs_ltref[i]->is_reference & 2) &&
            ((p_Dpb->fs_ltref[i]->is_long_term & 2))) {
          if (p_Dpb->fs_ltref[i]->bottom_field->long_term_pic_num ==
              long_term_pic_num) {
 ...
 ************************************************************************
 * \brief
 *    Mark a long-term reference frame or complementary field pair unused for
 *referemce
 ************************************************************************
 */
#if (MVC_EXTENSION_ENABLE)
static void unmark_long_term_frame_for_reference_by_frame_idx(
    DecodedPictureBuffer *p_Dpb, int long_term_frame_idx, int view_id)
#else
static void
unmark_long_term_frame_for_reference_by_frame_idx(DecodedPictureBuffer *p_Dpb,
...
#endif
{
  ...
  for (i = 0; i < p_Dpb->ltref_frames_in_buffer; i++) {
    ...
    if (p_Dpb->fs_ltref[i]->long_term_frame_idx == long_term_frame_idx &&
        p_Dpb->fs_ltref[i]->view_id == view_id)
#else
    if (p_Dpb->fs_ltref[i]->long_term_frame_idx == long_term_frame_idx)
#endif
      unmark_for_long_term_reference(p_Dpb->fs_ltref[i]);
...
static void unmark_long_term_field_for_reference_by_frame_idx(
    ...
    int curr_pic_num, int curr_view_id)
#else
static void unmark_long_term_field_for_reference_by_frame_idx(
    DecodedPictureBuffer *p_Dpb, PictureStructure structure,
    int long_term_frame_idx, int mark_current, unsigned curr_frame_num,
 ...
 ************************************************************************
 */
static void mark_pic_long_term(DecodedPictureBuffer *p_Dpb, StorablePicture *p,
  ...
  if (p->structure == FRAME) {
    for (i = 0; i < p_Dpb->ref_frames_in_buffer; i++) {
      ...
      if (p_Dpb->fs_ref[i]->view_id == p->view_id) {
        ...
        if (p_Dpb->fs_ref[i]->is_reference == 3) {
          if ((!p_Dpb->fs_ref[i]->frame->is_long_term) &&
              (p_Dpb->fs_ref[i]->frame->pic_num == picNumX)) {
            p_Dpb->fs_ref[i]->long_term_frame_idx =
                p_Dpb->fs_ref[i]->frame->long_term_frame_idx =
                    long_term_frame_idx;
            ...
            if (p_Dpb->fs_ref[i]->top_field && p_Dpb->fs_ref[i]->bottom_field) {
              p_Dpb->fs_ref[i]->top_field->long_term_frame_idx =
                  p_Dpb->fs_ref[i]->bottom_field->long_term_frame_idx =
                      long_term_frame_idx;
              p_Dpb->fs_ref[i]->top_field->long_term_pic_num =
                  long_term_frame_idx;
              p_Dpb->fs_ref[i]->bottom_field->long_term_pic_num =
                  long_term_frame_idx;
    ...
    for (i = 0; i < p_Dpb->ref_frames_in_buffer; i++) {
      ...
      if (p_Dpb->fs_ref[i]->view_id == p->view_id) {
        ...
        if (p_Dpb->fs_ref[i]->is_reference & 1) {
          if ((!p_Dpb->fs_ref[i]->top_field->is_long_term) &&
            ...
            if ((p_Dpb->fs_ref[i]->is_long_term) &&
                (p_Dpb->fs_ref[i]->long_term_frame_idx !=
                 long_term_frame_idx)) {
              printf("Warning: assigning long_term_frame_idx different from "
                     "other field\n");
            }
        ...
        if (p_Dpb->fs_ref[i]->is_reference & 2) {
          if ((!p_Dpb->fs_ref[i]->bottom_field->is_long_term) &&
              (p_Dpb->fs_ref[i]->bottom_field->pic_num == picNumX)) {
            if ((p_Dpb->fs_ref[i]->is_long_term) &&
                (p_Dpb->fs_ref[i]->long_term_frame_idx !=
                 long_term_frame_idx)) {
              printf("Warning: assigning long_term_frame_idx different from "
            ...
            p_Dpb->fs_ref[i]->long_term_frame_idx =
                p_Dpb->fs_ref[i]->bottom_field->long_term_frame_idx =
                    long_term_frame_idx;
            p_Dpb->fs_ref[i]->bottom_field->long_term_pic_num =
                2 * long_term_frame_idx + add_bottom;
            p_Dpb->fs_ref[i]->bottom_field->is_long_term = 1;
            p_Dpb->fs_ref[i]->is_long_term |= 2;
            if (p_Dpb->fs_ref[i]->is_long_term == 3) {
              p_Dpb->fs_ref[i]->frame->is_long_term = 1;
 ...
 ************************************************************************
 */
static void mm_assign_long_term_frame_idx(DecodedPictureBuffer *p_Dpb,
                                          StorablePicture *p,
                                          int difference_of_pic_nums_minus1,
                                          int long_term_frame_idx) {
  ...
  if (p->structure == FRAME) {
    ...
    unmark_long_term_frame_for_reference_by_frame_idx(
        p_Dpb, long_term_frame_idx, p->view_id);
#else
    ...
    unmark_long_term_field_for_reference_by_frame_idx(
        p_Dpb, structure, long_term_frame_idx, 0, 0, picNumX, p->view_id);
#else
    unmark_long_term_field_for_reference_by_frame_idx(
        p_Dpb, structure, long_term_frame_idx, 0, 0, picNumX);
#endif
  }
...
#endif
{
  ...
  for (i = 0; i < p_Dpb->ltref_frames_in_buffer; i++) {
    ...
    if (p_Dpb->fs_ltref[i]->long_term_frame_idx >
            p_Dpb->max_long_term_pic_idx[iVOIdx] &&
        p_Dpb->fs_ltref[i]->view_id == curr_view_id)
#else
...
#else
static void
mm_unmark_all_short_term_for_reference(DecodedPictureBuffer *p_Dpb) {
  unsigned int i;
  for (i = 0; i < p_Dpb->ref_frames_in_buffer; i++) {
    unmark_for_reference(p_Dpb->fs_ref[i]);
  }
 ...
 ************************************************************************
 */
static void mm_mark_current_picture_long_term(DecodedPictureBuffer *p_Dpb,
                                              StorablePicture *p,
                                              int long_term_frame_idx) {
  ...
  if (p->structure == FRAME) {
    ...
    unmark_long_term_field_for_reference_by_frame_idx(
        p_Dpb, p->structure, long_term_frame_idx, 1, p->pic_num, 0, p->view_id);
  }
#else
  if (p->structure == FRAME) {
    unmark_long_term_frame_for_reference_by_frame_idx(p_Dpb,
    ...
    unmark_long_term_field_for_reference_by_frame_idx(
        p_Dpb, p->structure, long_term_frame_idx, 1, p->pic_num, 0);
  }
#endif
  ...
  while (p->dec_ref_pic_marking_buffer) {
    ...
    case 0:
      if (tmp_drpm->Next != NULL) {
        error("memory_management_control_operation = 0 not last operation in "
              "buffer",
              500);
      }
      break;
    case 1:
      mm_unmark_short_term_for_reference(
          p_Dpb, p, tmp_drpm->difference_of_pic_nums_minus1);
#if (MVC_EXTENSION_ENABLE)
      update_ref_list(p_Dpb, p->view_id);
#else
      update_ref_list(p_Dpb);
    ...
    case 4:
      ...
      mm_update_max_long_term_frame_idx(
          p_Dpb, tmp_drpm->max_long_term_frame_idx_plus1, p->view_id);
      update_ltref_list(p_Dpb, p->view_id);
#else
      mm_update_max_long_term_frame_idx(
          p_Dpb, tmp_drpm->max_long_term_frame_idx_plus1);
      update_ltref_list(p_Dpb);
#endif
      break;
    case 5:
...
void store_picture_in_dpb(DecodedPictureBuffer *p_Dpb, StorablePicture *p) {
  ...
  // FRAME)?"FRAME":(p->type == TOP_FIELD)?"TOP_FIELD":"BOTTOM_FIELD",
  // p->pic_num);
  ...
  if ((p->structure == TOP_FIELD) || (p->structure == BOTTOM_FIELD)) {
    ...
    if (!(p_Dpb->last_picture && p_Dpb->last_picture->view_id == p->view_id)) {
      for (i = 0; i < p_Dpb->used_size; i++) {
        if (p_Dpb->fs[i]->view_id == p->view_id &&
            ((p->structure == TOP_FIELD && p_Dpb->fs[i]->is_used == 2) ||
             (p->structure == BOTTOM_FIELD && p_Dpb->fs[i]->is_used == 1)))
          break;
      }
      ...
      if ((int)p_Dpb->last_picture->frame_num == p->pic_num) {
        if (((p->structure == TOP_FIELD) &&
          ...
          if ((p->used_for_reference &&
               (p_Dpb->last_picture->is_orig_reference != 0)) ||
  ...
  if ((!p->idr_flag) &&
      (p->used_for_reference && (!p->adaptive_ref_pic_buffering_flag))) {
  ...
  if ((p->used_for_reference) && (!p->is_long_term)) {
    for (i = 0; i < p_Dpb->ref_frames_in_buffer; i++) {
      ...
      if (p_Dpb->fs_ref[i]->frame_num == p->frame_num &&
          p_Dpb->fs_ref[i]->view_id == p->view_id)
#else
      if (p_Dpb->fs_ref[i]->frame_num == p->frame_num)
#endif
      {
        error("duplicate frame_num in short-term reference picture buffer",
              500);
      }
    }
  }
...
static void insert_picture_in_dpb(VideoParameters *p_Vid, FrameStore *fs,
  ...
  //  FRAME)?"FRAME":(p->structure == TOP_FIELD)?"TOP_FIELD":"BOTTOM_FIELD",
  //  p->pic_num, p->poc);
  assert(p != NULL);
  assert(fs != NULL);
  switch (p->structure) {
 ...
 ************************************************************************
 * \brief
 *    Check if one of the frames/fields in frame store is used for short-term
 *reference
 ************************************************************************
 */
static int is_short_term_reference(FrameStore *fs) {
  ...
  *poc = INT_MAX;
  for (i = 0; i < p_Dpb->used_size; i++) {
    ...
    if ((*poc > p_Dpb->fs[i]->poc) && (!p_Dpb->fs[i]->is_output) &&
        (p_Dpb->fs[i]->view_id == curr_view_id || curr_view_id == -1))
#else
    if ((*poc > p_Dpb->fs[i]->poc) && (!p_Dpb->fs[i]->is_output))
#endif
    {
...
#endif
{
  ...
  //  printf ("output frame with frame_num #%d, poc %d (dpb. p_Dpb->size=%d,
  //  p_Dpb->used_size=%d)\n", p_Dpb->fs[pos]->frame_num,
  //  p_Dpb->fs[pos]->frame->poc, p_Dpb->size, p_Dpb->used_size);
  ...
  if (p_Vid->conceal_mode == 0)
    if (p_Dpb->last_output_poc >= poc &&
        p_Dpb->fs[pos]->view_id == p_Dpb->last_output_view_id) {
...
#endif
{
  ...
  // diagnostics
  // printf("Flush remaining frames from the dpb. p_Dpb->size=%d,
...
void dpb_split_field(VideoParameters *p_Vid, FrameStore *fs) {
  ...
  if (!frame->frame_mbs_only_flag) {
    fs_top = fs->top_field =
        alloc_storable_picture(p_Vid, TOP_FIELD, frame->size_x, frame->size_y,
                               frame->size_x_cr, frame->size_y_cr);
    fs_btm = fs->bottom_field = alloc_storable_picture(
        p_Vid, BOTTOM_FIELD, frame->size_x, frame->size_y, frame->size_x_cr,
        frame->size_y_cr);

    for (i = 0; i < (frame->size_y >> 1); i++) {
      memcpy(fs_top->imgY[i], frame->imgY[i * 2],
             frame->size_x * sizeof(imgpel));
    }

    for (i = 0; i < (frame->size_y_cr >> 1); i++) {
      memcpy(fs_top->imgUV[0][i], frame->imgUV[0][i * 2],
             frame->size_x_cr * sizeof(imgpel));
      memcpy(fs_top->imgUV[1][i], frame->imgUV[1][i * 2],
             frame->size_x_cr * sizeof(imgpel));
    }

    for (i = 0; i < (frame->size_y >> 1); i++) {
      memcpy(fs_btm->imgY[i], frame->imgY[i * 2 + 1],
             frame->size_x * sizeof(imgpel));
    }

    for (i = 0; i < (frame->size_y_cr >> 1); i++) {
      memcpy(fs_btm->imgUV[0][i], frame->imgUV[0][i * 2 + 1],
             frame->size_x_cr * sizeof(imgpel));
      memcpy(fs_btm->imgUV[1][i], frame->imgUV[1][i * 2 + 1],
             frame->size_x_cr * sizeof(imgpel));
    }
      ...
      for (j = 0; j < (frame->size_y >> 3); j++) {
        ...
        for (i = 0; i < (frame->size_x >> 2); i++) {
          ...
          if (frm_motion->mb_field[currentmb]) {
            // fs_btm->mv_info[j][i].field_frame  =
            // fs_top->mv_info[j][i].field_frame = 1;
            // frame->mv_info[2*j][i].field_frame =
            // frame->mv_info[2*j+1][i].field_frame = 1;

            fs_btm->mv_info[j][i].mv[LIST_0] =
                frame->mv_info[jj4][i].mv[LIST_0];
            fs_btm->mv_info[j][i].mv[LIST_1] =
                frame->mv_info[jj4][i].mv[LIST_1];
            fs_btm->mv_info[j][i].ref_idx[LIST_0] =
                frame->mv_info[jj4][i].ref_idx[LIST_0];
            if (fs_btm->mv_info[j][i].ref_idx[LIST_0] >= 0)
              fs_btm->mv_info[j][i].ref_pic[LIST_0] =
                  p_Vid->ppSliceList[0]
                      ->listX[4][(short)fs_btm->mv_info[j][i].ref_idx[LIST_0]];
            else
              fs_btm->mv_info[j][i].ref_pic[LIST_0] = NULL;
            fs_btm->mv_info[j][i].ref_idx[LIST_1] =
                frame->mv_info[jj4][i].ref_idx[LIST_1];
            if (fs_btm->mv_info[j][i].ref_idx[LIST_1] >= 0)
              fs_btm->mv_info[j][i].ref_pic[LIST_1] =
                  p_Vid->ppSliceList[0]
                      ->listX[5][(short)fs_btm->mv_info[j][i].ref_idx[LIST_1]];
            else
            ...
            fs_top->mv_info[j][i].ref_idx[LIST_0] =
                frame->mv_info[jj][i].ref_idx[LIST_0];
            if (fs_top->mv_info[j][i].ref_idx[LIST_0] >= 0)
              fs_top->mv_info[j][i].ref_pic[LIST_0] =
                  p_Vid->ppSliceList[0]
                      ->listX[2][(short)fs_top->mv_info[j][i].ref_idx[LIST_0]];
            else
              fs_top->mv_info[j][i].ref_pic[LIST_0] = NULL;
            fs_top->mv_info[j][i].ref_idx[LIST_1] =
                frame->mv_info[jj][i].ref_idx[LIST_1];
            if (fs_top->mv_info[j][i].ref_idx[LIST_1] >= 0)
              fs_top->mv_info[j][i].ref_pic[LIST_1] =
                  p_Vid->ppSliceList[0]
                      ->listX[3][(short)fs_top->mv_info[j][i].ref_idx[LIST_1]];
            else
    ...
    for (j = 0; j < (frame->size_y >> 3); j++) {
      ...
      for (i = 0; i < (frame->size_x >> 2); i++) {
        ...
        if (!frame->mb_aff_frame_flag || !frame->motion.mb_field[currentmb]) {
          // frame->mv_info[2*j+1][i].field_frame =
          // frame->mv_info[2*j][i].field_frame = 0;

          // fs_top->mv_info[j][i].field_frame =
          // fs_btm->mv_info[j][i].field_frame = 0;

          fs_top->mv_info[j][i].mv[LIST_0] = fs_btm->mv_info[j][i].mv[LIST_0] =
              frame->mv_info[jj][ii].mv[LIST_0];
          fs_top->mv_info[j][i].mv[LIST_1] = fs_btm->mv_info[j][i].mv[LIST_1] =
              frame->mv_info[jj][ii].mv[LIST_1];

          // Scaling of references is done here since it will not affect spatial
          // direct (2*0 =0)
          if (frame->mv_info[jj][ii].ref_idx[LIST_0] == -1)
            fs_top->mv_info[j][i].ref_idx[LIST_0] =
                fs_btm->mv_info[j][i].ref_idx[LIST_0] = -1;
          else {
            fs_top->mv_info[j][i].ref_idx[LIST_0] =
                fs_btm->mv_info[j][i].ref_idx[LIST_0] =
                    frame->mv_info[jj][ii].ref_idx[LIST_0];
            fs_top->mv_info[j][i].ref_pic[LIST_0] =
                fs_btm->mv_info[j][i].ref_pic[LIST_0] =
                    p_Vid->ppSliceList[0]
                        ->listX[LIST_0]
                               [(short)frame->mv_info[jj][ii].ref_idx[LIST_0]];
          }

          if (frame->mv_info[jj][ii].ref_idx[LIST_1] == -1)
            fs_top->mv_info[j][i].ref_idx[LIST_1] =
                fs_btm->mv_info[j][i].ref_idx[LIST_1] = -1;
          else {
            fs_top->mv_info[j][i].ref_idx[LIST_1] =
                fs_btm->mv_info[j][i].ref_idx[LIST_1] =
                    frame->mv_info[jj][ii].ref_idx[LIST_1];
            fs_top->mv_info[j][i].ref_pic[LIST_1] =
                fs_btm->mv_info[j][i].ref_pic[LIST_1] =
                    p_Vid->ppSliceList[0]
                        ->listX[LIST_1]
                               [(short)frame->mv_info[jj][ii].ref_idx[LIST_1]];
          }
        } else {
          // frame->mv_info[2*j+1][i].field_frame =
          // frame->mv_info[2*j][i].field_frame =
          // frame->motion.mb_field[currentmb];
        }
      }
    }
...
void dpb_combine_field_yuv(VideoParameters *p_Vid, FrameStore *fs) {
  ...
  if (!fs->frame) {
    fs->frame = alloc_storable_picture(
        p_Vid, FRAME, fs->top_field->size_x, fs->top_field->size_y * 2,
        fs->top_field->size_x_cr, fs->top_field->size_y_cr * 2);
  }

  for (i = 0; i < fs->top_field->size_y; i++) {
    memcpy(fs->frame->imgY[i * 2], fs->top_field->imgY[i],
           fs->top_field->size_x * sizeof(imgpel)); // top field
    memcpy(fs->frame->imgY[i * 2 + 1], fs->bottom_field->imgY[i],
           fs->bottom_field->size_x * sizeof(imgpel)); // bottom field
  }

  for (j = 0; j < 2; j++) {
    for (i = 0; i < fs->top_field->size_y_cr; i++) {
      memcpy(fs->frame->imgUV[j][i * 2], fs->top_field->imgUV[j][i],
             fs->top_field->size_x_cr * sizeof(imgpel));
      memcpy(fs->frame->imgUV[j][i * 2 + 1], fs->bottom_field->imgUV[j][i],
             fs->bottom_field->size_x_cr * sizeof(imgpel));
    }
  }

  fs->poc = fs->frame->poc = fs->frame->frame_poc =
      imin(fs->top_field->poc, fs->bottom_field->poc);
  ...
  if (fs->frame->frame_cropping_flag) {
    fs->frame->frame_cropping_rect_top_offset =
        fs->top_field->frame_cropping_rect_top_offset;
    fs->frame->frame_cropping_rect_bottom_offset =
        fs->top_field->frame_cropping_rect_bottom_offset;
    fs->frame->frame_cropping_rect_left_offset =
        fs->top_field->frame_cropping_rect_left_offset;
    fs->frame->frame_cropping_rect_right_offset =
        fs->top_field->frame_cropping_rect_right_offset;
  }
...
void dpb_combine_field(VideoParameters *p_Vid, FrameStore *fs) {
...
#endif
  fs->frame->iCodingType =
      fs->top_field->iCodingType; // FIELD_CODING;
  ...
  for (j = 0; j < (fs->top_field->size_y >> 2); j++) {
    ...
    for (i = 0; i < (fs->top_field->size_x >> 2); i++) {
      // fs->frame->mv_info[jj][i].field_frame=
      // fs->frame->mv_info[jj4][i].field_frame = 1;

      fs->frame->mv_info[jj][i].mv[LIST_0] =
          fs->top_field->mv_info[j][i].mv[LIST_0];
      fs->frame->mv_info[jj][i].mv[LIST_1] =
          fs->top_field->mv_info[j][i].mv[LIST_1];

      fs->frame->mv_info[jj][i].ref_idx[LIST_0] =
          fs->top_field->mv_info[j][i].ref_idx[LIST_0];
      fs->frame->mv_info[jj][i].ref_idx[LIST_1] =
          fs->top_field->mv_info[j][i].ref_idx[LIST_1];
      ...
      if (fs->top_field->listXsize[LIST_0] > 0) {
        ...
        fs->frame->mv_info[jj][i].ref_pic[LIST_0] =
            k >= 0 ? fs->top_field->listX[LIST_0][k] : NULL;
        k = fs->top_field->mv_info[j][i].ref_idx[LIST_1];
        assert(k < fs->top_field->listXsize[LIST_1]);
        fs->frame->mv_info[jj][i].ref_pic[LIST_1] =
            k >= 0 ? fs->top_field->listX[LIST_1][k] : NULL;
      } else {
        k = fs->top_field->mv_info[j][i].ref_idx[LIST_0];
        assert(k < imin(p_Vid->ppSliceList[0]->num_ref_idx_active[LIST_0],
                        p_Vid->ppSliceList[0]->listXsize[LIST_0]));
        fs->frame->mv_info[jj][i].ref_pic[LIST_0] =
            k >= 0 ? p_Vid->ppSliceList[0]->listX[LIST_0][k] : NULL;
        k = fs->top_field->mv_info[j][i].ref_idx[LIST_1];
        assert(k < imin(p_Vid->ppSliceList[0]->num_ref_idx_active[LIST_1],
                        p_Vid->ppSliceList[0]->listXsize[LIST_1]));
        fs->frame->mv_info[jj][i].ref_pic[LIST_1] =
            k >= 0 ? p_Vid->ppSliceList[0]->listX[LIST_1][k] : NULL;
      }

      //! association with id already known for fields.
      fs->frame->mv_info[jj4][i].mv[LIST_0] =
          fs->bottom_field->mv_info[j][i].mv[LIST_0];
      fs->frame->mv_info[jj4][i].mv[LIST_1] =
          fs->bottom_field->mv_info[j][i].mv[LIST_1];

      fs->frame->mv_info[jj4][i].ref_idx[LIST_0] =
          fs->bottom_field->mv_info[j][i].ref_idx[LIST_0];
      fs->frame->mv_info[jj4][i].ref_idx[LIST_1] =
          fs->bottom_field->mv_info[j][i].ref_idx[LIST_1];
      ...
      if (fs->bottom_field->listXsize[LIST_0] > 0) {
        ...
        fs->frame->mv_info[jj4][i].ref_pic[LIST_0] =
            k >= 0 ? fs->bottom_field->listX[LIST_0][k] : NULL;
        k = fs->bottom_field->mv_info[j][i].ref_idx[LIST_1];
        assert(k < fs->bottom_field->listXsize[LIST_1]);
        fs->frame->mv_info[jj4][i].ref_pic[LIST_1] =
            k >= 0 ? fs->bottom_field->listX[LIST_1][k] : NULL;
      } else {
        k = fs->bottom_field->mv_info[j][i].ref_idx[LIST_0];
        assert(k < imin(p_Vid->ppSliceList[0]->num_ref_idx_active[LIST_0],
                        p_Vid->ppSliceList[0]->listXsize[LIST_0]));
...
void alloc_ref_pic_list_reordering_buffer(Slice *currSlice) {
  ...
  if (p_Vid->type != I_SLICE && p_Vid->type != SI_SLICE) {
    if ((currSlice->reordering_of_pic_nums_idc[LIST_0] =
             calloc(size, sizeof(int))) == NULL)
      no_mem_exit("alloc_ref_pic_list_reordering_buffer: "
                  "reordering_of_pic_nums_idc_l0");
    if ((currSlice->abs_diff_pic_num_minus1[LIST_0] =
             calloc(size, sizeof(int))) == NULL)
      no_mem_exit(
          "alloc_ref_pic_list_reordering_buffer: abs_diff_pic_num_minus1_l0");
    if ((currSlice->long_term_pic_idx[LIST_0] = calloc(size, sizeof(int))) ==
        NULL)
    ...
    if ((currSlice->abs_diff_view_idx_minus1[LIST_0] =
             calloc(size, sizeof(int))) == NULL)
      no_mem_exit(
          "alloc_ref_pic_list_reordering_buffer: abs_diff_view_idx_minus1_l0");
#endif
  } else {
  ...
  if (p_Vid->type == B_SLICE) {
    if ((currSlice->reordering_of_pic_nums_idc[LIST_1] =
             calloc(size, sizeof(int))) == NULL)
      no_mem_exit("alloc_ref_pic_list_reordering_buffer: "
                  "reordering_of_pic_nums_idc_l1");
    if ((currSlice->abs_diff_pic_num_minus1[LIST_1] =
             calloc(size, sizeof(int))) == NULL)
      no_mem_exit(
          "alloc_ref_pic_list_reordering_buffer: abs_diff_pic_num_minus1_l1");
    if ((currSlice->long_term_pic_idx[LIST_1] = calloc(size, sizeof(int))) ==
        NULL)
    ...
    if ((currSlice->abs_diff_view_idx_minus1[LIST_1] =
             calloc(size, sizeof(int))) == NULL)
      no_mem_exit(
          "alloc_ref_pic_list_reordering_buffer: abs_diff_view_idx_minus1_l1");
#endif
  } else {
...
void fill_frame_num_gap(VideoParameters *p_Vid, Slice *currSlice) {
  ...
  while (CurrFrameNum != UnusedShortTermFrameNum) {
    picture = alloc_storable_picture(p_Vid, FRAME, p_Vid->width, p_Vid->height,
                                     p_Vid->width_cr, p_Vid->height_cr);
    ...
    UnusedShortTermFrameNum =
        (UnusedShortTermFrameNum + 1) % p_Vid->MaxFrameNum;
  }
  ...
  if (currSlice->direct_spatial_mv_pred_flag == 0) {
    for (j = 0; j < 2 + (currSlice->mb_aff_frame_flag * 4); j += 2) {
      for (i = 0; i < currSlice->listXsize[j]; i++) {
        ...
        if (j == 0) {
          iTRb = iClip3(-128, 127,
                        p_Vid->dec_picture->poc - listX[LIST_0 + j][i]->poc);
        } else if (j == 2) {
          iTRb =
              iClip3(-128, 127,
                     p_Vid->dec_picture->top_poc - listX[LIST_0 + j][i]->poc);
        } else {
          iTRb = iClip3(-128, 127,
                        p_Vid->dec_picture->bottom_poc -
                            listX[LIST_0 + j][i]->poc);
        }

        iTRp = iClip3(-128, 127,
                      listX[LIST_1 + j][0]->poc - listX[LIST_0 + j][i]->poc);

        if (iTRp != 0) {
          prescale = (16384 + iabs(iTRp / 2)) / iTRp;
          currSlice->mvscale[j][i] =
              iClip3(-1024, 1023, (iTRb * prescale + 32) >> 6);
        } else {
...
int GetMaxDecFrameBuffering(VideoParameters *p_Vid) {
  ...
  for (i = 0; i < MAXSPS; i++) {
    if (curr_subset_sps->Valid &&
        curr_subset_sps->sps.seq_parameter_set_id < MAXSPS) {
      j = curr_subset_sps->sps.max_dec_frame_buffering;

      if (curr_subset_sps->sps.vui_parameters_present_flag &&
          curr_subset_sps->sps.vui_seq_parameters.bitstream_restriction_flag) {
        if ((int)curr_subset_sps->sps.vui_seq_parameters
                .max_dec_frame_buffering > j) {
          error("max_dec_frame_buffering larger than MaxDpbSize", 500);
        }
        j = imax(
            1, curr_subset_sps->sps.vui_seq_parameters.max_dec_frame_buffering);
      }
    ...
    if (curr_sps->Valid) {
      ...
      if (curr_sps->vui_parameters_present_flag &&
          curr_sps->vui_seq_parameters.bitstream_restriction_flag) {
...
static int is_view_id_in_ref_view_list(int view_id, int *ref_view_id,
                                       int num_ref_views) {
  int i;
...
void append_interview_list(
    DecodedPictureBuffer *p_Dpb,
    PictureStructure
        currPicStructure, // 0: frame; 1:top field; 2: bottom field;
  ...
  if (anchor_pic_flag) {
    num_ref_views = list_idx
                        ? p_Vid->active_subset_sps->num_anchor_refs_l1[iVOIdx]
                        : p_Vid->active_subset_sps->num_anchor_refs_l0[iVOIdx];
    ref_view_id = list_idx ? p_Vid->active_subset_sps->anchor_ref_l1[iVOIdx]
                           : p_Vid->active_subset_sps->anchor_ref_l0[iVOIdx];
  } else {
    num_ref_views =
        list_idx ? p_Vid->active_subset_sps->num_non_anchor_refs_l1[iVOIdx]
                 : p_Vid->active_subset_sps->num_non_anchor_refs_l0[iVOIdx];
    ref_view_id = list_idx
                      ? p_Vid->active_subset_sps->non_anchor_ref_l1[iVOIdx]
                      : p_Vid->active_subset_sps->non_anchor_ref_l0[iVOIdx];
  }
  ...
  for (i = 0; i < p_Dpb->used_size; i++) {
    ...
    if (pic_avail && p_Dpb->fs[i]->inter_view_flag[fld_idx]) {
      if (poc == currPOC) {
        if (is_view_id_in_ref_view_list(p_Dpb->fs[i]->view_id, ref_view_id,
                                        num_ref_views)) {
