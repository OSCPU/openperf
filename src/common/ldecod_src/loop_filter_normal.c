 ...
 *    - Jani Lainema     Jani.Lainema@nokia.com:   Some bug fixing, removal of
 *recursiveness     (16-Aug-2001)
 *    - Peter List       Peter.List@t-systems.de:  inplace filtering and various
 *simplifications (10-Jan-2002)
 *    - Anthony Joch     anthony@ubvideo.com:      Simplified switching between
 *filters and non-recursive default filter.                 (08-Jul-2002)
 *    - Cristina Gomila  cristina.gomila@thomson.net: Simplification of the
 *chroma deblocking from JVT-E089                              (21-Nov-2002)
 *    - Alexis Michael Tourapis atour@dolby.com:   Speed/Architecture
 *improvements               (08-Feb-2007)
 *************************************************************************************
 */
...
static void GetStrengthVer(byte Strength[MB_BLOCK_SIZE], Macroblock *MbQ,
                           int edge, int mvlimit, StorablePicture *p);
static void GetStrengthHor(byte Strength[MB_BLOCK_SIZE], Macroblock *MbQ,
                           int edge, int mvlimit, StorablePicture *p);
static void EdgeLoopLumaVer(ColorPlane pl, imgpel **Img,
                            byte Strength[MB_BLOCK_SIZE], Macroblock *MbQ,
                            int edge, StorablePicture *p);
static void EdgeLoopLumaHor(ColorPlane pl, imgpel **Img,
                            byte Strength[MB_BLOCK_SIZE], Macroblock *MbQ,
                            int edge, StorablePicture *p);
static void EdgeLoopChromaVer(imgpel **Img, byte Strength[MB_BLOCK_SIZE],
                              Macroblock *MbQ, int edge, int uv,
                              StorablePicture *p);
static void EdgeLoopChromaHor(imgpel **Img, byte Strength[MB_BLOCK_SIZE],
                              Macroblock *MbQ, int edge, int uv,
                              StorablePicture *p);
 ...
 *********************************************************************************************
 * \brief
 *    returns a buffer of 16 Strength values for one stripe in a mb (for
 *different Frame or Field types)
 *********************************************************************************************
 */
static void GetStrengthVer(byte Strength[MB_BLOCK_SIZE], Macroblock *MbQ,
                           int edge, int mvlimit, StorablePicture *p) {
  ...
  if ((currSlice->slice_type == SP_SLICE) ||
    ...
    StrValue =
        (edge == 0 && (((p->structure == FRAME)) || ((p->structure != FRAME))))
            ? 4
            : 3;
    ...
    if (MbQ->is_intra_block == FALSE) {
      ...
      if (edge || MbP->is_intra_block == FALSE) {
        ...
        for (idx = 0; idx < MB_BLOCK_SIZE; idx += BLOCK_SIZE) {
          ...
          if (((MbQ->cbp_blk[0] & i64_power2(blkQ)) != 0) ||
              ((MbP->cbp_blk[0] & i64_power2(blkP)) != 0))
            StrValue = 2;
          else if (edge && ((MbQ->mb_type == 1) || (MbQ->mb_type == 2)))
            StrValue = 0; // if internal edge of certain types, we already know
                          // StrValue should be 0
          else // for everything else, if no coefs, but vector difference >= 1
               // set Strength=1
          {
            ...
            if (((ref_p0 == ref_q0) && (ref_p1 == ref_q1)) ||
              ...
              if (ref_p0 != ref_p1) {
                ...
                if (ref_p0 == ref_q0) {
                  StrValue = compare_mvs(&mv_info_p->mv[LIST_0],
                                         &mv_info_q->mv[LIST_0], mvlimit) |
                             compare_mvs(&mv_info_p->mv[LIST_1],
                                         &mv_info_q->mv[LIST_1], mvlimit);
                } else {
                  StrValue = compare_mvs(&mv_info_p->mv[LIST_0],
                                         &mv_info_q->mv[LIST_1], mvlimit) |
                             compare_mvs(&mv_info_p->mv[LIST_1],
              ...
              } else { // L0 and L1 reference pictures of p0 are the same; q0 as
                       // well
                StrValue = ((compare_mvs(&mv_info_p->mv[LIST_0],
                                         &mv_info_q->mv[LIST_0], mvlimit) |
                             compare_mvs(&mv_info_p->mv[LIST_1],
                                         &mv_info_q->mv[LIST_1], mvlimit)) &&
                            (compare_mvs(&mv_info_p->mv[LIST_0],
                                         &mv_info_q->mv[LIST_1], mvlimit) |
                             compare_mvs(&mv_info_p->mv[LIST_1],
                                         &mv_info_q->mv[LIST_0], mvlimit)));
              }
          ...
          //*(int*)(Strength+idx) =
          //(StrValue<<24)|(StrValue<<16)|(StrValue<<8)|StrValue;
          *(int *)(Strength + idx) = StrValue * 0x01010101;
        }
      } else {
        // Start with Strength=3. or Strength=4 for Mb-edge
        StrValue = (edge == 0 &&
                    ((((p->structure == FRAME))) || ((p->structure != FRAME))))
                       ? 4
                       : 3;
        memset(Strength, (byte)StrValue, MB_BLOCK_SIZE * sizeof(byte));
      }
 ...
 *****************************************************************************************
 */
static void EdgeLoopLumaVer(ColorPlane pl, imgpel **Img, byte Strength[16],
                            Macroblock *MbQ, int edge, StorablePicture *p) {
  ...
  if (MbP || (MbQ->DFDisableIdc == 0)) {
    int bitdepth_scale =
        pl ? p_Vid->bitdepth_scale[IS_CHROMA] : p_Vid->bitdepth_scale[IS_LUMA];
              ...
              if ((iabs(R0 - R1) < Beta) && (iabs(L0 - L1) < Beta)) {
                if ((iabs(R0 - L0) < ((Alpha >> 2) + 2))) {
                  ...
                  if ((iabs(L0 - L2) < Beta)) {
                    imgpel L3 = *(SrcPtrP - 3);
                    *(SrcPtrP--) =
                        (imgpel)((R1 + ((L1 + RL0) << 1) + L2 + 4) >> 3);
                    *(SrcPtrP--) = (imgpel)((L2 + L1 + RL0 + 2) >> 2);
                    *(SrcPtrP) =
                        (imgpel)((((L3 + L2) << 1) + L2 + L1 + RL0 + 4) >> 3);
                  } else {
                  ...
                  if ((iabs(R0 - R2) < Beta)) {
                    imgpel R3 = *(SrcPtrQ + 3);
                    *(SrcPtrQ++) =
                        (imgpel)((L1 + ((R1 + RL0) << 1) + R2 + 4) >> 3);
                    *(SrcPtrQ++) = (imgpel)((R2 + R0 + L0 + R1 + 2) >> 2);
                    *(SrcPtrQ) =
                        (imgpel)((((R3 + R2) << 1) + R2 + R1 + RL0 + 4) >> 3);
                  } else {
          ...
          if (C0 == 0) {
            for (i = 0; i < BLOCK_SIZE; ++i) {
              ...
              if (iabs(edge_diff) < Alpha) {
                ...
                if ((iabs(*SrcPtrQ - *SrcPtrQ1) < Beta) &&
                    (iabs(*SrcPtrP - *SrcPtrP1) < Beta)) {
            ...
            for (i = 0; i < BLOCK_SIZE; ++i) {
              ...
              if (iabs(edge_diff) < Alpha) {
                ...
                if ((iabs(*SrcPtrQ - *SrcPtrQ1) < Beta) &&
                  ...
                  if (ap)
                    *SrcPtrP1 +=
                        iClip3(-C0, C0, (L2 + RL0 - (*SrcPtrP1 << 1)) >> 1);
                  ...
                  if (aq)
                    *SrcPtrQ1 +=
                        iClip3(-C0, C0, (R2 + RL0 - (*SrcPtrQ1 << 1)) >> 1);
                }
              }
            }
          }
 ...
 *****************************************************************************************
 */
static void EdgeLoopLumaHor(ColorPlane pl, imgpel **Img, byte Strength[16],
                            Macroblock *MbQ, int edge, StorablePicture *p) {
  VideoParameters *p_Vid = MbQ->p_Vid;
 ...
 *****************************************************************************************
 */
static void EdgeLoopChromaVer(imgpel **Img, byte Strength[16], Macroblock *MbQ,
                              int edge, int uv, StorablePicture *p) {
  VideoParameters *p_Vid = MbQ->p_Vid;
  ...
  Macroblock *MbP =
      get_non_aff_neighbor_chroma(MbQ, xQ, yQ, block_width, block_height);
  ...
  if (MbP || (MbQ->DFDisableIdc == 0)) {
    ...
    if ((Alpha | Beta) != 0) {
      ...
      for (pel = 0; pel < PelNum; ++pel) {
        int Strng =
            Strength[(PelNum == 8) ? (((pel >> 1) << 2) + (pel & 0x01)) : pel];
              ...
              if (iabs(*SrcPtrP - L1) < Beta) {
                if (Strng == 4) // INTRA strong filtering
                {
                  ...
                  *SrcPtrQ = (imgpel)(((R1 << 1) + *SrcPtrQ + L1 + 2) >> 2);
                  ...
                  int dif = iClip3(-tc0, tc0,
                                   (((edge_diff) << 2) + (L1 - R1) + 4) >> 3);
 ...
 *****************************************************************************************
 */
static void EdgeLoopChromaHor(imgpel **Img, byte Strength[16], Macroblock *MbQ,
                              int edge, int uv, StorablePicture *p) {
  VideoParameters *p_Vid = MbQ->p_Vid;
  int block_width = p_Vid->mb_cr_size_x;
  int block_height = p_Vid->mb_cr_size_y;
  ...
  if (MbP || (MbQ->DFDisableIdc == 0)) {
    ...
    if ((Alpha | Beta) != 0) {
      ...
      imgpel *imgP = &Img[get_pos_y_chroma(MbP, yQ, (block_height - 1))]
                         [get_pos_x_chroma(MbP, xQ, (block_width - 1))];
      imgpel *imgQ = imgP + width;
