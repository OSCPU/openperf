...
static void GetStrengthVerMBAff(byte Strength[MB_BLOCK_SIZE], Macroblock *MbQ,
                                int edge, int mvlimit, StorablePicture *p);
static void GetStrengthHorMBAff(byte Strength[MB_BLOCK_SIZE], Macroblock *MbQ,
                                int edge, int mvlimit, StorablePicture *p);
static void EdgeLoopLumaVerMBAff(ColorPlane pl, imgpel **Img,
                                 byte Strength[MB_BLOCK_SIZE], Macroblock *MbQ,
                                 int edge, StorablePicture *p);
static void EdgeLoopLumaHorMBAff(ColorPlane pl, imgpel **Img,
                                 byte Strength[MB_BLOCK_SIZE], Macroblock *MbQ,
                                 int edge, StorablePicture *p);
static void EdgeLoopChromaVerMBAff(imgpel **Img, byte Strength[MB_BLOCK_SIZE],
                                   Macroblock *MbQ, int edge, int uv,
                                   StorablePicture *p);
static void EdgeLoopChromaHorMBAff(imgpel **Img, byte Strength[MB_BLOCK_SIZE],
                                   Macroblock *MbQ, int edge, int uv,
                                   StorablePicture *p);
...
Macroblock *get_non_aff_neighbor_chroma(Macroblock *mb, int xN, int yN,
 ...
 *********************************************************************************************
 */
static void GetStrengthVerMBAff(byte Strength[16], Macroblock *MbQ, int edge,
                                int mvlimit, StorablePicture *p) {
  short blkP, blkQ, idx;
  ...
  if ((p->slice_type == SP_SLICE) || (p->slice_type == SI_SLICE)) {
    for (idx = 0; idx < MB_BLOCK_SIZE; ++idx) {
      ...
      MbQ->mixedModeEdgeFlag =
          (byte)(MbQ->mb_field !=
                 MbP->mb_field); // currSlice->mixedModeEdgeFlag = (byte)
                                 // (MbQ->mb_field != MbP->mb_field);
    ...
    if ((MbQ->mb_field == FALSE && MbP->mb_field == FALSE)) {
               ...
               // set Strength=1
          {
            ...
            if (((ref_p0 == ref_q0) && (ref_p1 == ref_q1)) ||
              ...
              if (ref_p0 != ref_p1) {
                ...
                if (ref_p0 == ref_q0) {
                  StrValue =
                      (byte)(compare_mvs(&mv_info_p->mv[LIST_0],
      ...
      for (idx = 0; idx < MB_BLOCK_SIZE; ++idx) {
        ...
        Strength[idx] =
            (edge == 0 &&
             (((!p->mb_aff_frame_flag && (p->structure == FRAME)) ||
        ...
        if (MbQ->is_intra_block == FALSE && MbP->is_intra_block == FALSE) {
          if (((MbQ->cbp_blk[0] & i64_power2(blkQ)) != 0) ||
            ...
            // if this is a mixed mode edge then one set of reference pictures
            // will be frame and the other will be field
            if (MbQ->mixedModeEdgeFlag) // if (currSlice->mixedModeEdgeFlag)
            {
              ...
              get_mb_block_pos_mbaff(MbQ->mbAddrX, &mb_x, &mb_y);
              {
                ...
                if (((ref_p0 == ref_q0) && (ref_p1 == ref_q1)) ||
                    ((ref_p0 == ref_q1) && (ref_p1 == ref_q0))) {
                  Strength[idx] = 0;
                  // L0 and L1 reference pictures of p0 are different; q0 as
                  // well
                  if (ref_p0 != ref_p1) {
                    // compare MV for the same reference picture
                    if (ref_p0 == ref_q0) {
                  ...
                  } else { // L0 and L1 reference pictures of p0 are the same;
                           // q0 as well
          ...
          if (MbQ->mixedModeEdgeFlag) // if (currSlice->mixedModeEdgeFlag)
          {
...

            {
              ...
              if (((ref_p0 == ref_q0) && (ref_p1 == ref_q1)) ||
                ...
                } else { // L0 and L1 reference pictures of p0 are the same; q0
                         // as well
                  StrValue =
                      (byte)((compare_mvs(&mv_info_p->mv[LIST_0],
                                          &mv_info_q->mv[LIST_0], mvlimit) ||
    ...
    for (pel = 0; pel < PelNum; ++pel) {
      ...
      if (pixP.available) {
        if ((Strng = Strength[pel]) != 0) {
          ...
          QP = pl ? ((MbP->qpc[pl - 1] + MbQ->qpc[pl - 1] + 1) >> 1)
                  : (MbP->qp + MbQ->qp + 1) >> 1;
          ...
          if (iabs(R0 - L0) < Alpha) {
            ...
            if ((iabs(R0 - R1) < Beta) && (iabs(L0 - L1) < Beta)) {
              ...
              } else // normal filtering
              {
                ...
                int tc0 = (C0 + ap + aq);
                int dif =
                    iClip3(-tc0, tc0, (((R0 - L0) << 2) + (L1 - R1) + 4) >> 3);

                if (ap && (C0 != 0))
                  *(SrcPtrP - 1) +=
                      iClip3(-C0, C0, (L2 + RL0 - (L1 << 1)) >> 1);
...
*****************************************************************************************
*/
static void EdgeLoopChromaVerMBAff(imgpel **Img, byte Strength[16],
                                   Macroblock *MbQ, int edge, int uv,
                                   StorablePicture *p) {
  int pel, Strng;
  ...
  for (pel = 0; pel < PelNum; ++pel) {
    ...
    MbP = &(p_Vid->mb_data[pixP.mb_addr]);
    StrengthIdx = (PelNum == 8) ? ((MbQ->mb_field && !MbP->mb_field)
                                       ? pel << 1
                                       : ((pel >> 1) << 2) + (pel & 0x01))
                                : pel;
...
*****************************************************************************************
*/
static void EdgeLoopChromaHorMBAff(imgpel **Img, byte Strength[16],
                                   Macroblock *MbQ, int edge, int uv,
                                   StorablePicture *p) {
  VideoParameters *p_Vid = MbQ->p_Vid;
