...
int FirstPartOfSliceHeader(Slice *currSlice) {
  ...
  p_Dec->UsedBits =
      partition->bitstream->frame_bitoffset; // was hardcoded to 31 for previous
                                             // start-code. This is better.
  ...
  currSlice->pic_parameter_set_id =
      ue_v("SH: pic_parameter_set_id", currStream);
...
int RestOfSliceHeader(Slice *currSlice) {
  ...
  currSlice->frame_num = u_v(active_sps->log2_max_frame_num_minus4 + 4,
                             "SH: frame_num", currStream);
  ...
  if (active_sps->frame_mbs_only_flag) {
    ...
    if (currSlice->field_pic_flag) {
      ...
      currSlice->bottom_field_flag =
          (byte)u_1("SH: bottom_field_flag", currStream);
      p_Vid->structure =
          currSlice->bottom_field_flag ? BOTTOM_FIELD : TOP_FIELD;
    } else {
      p_Vid->structure = FRAME;
  ...
  currSlice->mb_aff_frame_flag = (active_sps->mb_adaptive_frame_field_flag &&
                                  (currSlice->field_pic_flag == 0));
  ...
  if (p_Vid->structure == BOTTOM_FIELD)
    assert(currSlice->field_pic_flag == 1 &&
           (currSlice->bottom_field_flag == TRUE));
  ...
  if (active_sps->pic_order_cnt_type == 0) {
    currSlice->pic_order_cnt_lsb =
        u_v(active_sps->log2_max_pic_order_cnt_lsb_minus4 + 4,
            "SH: pic_order_cnt_lsb", currStream);
    if (p_Vid->active_pps->bottom_field_pic_order_in_frame_present_flag == 1 &&
      ...
      currSlice->delta_pic_order_cnt_bottom =
          se_v("SH: delta_pic_order_cnt_bottom", currStream);
    else
  ...
  if (currSlice->slice_type == B_SLICE) {
    currSlice->direct_spatial_mv_pred_flag =
        u_1("SH: direct_spatial_mv_pred_flag", currStream);
  }

  currSlice->num_ref_idx_active[LIST_0] =
      p_Vid->active_pps->num_ref_idx_l0_active_minus1 + 1;
  currSlice->num_ref_idx_active[LIST_1] =
      p_Vid->active_pps->num_ref_idx_l1_active_minus1 + 1;
  ...
  if (p_Vid->type == P_SLICE || p_Vid->type == SP_SLICE ||
    ...
    if (val) {
      currSlice->num_ref_idx_active[LIST_0] =
          1 + ue_v("SH: num_ref_idx_l0_active_minus1", currStream);

      if (p_Vid->type == B_SLICE) {
        currSlice->num_ref_idx_active[LIST_1] =
            1 + ue_v("SH: num_ref_idx_l1_active_minus1", currStream);
      }
    }
  }
  if (currSlice->slice_type != B_SLICE) {
  ...
  currSlice->weighted_pred_flag =
      (unsigned short)((currSlice->slice_type == P_SLICE ||
                        currSlice->slice_type == SP_SLICE)
                           ? p_Vid->active_pps->weighted_pred_flag
                           : (currSlice->slice_type == B_SLICE &&
                              p_Vid->active_pps->weighted_bipred_idc == 1));
  currSlice->weighted_bipred_idc =
      (unsigned short)(currSlice->slice_type == B_SLICE &&
                       p_Vid->active_pps->weighted_bipred_idc > 0);

  if ((p_Vid->active_pps->weighted_pred_flag &&
       (p_Vid->type == P_SLICE || p_Vid->type == SP_SLICE)) ||
      (p_Vid->active_pps->weighted_bipred_idc == 1 &&
       (p_Vid->type == B_SLICE))) {
  ...
  if (p_Vid->active_pps->entropy_coding_mode_flag && p_Vid->type != I_SLICE &&
      p_Vid->type != SI_SLICE) {
  ...
  // currSlice->qp = p_Vid->qp = 26 + p_Vid->active_pps->pic_init_qp_minus26 +
  // val;
  ...
  if (!HI_INTRA_ONLY_PROFILE ||
      (HI_INTRA_ONLY_PROFILE && (p_Inp->intra_profile_deblocking == 1)))
  // then read flags and parameters from bistream
  {
    if (p_Vid->active_pps->deblocking_filter_control_present_flag) {
      currSlice->DFDisableIdc =
          (short)ue_v("SH: disable_deblocking_filter_idc", currStream);

      if (currSlice->DFDisableIdc != 1) {
        currSlice->DFAlphaC0Offset =
            (short)(2 * se_v("SH: slice_alpha_c0_offset_div2", currStream));
        currSlice->DFBetaOffset =
            (short)(2 * se_v("SH: slice_beta_offset_div2", currStream));
      } else {
        currSlice->DFAlphaC0Offset = currSlice->DFBetaOffset = 0;
      }
    } else {
      currSlice->DFDisableIdc = currSlice->DFAlphaC0Offset =
          currSlice->DFBetaOffset = 0;
    }
  } else // By default the Loop Filter is Off
  {      // 444_TEMP_NOTE: change made below. 08/07/07
    // still need to parse the SEs (read flags and parameters from bistream) but
    // will ignore
    if (p_Vid->active_pps->deblocking_filter_control_present_flag) {
      currSlice->DFDisableIdc =
          (short)ue_v("SH: disable_deblocking_filter_idc", currStream);
  ...
  if (p_Vid->active_pps->num_slice_groups_minus1 > 0 &&
      p_Vid->active_pps->slice_group_map_type >= 3 &&
      p_Vid->active_pps->slice_group_map_type <= 5) {
    len = (active_sps->pic_height_in_map_units_minus1 + 1) *
          (active_sps->pic_width_in_mbs_minus1 + 1) /
          (p_Vid->active_pps->slice_group_change_rate_minus1 + 1);
    if (((active_sps->pic_height_in_map_units_minus1 + 1) *
         (active_sps->pic_width_in_mbs_minus1 + 1)) %
        (p_Vid->active_pps->slice_group_change_rate_minus1 + 1))
      len += 1;
    ...
    currSlice->slice_group_change_cycle =
        u_v(len, "SH: slice_group_change_cycle", currStream);
  }
  p_Vid->PicHeightInMbs =
      p_Vid->FrameHeightInMbs / (1 + currSlice->field_pic_flag);
...
static void ref_pic_list_reordering(Slice *currSlice) {
  ...
  if (p_Vid->type != I_SLICE && p_Vid->type != SI_SLICE) {
    val = currSlice->ref_pic_list_reordering_flag[LIST_0] =
        u_1("SH: ref_pic_list_reordering_flag_l0", currStream);

    if (val) {
      i = 0;
      do {
        val = currSlice->reordering_of_pic_nums_idc[LIST_0][i] =
            ue_v("SH: reordering_of_pic_nums_idc_l0", currStream);
        if (val == 0 || val == 1) {
          currSlice->abs_diff_pic_num_minus1[LIST_0][i] =
              ue_v("SH: abs_diff_pic_num_minus1_l0", currStream);
        } else {
          if (val == 2) {
            currSlice->long_term_pic_idx[LIST_0][i] =
                ue_v("SH: long_term_pic_idx_l0", currStream);
          }
        }
        i++;
  ...
  if (p_Vid->type == B_SLICE) {
    val = currSlice->ref_pic_list_reordering_flag[LIST_1] =
        u_1("SH: ref_pic_list_reordering_flag_l1", currStream);

    if (val) {
      i = 0;
      do {
        val = currSlice->reordering_of_pic_nums_idc[LIST_1][i] =
            ue_v("SH: reordering_of_pic_nums_idc_l1", currStream);
        if (val == 0 || val == 1) {
          currSlice->abs_diff_pic_num_minus1[LIST_1][i] =
              ue_v("SH: abs_diff_pic_num_minus1_l1", currStream);
        } else {
          if (val == 2) {
            currSlice->long_term_pic_idx[LIST_1][i] =
                ue_v("SH: long_term_pic_idx_l1", currStream);
          }
        }
        i++;
...
static void ref_pic_list_mvc_modification(Slice *currSlice) {
  ...
  if ((p_Vid->type % 5) != I_SLICE && (p_Vid->type % 5) != SI_SLICE) {
    val = currSlice->ref_pic_list_reordering_flag[LIST_0] =
        u_1("SH: ref_pic_list_modification_flag_l0", currStream);

    if (val) {
      i = 0;
      do {
        val = currSlice->reordering_of_pic_nums_idc[LIST_0][i] =
            ue_v("SH: modification_of_pic_nums_idc_l0", currStream);
        if (val == 0 || val == 1) {
          currSlice->abs_diff_pic_num_minus1[LIST_0][i] =
              ue_v("SH: abs_diff_pic_num_minus1_l0", currStream);
        } else {
          if (val == 2) {
            ...
            currSlice->abs_diff_view_idx_minus1[LIST_0][i] =
                ue_v("SH: abs_diff_view_idx_minus1_l0", currStream);
          }
        }
        i++;
  ...
  if ((p_Vid->type % 5) == B_SLICE) {
    ...
    if (val) {
      ...
      do {
        val = currSlice->reordering_of_pic_nums_idc[LIST_1][i] =
            ue_v("SH: modification_of_pic_nums_idc_l1", currStream);
        if (val == 0 || val == 1) {
          currSlice->abs_diff_pic_num_minus1[LIST_1][i] =
              ue_v("SH: abs_diff_pic_num_minus1_l1", currStream);
        } else {
          if (val == 2) {
            ...
            currSlice->abs_diff_view_idx_minus1[LIST_1][i] =
                ue_v("SH: abs_diff_view_idx_minus1_l1", currStream);
          }
        }
        i++;
...
static void pred_weight_table(Slice *currSlice) {
  ...
  DataPartition *partition = &(currSlice->partArr[dP_nr]);
  Bitstream *currStream = partition->bitstream;
  int luma_weight_flag_l0, luma_weight_flag_l1, chroma_weight_flag_l0,
      chroma_weight_flag_l1;
  int i, j;

  currSlice->luma_log2_weight_denom =
      (unsigned short)ue_v("SH: luma_log2_weight_denom", currStream);
  currSlice->wp_round_luma = currSlice->luma_log2_weight_denom
                                 ? 1 << (currSlice->luma_log2_weight_denom - 1)
                                 : 0;

  if (0 != active_sps->chroma_format_idc) {
    currSlice->chroma_log2_weight_denom =
        (unsigned short)ue_v("SH: chroma_log2_weight_denom", currStream);
    currSlice->wp_round_chroma =
        currSlice->chroma_log2_weight_denom
            ? 1 << (currSlice->chroma_log2_weight_denom - 1)
            : 0;
  }
  ...
  for (i = 0; i < currSlice->num_ref_idx_active[LIST_0]; i++) {
    ...
    if (luma_weight_flag_l0) {
      ...
      currSlice->wp_offset[0][i][0] = currSlice->wp_offset[0][i][0]
    ...
    if (active_sps->chroma_format_idc != 0) {
      ...
      for (j = 1; j < 3; j++) {
        if (chroma_weight_flag_l0) {
          currSlice->wp_weight[0][i][j] =
              se_v("SH: chroma_weight_l0", currStream);
          currSlice->wp_offset[0][i][j] =
              se_v("SH: chroma_offset_l0", currStream);
          currSlice->wp_offset[0][i][j] = currSlice->wp_offset[0][i][j]
                                          << (p_Vid->bitdepth_chroma - 8);
        } else {
          currSlice->wp_weight[0][i][j] =
              1 << currSlice->chroma_log2_weight_denom;
  ...
  if ((p_Vid->type == B_SLICE) && p_Vid->active_pps->weighted_bipred_idc == 1) {
    for (i = 0; i < currSlice->num_ref_idx_active[LIST_1]; i++) {
      ...
      if (luma_weight_flag_l1) {
        ...
        currSlice->wp_offset[1][i][0] = currSlice->wp_offset[1][i][0]
                                        << (p_Vid->bitdepth_luma - 8);
      } else {
      ...
      if (active_sps->chroma_format_idc != 0) {
        ...
        for (j = 1; j < 3; j++) {
          if (chroma_weight_flag_l1) {
            currSlice->wp_weight[1][i][j] =
                se_v("SH: chroma_weight_l1", currStream);
            currSlice->wp_offset[1][i][j] =
                se_v("SH: chroma_offset_l1", currStream);
            currSlice->wp_offset[1][i][j] = currSlice->wp_offset[1][i][j]
                                            << (p_Vid->bitdepth_chroma - 8);
          } else {
            currSlice->wp_weight[1][i][j] =
                1 << currSlice->chroma_log2_weight_denom;
    ...
    if (pSlice->adaptive_ref_pic_buffering_flag) {
      ...
      do {
        ...
        val = tmp_drpm->memory_management_control_operation =
            ue_v("SH: memory_management_control_operation", currStream);

        if ((val == 1) || (val == 3)) {
          tmp_drpm->difference_of_pic_nums_minus1 =
              ue_v("SH: difference_of_pic_nums_minus1", currStream);
        }
        if (val == 2) {
          tmp_drpm->long_term_pic_num =
              ue_v("SH: long_term_pic_num", currStream);
        }

        if ((val == 3) || (val == 6)) {
          tmp_drpm->long_term_frame_idx =
              ue_v("SH: long_term_frame_idx", currStream);
        }
        if (val == 4) {
          tmp_drpm->max_long_term_frame_idx_plus1 =
              ue_v("SH: max_long_term_pic_idx_plus1", currStream);
        }
 ...
 *    values, but the management of POCs in buffered pictures may need more
 *work. \return none
 ************************************************************************
 */
void decode_poc(VideoParameters *p_Vid, Slice *pSlice) {
  ...
  unsigned int MaxPicOrderCntLsb =
      (1 << (active_sps->log2_max_pic_order_cnt_lsb_minus4 + 4));
  ...
  case 0: // POC MODE 0
    ...
    if (pSlice->pic_order_cnt_lsb < p_Vid->PrevPicOrderCntLsb &&
      ...
      pSlice->PicOrderCntMsb = p_Vid->PrevPicOrderCntMsb + MaxPicOrderCntLsb;
             ...
             (pSlice->pic_order_cnt_lsb - p_Vid->PrevPicOrderCntLsb) >
                 (MaxPicOrderCntLsb / 2))
      pSlice->PicOrderCntMsb = p_Vid->PrevPicOrderCntMsb - MaxPicOrderCntLsb;
    else
    ...
    if (pSlice->field_pic_flag == 0) { // frame pix
      ...
      pSlice->ThisPOC = pSlice->framepoc = (pSlice->toppoc < pSlice->bottompoc)
                                               ? pSlice->toppoc
                                               : pSlice->bottompoc; // POC200301
    } else if (pSlice->bottom_field_flag == FALSE) {                // top field
      pSlice->ThisPOC = pSlice->toppoc =
          pSlice->PicOrderCntMsb + pSlice->pic_order_cnt_lsb;
    } else { // bottom field
  ...
  case 1: // POC MODE 1
    ...
    if (pSlice->idr_flag) {
      ...
      if (pSlice->frame_num <
          p_Vid->PreviousFrameNum) { // not first pix of IDRGOP
        p_Vid->FrameNumOffset =
            p_Vid->PreviousFrameNumOffset + p_Vid->MaxFrameNum;
      } else {
    ...
    if (pSlice->field_pic_flag == 0) { // frame pix
      pSlice->toppoc =
          p_Vid->ExpectedPicOrderCnt + pSlice->delta_pic_order_cnt[0];
