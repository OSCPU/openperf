 ...
 ************************************************************************
 */
void reorder_short_term(Slice *currSlice, int cur_list,
  ...
  for (cIdx = *refIdxLX; cIdx <= num_ref_idx_lX_active_minus1 + 1; cIdx++)
    if (RefPicListX[cIdx]) {
      if ((RefPicListX[cIdx]->is_long_term) ||
          ...
          (currViewID != -1 && RefPicListX[cIdx]->view_id != currViewID))
...
void reorder_long_term(Slice *currSlice, StorablePicture **RefPicListX,
                       int num_ref_idx_lX_active_minus1, int LongTermPicNum,
                       int *refIdxLX, int currViewID) {
  int cIdx, nIdx;
 ...
 ************************************************************************
 */
static StorablePicture *get_inter_view_pic(VideoParameters *p_Vid,
                                           Slice *currSlice, int targetViewID,
  ...
  for (i = 0; i < listinterview_size; i++) {
    if (fs_listinterview[i]->view_id == targetViewID) {
      if (p_Vid->structure == FRAME &&
          fs_listinterview[i]->frame->poc == currPOC) {
        return fs_listinterview[i]->frame;
      } else if (p_Vid->structure == TOP_FIELD &&
                 fs_listinterview[i]->top_field->poc == currPOC) {
        return fs_listinterview[i]->top_field;
      } else if (p_Vid->structure == BOTTOM_FIELD &&
                 fs_listinterview[i]->bottom_field->poc == currPOC) {
...
static void gen_pic_list_from_frame_interview_list(
    PictureStructure currStructure, FrameStore **fs_list, int list_idx,
...
void init_lists_b_slice_mvc(Slice *currSlice) {
...

  {
    ...
    if (currSlice->structure == FRAME) {
      ...
      //      printf("currSlice->listX[0] currPoc=%d (Poc): ",
      //      currSlice->framepoc); for (i=0; i<currSlice->listXsize[0];
      //      i++){printf ("%d  ", currSlice->listX[0][i]->poc);} printf("\n");
      //      printf("currSlice->listX[1] currPoc=%d (Poc): ",
      //      currSlice->framepoc); for (i=0; i<currSlice->listXsize[1];
      //      i++){printf ("%d  ", currSlice->listX[1][i]->poc);} printf("\n");
...
static void reorder_interview(VideoParameters *p_Vid, Slice *currSlice,
  ...
  if (picLX) {
    ...
    for (cIdx = *refIdxLX; cIdx <= num_ref_idx_lX_active_minus1 + 1; cIdx++)
      if ((RefPicListX[cIdx]->view_id != targetViewID) ||
          (RefPicListX[cIdx]->poc != currPOC))
        RefPicListX[nIdx++] = RefPicListX[cIdx];
  }
}
 ...
 ************************************************************************
 */
void reorder_ref_pic_list_mvc(Slice *currSlice, int cur_list, int **anchor_ref,
                              int **non_anchor_ref, int view_id,
                              int anchor_pic_flag, int currPOC, int listidx) {
  VideoParameters *p_Vid = currSlice->p_Vid;
  ...
  for (i = 0; reordering_of_pic_nums_idc[i] != 3; i++) {
    ...
    if (reordering_of_pic_nums_idc[i] < 2) {
      ...
      reorder_short_term(currSlice, cur_list, num_ref_idx_lX_active_minus1,
                         picNumLX, &refIdxLX, view_id);
    } else if (reordering_of_pic_nums_idc[i] ==
               2) //(remapping_of_pic_nums_idc[i] == 2)
    {
      reorder_long_term(currSlice, currSlice->listX[cur_list],
                        num_ref_idx_lX_active_minus1, long_term_pic_idx[i],
                        &refIdxLX, view_id);
    } else {
      if (reordering_of_pic_nums_idc[i] ==
          4) //(remapping_of_pic_nums_idc[i] == 4)
      {
      ...
      reorder_interview(p_Vid, currSlice, currSlice->listX[cur_list],
                        num_ref_idx_lX_active_minus1, &refIdxLX, targetViewID,
                        currPOC, listidx);
    }
  }
  // that's a definition
...
void reorder_lists_mvc(Slice *currSlice, int currPOC) {
  ...
  if ((currSlice->slice_type != I_SLICE) &&
    ...
    if (currSlice->ref_pic_list_reordering_flag[LIST_0]) {
      reorder_ref_pic_list_mvc(
          currSlice, LIST_0, p_Vid->active_subset_sps->anchor_ref_l0,
          p_Vid->active_subset_sps->non_anchor_ref_l0, currSlice->view_id,
          currSlice->anchor_pic_flag, currPOC, 0);
    }
  ...
  if (currSlice->slice_type == B_SLICE) {
    if (currSlice->ref_pic_list_reordering_flag[LIST_1]) {
      reorder_ref_pic_list_mvc(
          ...
          p_Vid->active_subset_sps->non_anchor_ref_l1, currSlice->view_id,
          currSlice->anchor_pic_flag, currPOC, 1);
    }
    if (p_Vid->no_reference_picture ==
