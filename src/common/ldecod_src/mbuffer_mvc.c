 ...
 ************************************************************************
 */
static void gen_pic_list_from_frame_interview_list(PictureStructure currStructure, FrameStore **fs_list, int list_idx, StorablePicture **list, signed char *list_size)
{
  int i;
...
void reorder_lists_mvc(Slice * currSlice, int currPOC)
{
  VideoParameters *p_Vid = currSlice->p_Vid;
	...
	if ((currSlice->slice_type != I_SLICE)&&(currSlice->slice_type != SI_SLICE))
	{
		...
		// that's a definition
		currSlice->listXsize[0] = (signed char)currSlice->num_ref_idx_active[LIST_0];
	}
	if (currSlice->slice_type == B_SLICE)
	{
		...
		// that's a definition
		currSlice->listXsize[1] = (signed char)currSlice->num_ref_idx_active[LIST_1];
	}
