...
extern int get_colocated_info_8x8(Macroblock *currMB, StorablePicture *list1,
                                  int i, int j);
extern int get_colocated_info_4x4(Macroblock *currMB, StorablePicture *list1,
                                  int i, int j);

int mb_pred_intra4x4(Macroblock *currMB, ColorPlane curr_plane,
  ...
  currMB->itrans_4x4 =
      (currMB->is_lossless == FALSE) ? itrans4x4 : Inv_Residual_trans_4x4;
  ...
  for (block8x8 = 0; block8x8 < 4; block8x8++) {
    for (k = block8x8 * 4; k < block8x8 * 4 + 4; k++) {
      ...
      // PREDICTION
      //===== INTRA PREDICTION =====
      if (intrapred(currMB, curr_plane, ioff, joff, i4, j4) ==
          SEARCH_SYNC) /* make 4x4 prediction block mpr from given prediction
                          p_Vid->mb_mode */
      ...
      copy_image_data_4x4(&currImg[j_pos], &currSlice->mb_rec[curr_plane][joff],
                          i_pos, ioff);
    }
  }
...
int mb_pred_intra16x16(Macroblock *currMB, ColorPlane curr_plane,
                       StorablePicture *dec_picture) {
  int yuv = dec_picture->chroma_format_idc - 1;

  intrapred16x16(currMB, curr_plane, currMB->i16mode);
  currMB->ipmode_DPCM = (signed char)currMB->i16mode; // For residual DPCM
  // =============== 4x4 itrans ================
  // -------------------------------------------
...
int mb_pred_intra8x8(Macroblock *currMB, ColorPlane curr_plane,
                     imgpel **currImg, StorablePicture *dec_picture) {
  Slice *currSlice = currMB->p_Slice;
  int yuv = dec_picture->chroma_format_idc - 1;
  ...
  int block8x8; // needed for ABT
  currMB->itrans_8x8 =
      (currMB->is_lossless == FALSE) ? itrans8x8 : Inv_Residual_trans_8x8;
  ...
  for (block8x8 = 0; block8x8 < 4; block8x8++) {
    ...
    currMB->itrans_8x8(currMB, curr_plane, ioff,
                       joff); // use inverse integer transform and make 8x8
                              // block m7 from prediction block mpr

    copy_image_data_8x8(&currImg[currMB->pix_y + joff],
                        &currSlice->mb_rec[curr_plane][joff],
                        currMB->pix_x + ioff, ioff);
  }
  // chroma decoding *******************************************************
  if ((dec_picture->chroma_format_idc != YUV400) &&
      (dec_picture->chroma_format_idc != YUV444)) {
...
static void set_chroma_vector(Macroblock *currMB) {
    ...
    if (currSlice->structure == TOP_FIELD) {
      ...
      for (l = LIST_0; l <= (LIST_1); l++) {
        for (k = 0; k < currSlice->listXsize[l]; k++) {
          if (p_Vid->structure != (int)currSlice->listX[l][k]->structure)
  ...
  currSlice->max_mb_vmv_r =
      (currSlice->structure != FRAME || (currMB->mb_field))
          ? p_Vid->max_vmv_r >> 1
          : p_Vid->max_vmv_r;
}

int mb_pred_skip(Macroblock *currMB, ColorPlane curr_plane, imgpel **currImg,
                 StorablePicture *dec_picture) {
  Slice *currSlice = currMB->p_Slice;
  VideoParameters *p_Vid = currMB->p_Vid;
  ...
  perform_mc(currMB, curr_plane, dec_picture, LIST_0, 0, 0, MB_BLOCK_SIZE,
             MB_BLOCK_SIZE);

  copy_image_data_16x16(&currImg[currMB->pix_y], currSlice->mb_pred[curr_plane],
                        currMB->pix_x, 0);

  if ((dec_picture->chroma_format_idc != YUV400) &&
      (dec_picture->chroma_format_idc != YUV444)) {

    copy_image_data(&dec_picture->imgUV[0][currMB->pix_c_y],
                    currSlice->mb_pred[1], currMB->pix_c_x, 0,
                    p_Vid->mb_size[1][0], p_Vid->mb_size[1][1]);
    copy_image_data(&dec_picture->imgUV[1][currMB->pix_c_y],
                    currSlice->mb_pred[2], currMB->pix_c_x, 0,
                    p_Vid->mb_size[1][0], p_Vid->mb_size[1][1]);
  }
  return 1;
}

int mb_pred_sp_skip(Macroblock *currMB, ColorPlane curr_plane,
                    StorablePicture *dec_picture) {
  set_chroma_vector(currMB);
...
int mb_pred_p_inter8x8(Macroblock *currMB, ColorPlane curr_plane,
                       StorablePicture *dec_picture) {
  int block8x8; // needed for ABT
  int i = 0, j = 0, k;
  ...
  for (block8x8 = 0; block8x8 < 4; block8x8++) {
    ...
    int k_end = (mv_mode == SMB8x8)
                    ? k_start + 1
                    : ((mv_mode == SMB4x4) ? k_start + 4 : k_start + k_inc + 1);

    int block_size_x =
        (mv_mode == SMB8x4 || mv_mode == SMB8x8) ? SMB_BLOCK_SIZE : BLOCK_SIZE;
    int block_size_y =
        (mv_mode == SMB4x8 || mv_mode == SMB8x8) ? SMB_BLOCK_SIZE : BLOCK_SIZE;

    for (k = k_start; k < k_end; k += k_inc) {
      i = (decode_block_scan[k] & 3);
      j = ((decode_block_scan[k] >> 2) & 3);
      perform_mc(currMB, curr_plane, dec_picture, pred_dir, i, j, block_size_x,
                 block_size_y);
    }
  }
...
int mb_pred_p_inter16x16(Macroblock *currMB, ColorPlane curr_plane,
                         StorablePicture *dec_picture) {
  int smb = (currMB->p_Vid->type == SP_SLICE);
  Slice *currSlice = currMB->p_Slice;

  set_chroma_vector(currMB);
  perform_mc(currMB, curr_plane, dec_picture, currMB->b8pdir[0], 0, 0,
...
int mb_pred_p_inter16x8(Macroblock *currMB, ColorPlane curr_plane,
                        StorablePicture *dec_picture) {
  int smb = (currMB->p_Vid->type == SP_SLICE);
  Slice *currSlice = currMB->p_Slice;
  ...
  perform_mc(currMB, curr_plane, dec_picture, currMB->b8pdir[0], 0, 0,
             MB_BLOCK_SIZE, BLOCK_SIZE_8x8);
  perform_mc(currMB, curr_plane, dec_picture, currMB->b8pdir[2], 0, 2,
             MB_BLOCK_SIZE, BLOCK_SIZE_8x8);
  iTransform(currMB, curr_plane, smb);
...
int mb_pred_p_inter8x16(Macroblock *currMB, ColorPlane curr_plane,
                        StorablePicture *dec_picture) {
  int smb = (currMB->p_Vid->type == SP_SLICE);
  Slice *currSlice = currMB->p_Slice;
  ...
  perform_mc(currMB, curr_plane, dec_picture, currMB->b8pdir[0], 0, 0,
             BLOCK_SIZE_8x8, MB_BLOCK_SIZE);
  perform_mc(currMB, curr_plane, dec_picture, currMB->b8pdir[1], 2, 0,
             BLOCK_SIZE_8x8, MB_BLOCK_SIZE);
  iTransform(currMB, curr_plane, smb);
...
static inline void update_neighbor_mvs(PicMotionParams **motion,
                                       const PicMotionParams *mv_info, int i4) {
  (*motion++)[i4 + 1] = *mv_info;
  (*motion)[i4] = *mv_info;
  (*motion)[i4 + 1] = *mv_info;
}

int mb_pred_b_d8x8temporal(Macroblock *currMB, ColorPlane curr_plane,
                           imgpel **currImg, StorablePicture *dec_picture) {
  short ref_idx;
  int refList;
  ...
  for (block8x8 = 0; block8x8 < 4; block8x8++) {
    ...
    for (k = k_start; k < k_start + BLOCK_MULTIPLE; k++) {
      ...
      if(currSlice->mb_aff_frame_flag /*&& (!p_Vid->active_sps->frame_mbs_only_flag || p_Vid->active_sps->direct_8x8_inference_flag)*/)
      {
        ...
        if (!currMB->mb_field &&
            ((currSlice->listX[LIST_1][0]->iCodingType ==
                  FRAME_MB_PAIR_CODING &&
              currSlice->listX[LIST_1][0]->motion.mb_field[currMB->mbAddrX]) ||
          ...
          if (iabs(dec_picture->poc - currSlice->listX[LIST_1 + 4][0]->poc) >
            ...
            colocated =
                p_Vid->active_sps->direct_8x8_inference_flag
                    ? &currSlice->listX[LIST_1 + 2][0]
                           ->mv_info[RSD(j6) >> 1][RSD(i4)]
                    : &currSlice->listX[LIST_1 + 2][0]->mv_info[j6 >> 1][i4];
          } else {
            colocated =
                p_Vid->active_sps->direct_8x8_inference_flag
                    ? &currSlice->listX[LIST_1 + 4][0]
                           ->mv_info[RSD(j6) >> 1][RSD(i4)]
                    : &currSlice->listX[LIST_1 + 4][0]->mv_info[j6 >> 1][i4];
          }
        }
      } else if (/*!currSlice->mb_aff_frame_flag &&*/ !p_Vid->active_sps
                     ->frame_mbs_only_flag &&
        ...
        if ((currSlice->mb_aff_frame_flag &&
            ...
            (!currSlice->mb_aff_frame_flag &&
             ((currSlice->field_pic_flag == 0 &&
              ...
              (currSlice->field_pic_flag == 1 &&
               colocated->ref_pic[refList]->structure == FRAME)))) {
          for (iref = 0;
               iref < imin(currSlice->num_ref_idx_active[LIST_0],
                           currSlice->listXsize[LIST_0 + list_offset]);
               iref++) {
            if (currSlice->listX[LIST_0 + list_offset][iref]->top_field ==
                    colocated->ref_pic[refList] ||
                currSlice->listX[LIST_0 + list_offset][iref]->bottom_field ==
                    colocated->ref_pic[refList] ||
                currSlice->listX[LIST_0 + list_offset][iref]->frame ==
                    colocated->ref_pic[refList]) {
              mapped_idx = iref;
              break;
            } else //! invalid index. Default to zero even though this case
                   //! should not happen
            {
          ...
          for (iref = 0;
               iref < imin(currSlice->num_ref_idx_active[LIST_0],
                           currSlice->listXsize[LIST_0 + list_offset]);
               iref++) {
            if (currSlice->listX[LIST_0 + list_offset][iref] ==
                colocated->ref_pic[refList]) {
              mapped_idx = iref;
              break;
            } else //! invalid index. Default to zero even though this case
        ...
        if (INVALIDINDEX != mapped_idx) {
          ...
          if ((currSlice->mb_aff_frame_flag && !currMB->mb_field &&
              ...
              (!currSlice->mb_aff_frame_flag &&
               ...
               colocated->ref_pic[refList]->structure != FRAME))
            mv_y *= 2;
          else if ((currSlice->mb_aff_frame_flag && currMB->mb_field &&
                    colocated->ref_pic[refList]->structure == FRAME) ||
                   (!currSlice->mb_aff_frame_flag &&
                    currSlice->field_pic_flag == 1 &&
                    colocated->ref_pic[refList]->structure == FRAME))
          ...
          if (mv_scale == 9999 ||
              currSlice->listX[LIST_0 + list_offset][mapped_idx]
                  ->is_long_term) {
            mv_info->mv[LIST_0].mv_x = colocated->mv[refList].mv_x;
            mv_info->mv[LIST_0].mv_y = (short)mv_y;
            mv_info->mv[LIST_1] = zero_mv;
          } else {
            mv_info->mv[LIST_0].mv_x =
                (short)((mv_scale * colocated->mv[refList].mv_x + 128) >> 8);
            mv_info->mv[LIST_0].mv_y =
                (short)((mv_scale * mv_y /*colocated->mv[refList].mv_y*/ +
                         128) >>
                        8);

            mv_info->mv[LIST_1].mv_x =
                (short)(mv_info->mv[LIST_0].mv_x - colocated->mv[refList].mv_x);
            mv_info->mv[LIST_1].mv_y =
                (short)(mv_info->mv[LIST_0].mv_y -
                        mv_y /*colocated->mv[refList].mv_y*/);
          }

          mv_info->ref_idx[LIST_0] =
              (signed char)mapped_idx; // colocated->ref_idx[refList];
          mv_info->ref_idx[LIST_1] = 0;
        } else if (INVALIDINDEX == mapped_idx) {
          error("temporal direct error: colocated block has ref that is "
                "unavailable",
                -1111);
        }
      }
    ...
    for (k = k_start; k < k_end; k++) {
      ...
      perform_mc(currMB, curr_plane, dec_picture, pred_dir, i, j,
                 SMB_BLOCK_SIZE, SMB_BLOCK_SIZE);
    }
  }

  if (currMB->cbp == 0) {
    copy_image_data_16x16(&currImg[currMB->pix_y],
                          currSlice->mb_pred[curr_plane], currMB->pix_x, 0);
...
int mb_pred_b_d4x4temporal(Macroblock *currMB, ColorPlane curr_plane,
                           imgpel **currImg, StorablePicture *dec_picture) {
  short ref_idx;
  int refList;
  ...
  for (block8x8 = 0; block8x8 < 4; block8x8++) {
    ...
    for (k = k_start; k < k_end; k++) {
      ...
      } else // co-located skip or inter mode
      {
        ...
        for (iref = 0; iref < imin(currSlice->num_ref_idx_active[LIST_0],
                                   currSlice->listXsize[LIST_0 + list_offset]);
             iref++) {
          if (currSlice->listX[LIST_0 + list_offset][iref] ==
              colocated->ref_pic[refList]) {
            mapped_idx = iref;
            break;
          } else //! invalid index. Default to zero even though this case should
                 //! not happen
          {
            mapped_idx = INVALIDINDEX;
          }
        ...
        if (INVALIDINDEX == mapped_idx) {
          ...
          if (mv_scale == 9999 ||
            ...
            mv_info->mv[LIST_0].mv_x =
                (short)((mv_scale * colocated->mv[refList].mv_x + 128) >> 8);
            mv_info->mv[LIST_0].mv_y =
                (short)((mv_scale * colocated->mv[refList].mv_y + 128) >> 8);

            mv_info->mv[LIST_1].mv_x =
                (short)(mv_info->mv[LIST_0].mv_x - colocated->mv[refList].mv_x);
            mv_info->mv[LIST_1].mv_y =
                (short)(mv_info->mv[LIST_0].mv_y - colocated->mv[refList].mv_y);
          }
    ...
    for (k = k_start; k < k_end; k++) {
      int i = (decode_block_scan[k] & 3);
      int j = ((decode_block_scan[k] >> 2) & 3);
      perform_mc(currMB, curr_plane, dec_picture, pred_dir, i, j, BLOCK_SIZE,
                 BLOCK_SIZE);
    }
  }
...
int mb_pred_b_d8x8spatial(Macroblock *currMB, ColorPlane curr_plane,
                          imgpel **currImg, StorablePicture *dec_picture) {
  ...
  prepare_direct_params(currMB, dec_picture, &pmvl0, &pmvl1, &l0_rFrame,
                        &l1_rFrame);
  ...
  if (l0_rFrame == 0 || l1_rFrame == 0) {
    ...
    for (block8x8 = 0; block8x8 < 4; block8x8++) {
      ...
      is_not_moving = (get_colocated_info_8x8(currMB, list1[0], i4,
                                              currMB->block_y_aff + j) == 0);
    ...
    if (l0_rFrame < 0 && l1_rFrame < 0) {
      pred_dir = 2;
      for (j4 = currMB->block_y; j4 < currMB->block_y + BLOCK_MULTIPLE;
           j4 += 2) {
        for (i4 = currMB->block_x; i4 < currMB->block_x + BLOCK_MULTIPLE;
             i4 += 2) {
          mv_info = &dec_picture->mv_info[j4][i4];
    ...
    perform_mc(currMB, curr_plane, dec_picture, pred_dir, 0, 0, MB_BLOCK_SIZE,
               MB_BLOCK_SIZE);
  }
...
int mb_pred_b_d4x4spatial(Macroblock *currMB, ColorPlane curr_plane,
                          imgpel **currImg, StorablePicture *dec_picture) {
  signed char l0_rFrame = -1, l1_rFrame = -1;
  MotionVector pmvl0 = zero_mv, pmvl1 = zero_mv;
  int k;
  ...
  for (block8x8 = 0; block8x8 < 4; block8x8++) {
    ...
    for (k = k_start; k < k_end; k++) {
      ...
      if (l0_rFrame == 0 || l1_rFrame == 0) {
        int is_not_moving =
            (get_colocated_info_4x4(currMB, list1[0], i4,
                                    currMB->block_y_aff + j) == 0);
...
int mb_pred_b_inter8x8(Macroblock *currMB, ColorPlane curr_plane,
                       StorablePicture *dec_picture) {
  signed char l0_rFrame = -1, l1_rFrame = -1;
  MotionVector pmvl0 = zero_mv, pmvl1 = zero_mv;
  ...
  if (currSlice->direct_spatial_mv_pred_flag &&
      (!(currMB->b8mode[0] && currMB->b8mode[1] && currMB->b8mode[2] &&
         currMB->b8mode[3])))
    prepare_direct_params(currMB, dec_picture, &pmvl0, &pmvl1, &l0_rFrame,
                          &l1_rFrame);
  ...
  for (block8x8 = 0; block8x8 < 4; block8x8++) {
    ...
    if (mv_mode != 0) {
      ...
      block_size_x = (mv_mode == SMB8x4 || mv_mode == SMB8x8) ? SMB_BLOCK_SIZE
                                                              : BLOCK_SIZE;
      block_size_y = (mv_mode == SMB4x8 || mv_mode == SMB8x8) ? SMB_BLOCK_SIZE
                                                              : BLOCK_SIZE;
      ...
      if (currSlice->direct_spatial_mv_pred_flag) {
        for (k = k_start; k < k_start + BLOCK_MULTIPLE; k++) {
          ...
          /*
                  if ((pred_dir & 0x01) == 0)
                    mv_info->ref_pic[LIST_0] =
             list0[(short)mv_info->ref_idx[LIST_0]]; if (pred_dir > 0)
                    mv_info->ref_pic[LIST_1] =
             list1[(short)mv_info->ref_idx[LIST_1]];
                    */
        }
      } else {
        for (k = k_start; k < k_start + BLOCK_MULTIPLE; k++) {
 ...
 ************************************************************************
 * \brief
 *    Copy IPCM coefficients to decoded picture buffer and set parameters for
 *this MB (for IPCM CABAC and IPCM CAVLC  28/11/2003)
 *
 * \author
...
int mb_pred_ipcm(Macroblock *currMB) {
  ...
  // IPCM coefficients are stored in currSlice->cof which is set in function
  // read_IPCM_coeffs_from_NAL()

  for (i = 0; i < MB_BLOCK_SIZE; ++i) {
    for (j = 0; j < MB_BLOCK_SIZE; ++j) {
      dec_picture->imgY[currMB->pix_y + i][currMB->pix_x + j] =
          (imgpel)currSlice->cof[0][i][j];
    }
  }
    ...
    for (k = 0; k < 2; ++k) {
      for (i = 0; i < p_Vid->mb_cr_size_y; ++i) {
        for (j = 0; j < p_Vid->mb_cr_size_x; ++j) {
          dec_picture->imgUV[k][currMB->pix_c_y + i][currMB->pix_c_x + j] =
              (imgpel)currSlice->cof[k + 1][i][j];
        }
      }
    }
  }
  ...
  memset(p_Vid->nz_coeff[currMB->mbAddrX][0][0], 16,
