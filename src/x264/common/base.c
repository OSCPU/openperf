...
#define HUGE_PAGE_THRESHOLD HUGE_PAGE_SIZE*7/8 /* FIXME: Is this optimal? */
    ...
    uint8_t *buf = bench_malloc( i_size + (NATIVE_ALIGN-1) + sizeof(void **) );
    if( buf )
    {
...
char *x264_param_strdup( x264_param_t *param, const char *src )
{
    ...
    if( !buf )
    {
        buf = bench_malloc( BUFFER_OFFSET + BUFFER_DEFAULT_SIZE * sizeof(void *) );
        if( !buf )
            goto fail;
    ...
    else if( buf->count == buf->size )
    {
        ...
        buf = bench_realloc( buf, BUFFER_OFFSET + new_size * sizeof(void *) );
        if( !buf )
            goto fail;
        buf->size = new_size;
        param->opaque = buf;
