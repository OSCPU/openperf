...
public:
  ...
  // SInt(etl::string initial) : ui(initial) {
  //   sign_extend();
  // }

  // TODO: make array's template parameters somehow inherit from ui
  SInt(etl::array<uint64_t, (w_+63)/64> raw_input_reversed) : ui(raw_input_reversed) {
    sign_extend();
  }
...
private:
  ...
  // template<int w>
  // friend etl::ostream& operator<<(etl::ostream& os, const SInt<w>& ui);
};

// template<int w>
// etl::ostream& operator<<(etl::ostream& os, const SInt<w>& si) {
