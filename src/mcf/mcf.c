...
double compute_alpha(MCF *mcf)
{
    ...
    for ( i = 0; i < mcf->no_node; i++) {
        if ( mcf->nodes[i].no_comm) {
            int *dest_flag = (int*)my_malloc((mcf->no_node)*sizeof(int));
            if ( dest_flag == NULL) {
                printf("\nError: Unable to my_malloc <getAlpha>.\n"); assert(0);
            }
    ...
    while (1) {
        ...
        for ( i = 0; i < mcf_v->no_node; i++) {
            if ( mcf_v->nodes[i].no_comm) { // if this node is source of "r" _commodities; // 如果该节点是“r”_commodities 的来源；
                ...
                int *dest_flag = (int*)my_malloc((mcf_v->no_node)*sizeof(int));
                if ( dest_flag == NULL) {
                    my_printf("\nError:  Unable to my_malloc <mcf>.\n"); assert(0);
                }
...
void shortest_paths(MCF *mcf_v, int s, int num_commodities, int *dest_flag) 
{
    ...
    PQUEUE pq;
    PQUEUE_init(&pq);
    int *pos = (int *)my_malloc(sizeof(int) * (mcf_v->no_node));
    if ( pos == NULL) {
        my_printf("\nError: Unable to my_malloc <shortest_path>.\n"); assert(0);
    }
...
void read_network_topology_and_demands(MCF *mcf_v, double rate) 
{
    ...
    mcf_v->nodes = (NODE*)my_malloc(sizeof(NODE) * (mcf_v->no_node));
    if ( mcf_v->nodes == NULL) {
        my_printf("\nError: Unable to my_malloc <nodes>.\n"); assert(0);
    }
    for ( int i = 0; i < mcf_v->no_node; i++) {
        id = node_buf[i].id;
        x  = node_buf[i].x;
        ...
        // mcf_v->nodes[i].edges = (int *)my_malloc(sizeof(int) * MAX_DEGREE);
        mcf_v->nodes[i].edges = (int *)my_malloc(sizeof(int) * node_buf[i].edge_num);
        if ( mcf_v->nodes[i].edges == NULL) {
            my_printf("\nError: Unable to my_malloc <nodes.edges>.\n"); assert(0);
        }
    }

    // (2) edges
    mcf_v->no_edge = edges_num;
    mcf_v->edges = (EDGE *)my_malloc(sizeof(EDGE) * (mcf_v->no_edge));
    if ( mcf_v->edges == NULL) {
        my_printf("\nError: Unable to my_malloc <edges>.\n"); assert(0);
    }
    mcf_v->_temp_edge_flow = (double*)my_malloc(sizeof(double) * (mcf_v->no_edge));
    if ( mcf_v->_temp_edge_flow == NULL) {
        my_printf("\nError: Unable to my_malloc <_temp_edge_flow>.\n"); assert(0);
    }
    ...
    mcf_v->_commodities = (COMMODITY *)my_malloc(sizeof(COMMODITY) * (mcf_v->no_commodity));
    if ( mcf_v->_commodities == NULL) {
        my_printf("\nError: Unable to my_malloc <_commodities>.\n"); assert(0);
    }
    for ( int i = 0; i < mcf_v->no_commodity; i++) {
        ...
        if (mcf_v->nodes[src].comms == NULL) {
            mcf_v->nodes[src].comms = (int *)my_malloc(sizeof(int) * mcf_v->no_node);
            if ( mcf_v->nodes[src].comms == NULL) {
                my_printf("\nError: Unable to my_malloc <nodes[src].comms>.\n"); assert(0);
            }
        }
    ...
    for ( int i = 0; i < mcf_v->no_edge; i++) {
        ...
        mcf_v->edges[i]._flows = (double *)my_malloc(sizeof(double) * (mcf_v->no_commodity));
        if ( mcf_v->edges[i]._flows == NULL) {
            my_printf("\nError: Unable to my_malloc <edges._flows>.\n"); assert(0);
        }
    ...
    for ( int i = 0; i < mcf_v->no_node; i++) {
        // Note2: same as above;
        // 注2：同上；
        mcf_v->nodes[i]._preferred_path = (int *)my_malloc(sizeof(int) * mcf_v->no_commodity);
        if ( mcf_v->nodes[i]._preferred_path == NULL) {
            my_printf("\nError: Unable to my_malloc <_preferred_path>.\n"); assert(0);
        }
        for ( int j = 0; j < mcf_v->no_commodity; j++) {
...
void free_topology(MCF *mcf_v)
{
    ...
    my_free( mcf_v->_commodities);

    for ( i = 0; i < mcf_v->no_edge; i++) {
        my_free( mcf_v->edges[i]._flows);
    }
    my_free( mcf_v->edges);
    my_free( mcf_v->_temp_edge_flow);

    for ( i = 0; i < mcf_v->no_node; i++) {
        my_free( mcf_v->nodes[i].comms);
        my_free( mcf_v->nodes[i].edges);
    }
    my_free( mcf_v->nodes);
