...
// ******************************************* 生成边 ******************************************* //
    ...
    for(int start_node = 0; start_node < node_num - 1; start_node++)       // 生成分支
    {
        ...
        int branch_num = (rand()%4 == 1) ? rand()%base : 0;    // 产生多少条分支，75%概率不生成
        ...
        for(int i = 0; i < len2end; i++) node_write[i] = 0; 
        ...
        for(int i = 0; i < branch_num; i++)     // 分支，不保证能用
        {
            ...
            fprintf(fp, "\t{%5d, %5d, %5d, %5d, %5d},\n", edge_num, start_node, end_node, capacity, delay); 
            edge_num++;         // 输出完成后再加
        }
    }   
    fprintf(fp, "};\n\n");
// ******************************************* 生成目标 ******************************************* //
    ...
    fprintf(fp, "\t{%5d, %5d, %5d, %5d},\n", 0, 0, node_num - 1, 10); // 打印第一个目标，该目标为固定值
    ...
    for(int start_node = 0; start_node < node_num - 1; start_node++)       // 生成分支
    {
        ...
        for(int i = 0; i < branch_num; i++)     // 开始生成
        {
            ...
            node_write[end_node - start_node - 1] = 1; // 不能重复
            fprintf(fp, "\t{%5d, %5d, %5d, %5d},\n", demands_num, start_node, end_node, demands);
            demands_num++;         // 输出完成后再加
        }
    }   
    fprintf(fp, "};\n");
