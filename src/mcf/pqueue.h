...
static inline void PQDATUM_init(PQDATUM *pqdatum){pqdatum->_node = -1; pqdatum->_dist = -1;}
...
static inline void set_node(PQDATUM *pqdatum, int node) { pqdatum->_node = node; }
static inline void set_dist(PQDATUM *pqdatum, double dist) { pqdatum->_dist = dist; }
...
static inline void PQUEUE_init(PQUEUE *pqueue){pqueue->_size = -1; pqueue->_avail = -1; pqueue->_step = -1;}
