...
ST_FUNC char *tcc_load_text(int fd) {
  int len = fs_lseek(fd, 0, SEEK_END);
...
PUB_FUNC void tcc_free(void *ptr) { bench_free(ptr); }

PUB_FUNC void *tcc_malloc(unsigned long size) {
  void *ptr;
  ptr = bench_malloc(size);
  if (!ptr && size)
...
no_file:
  ...
  if (!s1 || !s1->error_func) {
    /* default case: stderr */
    if (s1 && s1->output_type == TCC_OUTPUT_PREPROCESS && s1->ppfp == FD_STDOUT)
...
static int tcc_compile(TCCState *s1, int filetype, const char *str, int fd) {
  ...
  if (1 /*setjmp(s1->error_jmp_buf) == 0*/) {
    s1->nb_errors = 0;
...
static int tcc_set_linker(TCCState *s, const char *option) {
  ...
  while (*option) {
    ...
    if (link_option(option, "Bsymbolic", &p)) {
      ...
      // Since we don't have linker script, add an ability to move data segment
      // to a given location
...
PUB_FUNC void tcc_print_stats(TCCState *s1, unsigned total_time) {
  ...
  //        s1->total_output[3]
  //        );
  printf("* %d idents, %d lines, %d bytes\n"
         "* %0.3f s, %u lines/s, %0.1f MB/s\n",
         total_idents, total_lines, total_bytes, (double)total_time / 1000,
         (unsigned)total_lines * 1000 / total_time,
         (double)total_bytes / 1000 / total_time);
  printf("* text %d, data.rw %d, data.ro %d, bss %d bytes\n",
