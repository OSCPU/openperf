...
static void modify_reloctions_old_to_new(TCCState *s1, Section *s,
                                         int *old_to_new_syms) {
...
// static void tcc_output_elf(TCCState *s1, FILE *f, int phnum, ElfW(Phdr)
// *phdr,
...
//         else
//             ehdr.e_entry = get_sym_addr(s1, "_start", !!(file_type &
//             TCC_OUTPUT_EXE), 0);
...
//                               ElfW(Phdr) *phdr, int file_offset, int
//                               *sec_order)
// {
...
//         strerror(errno)); tcc_error_noabort("could not write '%s'",
//         filename); return -1;
//     }
//     if (s1->verbose)
//         printf("<- %s\n", filename);
...
/* Create an ELF file on disk.
   This function handle ELF specific layout requirements */
static void tcc_output_elf(TCCState *s1, int fd, int phnum, ElfW(Phdr) * phdr,
                           int file_offset, int *sec_order) {
  int i, shnum, offset, size, file_type;
  Section *s;
  ElfW(Ehdr) ehdr;
  ...
  fs_write(fd, &ehdr, sizeof(ElfW(Ehdr)));
  if (phdr)
    fs_write(fd, phdr, phnum * sizeof(ElfW(Phdr)));
  offset = sizeof(ElfW(Ehdr)) + phnum * sizeof(ElfW(Phdr));
  ...
  for (i = 0; i < shnum; i++) {
    ...
    fs_write(fd, sh, sizeof(ElfW(Shdr)));
  }
}

static void tcc_output_binary(TCCState *s1, int fd, const int *sec_order) {
  Section *s;
  int i, offset, size;
...
static int tcc_write_elf_file(TCCState *s1, const char *filename, int phnum,
  ...
  if (s1->output_format == TCC_OUTPUT_FORMAT_ELF)
    tcc_output_elf(s1, fd, phnum, phdr, file_offset, sec_order);
  else
    tcc_output_binary(s1, fd, sec_order);
  fs_close(fd);
  return 0;
}
...
ST_FUNC void *load_data(int fd, unsigned long file_offset, unsigned long size) {
  ...
  fs_lseek(fd, file_offset, SEEK_SET);
...
ST_FUNC int tcc_load_object_file(TCCState *s1, int fd,
  ...
  found:
    ...
    if (sh->sh_type != SHT_NOBITS) {
      unsigned char *ptr;
      fs_lseek(fd, file_offset + sh->sh_offset, SEEK_SET);
