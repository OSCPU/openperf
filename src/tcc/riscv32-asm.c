...
static void parse_operand(TCCState *s1, Operand *op) {
  ...
  if (!op->e.sym) {
    ...
    // If we have a symbol we will let the linker handle it and just put in a
    // zero for the immediate value, we also need to put an entry into the
    // reallocation table, this should however, the entry will be different
    // depending on the instruction, so ignore it for now.
...
void emit_R(uint32_t funct7, uint32_t rs2, uint32_t rs1, uint32_t funct3,
            uint32_t rd, uint32_t opcode) {
  const uint32_t instruction = (funct7 << 25) | (rs2 << 20) | (rs1 << 15) |
...
void emit_I(uint32_t imm, uint32_t rs1, uint32_t funct3, uint32_t rd,
            uint32_t opcode) {
...
// store instructions
void emit_S(uint32_t imm, uint32_t rs2, uint32_t rs1, uint32_t funct3,
            uint32_t opcode) {
 ...
 */
void emit_B(uint32_t imm, uint32_t rs2, uint32_t rs1, uint32_t funct3,
            uint32_t opcode) {
 ...
 * we expect the immediate passed into this function to be in the lower space
 * (i.e we will shift)
 */
 ...
 * The type of the relocation table entry needs to match format of immediate
...
static void asm_binary_pseudocode(TCCState *s1, int token) {
  ...
  parse_operand(s1, &rd);
  if (rd.type != OP_REG) {
    tcc_error("'%s': Expected destination operand that is a register",
  ...
  parse_operand(s1, &rs);
  if (rs.type != OP_REG) {
    tcc_error("'%s': Expected source operand that is a register",
              get_tok_str(token, NULL));
    return;
  }
  ...
  switch (token) {
  case TOK_ASM_mv:
    emit_MV(rd.reg, rs.reg);
    return;
  // comparison to zero pseudoinstructions
  case TOK_ASM_seqz:
    emit_SEQZ(rd.reg, rs.reg);
    return;
  case TOK_ASM_snez:
    emit_SNEZ(rd.reg, rs.reg);
    return;
  case TOK_ASM_sltz:
    emit_SLTZ(rd.reg, rs.reg);
    return;
  case TOK_ASM_sgtz:
    emit_SGTZ(rd.reg, rs.reg);
    return;
  }
}
...
static void asm_branch_zero_opcode(TCCState *s1, int token) {
  ...
  switch (token) {
  case TOK_ASM_beqz:
    emit_BEQZ(r1.reg, offset);
    return;
  case TOK_ASM_bnez:
    emit_BNEZ(r1.reg, offset);
    return;
  case TOK_ASM_blez:
    emit_BEQZ(r1.reg, offset);
    return;
  case TOK_ASM_bgez:
    emit_BGEZ(r1.reg, offset);
    return;
  case TOK_ASM_bltz:
    emit_BLTZ(r1.reg, offset);
    return;
  case TOK_ASM_bgtz:
    emit_BGTZ(r1.reg, offset);
    return;
  default:
...
static void asm_binary_opcode(TCCState *s1, int token) {
  ...
  if (token != TOK_ASM_la) {
    if (!(ops[1].type & OP_IM32)) {
      ...
      tcc_error("'%s': Expected second source operand that is an immediate "
                "value between 0 "
                "and 0xfffff",
                get_tok_str(token, NULL));
      return;
...
static void asm_branch_opcode(TCCState *s1, int token) {
  ...
  if (offset > 0xfff) {
    tcc_error("'%s': Expected third operand that is an immediate value between "
              "0 and 0xfff\n"
              "received: %u",
  ...
  switch (token) {
  case TOK_ASM_beq:
    emit_BEQ(ops[0].reg, ops[1].reg, offset);
    return;
  case TOK_ASM_bne:
    emit_BNE(ops[0].reg, ops[1].reg, offset);
    return;
  case TOK_ASM_blt:
    emit_BLT(ops[0].reg, ops[1].reg, offset);
    return;
  case TOK_ASM_ble:
    emit_BLE(ops[0].reg, ops[1].reg, offset);
    return;

  case TOK_ASM_bltu:
    emit_BLTU(ops[0].reg, ops[1].reg, offset);
    return;
  case TOK_ASM_bgeu:
    emit_BGEU(ops[0].reg, ops[1].reg, offset);
    return;
  case TOK_ASM_bgtu:
    emit_BGTU(ops[0].reg, ops[1].reg, offset);
    return;
  case TOK_ASM_bleu:
    emit_BLEU(ops[0].reg, ops[1].reg, offset);
    return;
...
// handle the register opcodes that don't have an immediate value (except for
// shifts)
static void asm_binary_register_opcode(TCCState *s1, int token) {
  // start by getting the next three operands and check that they are all
  // registers
  Operand rd, r1, r2;
  parse_operand(s1, &rd);
  if (!check_register(&rd)) {
    tcc_error("'%s': Expected destination to be a register",
              get_tok_str(token, NULL));
  }
  ...
  if (!check_register(&r1)) {
    tcc_error("'%s': Expected first source operand to be a register",
              get_tok_str(token, NULL));
  }
  ...
  parse_operand(s1, &r2);
  // check that we are using a register, or that we have an immediate between 0
  // and 31 (for shifts)
  if (!(r2.type & OP_REG) && r2.e.v > 31) {
    tcc_error("'%s': Expected second source operand to be a register or "
              "immediate between 0 and 31",
              get_tok_str(token, NULL));
    return;
  }
  ...
  switch (token) {
  case TOK_ASM_slli:
    emit_SLLI(rd.reg, r1.reg, r2.e.v);
    return;
  case TOK_ASM_srli:
    emit_SRLI(rd.reg, r1.reg, r2.e.v);
    return;
  case TOK_ASM_srai:
    emit_SRAI(rd.reg, r1.reg, r2.e.v);
    return;
  case TOK_ASM_add:
    emit_ADD(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_sub:
    emit_SUB(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_sll:
    emit_SLL(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_slt:
    emit_SLT(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_sltu:
    emit_SLTU(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_xor:
    emit_XOR(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_srl:
    emit_SRL(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_sra:
    emit_SRA(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_or:
    emit_OR(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_and:
    emit_AND(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_mul:
    emit_MUL(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_mulh:
    emit_MULH(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_mulhsu:
    emit_MULHSU(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_div:
    emit_DIV(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_divu:
    emit_DIVU(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_rem:
    emit_REM(rd.reg, r1.reg, r2.reg);
    return;
  case TOK_ASM_remu:
    emit_REMU(rd.reg, r1.reg, r2.reg);
    return;
  default:
...
static void asm_immediate_opcode(TCCState *s1, int token) {
  ...
  if (!check_register(&regs[1])) {
    tcc_error("'%s': Expected second source operand to be a register",
              get_tok_str(token, NULL));
    return;
  }
  ...
  switch (token) {
  case TOK_ASM_addi:
    emit_ADDI(regs[0].reg, regs[1].reg, imm.e.v);
    return;
  case TOK_ASM_slti:
    emit_SLTI(regs[0].reg, regs[1].reg, imm.e.v);
    return;
  case TOK_ASM_sltiu:
    emit_SLTIU(regs[0].reg, regs[1].reg, imm.e.v);
    return;
  case TOK_ASM_xori:
    emit_XORI(regs[0].reg, regs[1].reg, imm.e.v);
    return;
  case TOK_ASM_ori:
    emit_ORI(regs[0].reg, regs[1].reg, imm.e.v);
    return;
  case TOK_ASM_andi:
    emit_ANDI(regs[0].reg, regs[1].reg, imm.e.v);
    return;
  default:
...
// make a special case for load/store opcodes since they have a distinct syntax
// from the other commands
static void asm_load_store_opcode(TCCState *s1, int token) {
  // start by getting the next three operands and check that they are all
  // registers for a store instruction we expect the format to be s? src,
  // imm(dst) store src -> [loc_reg + imm] this formats to the opcode s?
  // loc_reg, src, imm
  Operand src_dst, imm, loc_reg;
  parse_operand(s1, &src_dst);
  if (!check_register(&src_dst)) {
    tcc_error("'%s': Expected source to be a register",
              get_tok_str(token, NULL));
  }
  ...
  // check if the immediate is available, if it isn't we will have a '('
  // character.
  if (tok != '(') {
  ...
  if (!check_register(&loc_reg)) {
    tcc_error("'%s': Expected offset operand to be a register",
              get_tok_str(token, NULL));
    return;
  }
  // get closing paren
  ...
  // load
  case TOK_ASM_lb:
    emit_LB(src_dst.reg, loc_reg.reg, imm.e.v);
    return;
  case TOK_ASM_lh:
    emit_LH(src_dst.reg, loc_reg.reg, imm.e.v);
    return;
  case TOK_ASM_lw:
    emit_LW(src_dst.reg, loc_reg.reg, imm.e.v);
    return;
  case TOK_ASM_lbu:
    emit_LBU(src_dst.reg, loc_reg.reg, imm.e.v);
    return;
  case TOK_ASM_lhu:
    emit_LHU(src_dst.reg, loc_reg.reg, imm.e.v);
    return;
  // store
  case TOK_ASM_sb:
    emit_SB(loc_reg.reg, src_dst.reg, imm.e.v);
    return;
  case TOK_ASM_sh:
    emit_SH(loc_reg.reg, src_dst.reg, imm.e.v);
    return;
  case TOK_ASM_sw:
    emit_SW(loc_reg.reg, src_dst.reg, imm.e.v);
    return;
  default:
...
static void asm_control_status_pseudo_opcode(TCCState *s1, int token) {
  ...
  if (token == TOK_ASM_csrr || token == TOK_ASM_csrwi ||
    ...
    if (token == TOK_ASM_csrr) {
      ...
      // otherwise we should send an error
        ...
        tcc_error("'%s': Expected second argument to be an immediate between 0 "
                  "and 0x1f",
                  get_tok_str(token, NULL));
        return;
      }
  ...
  // other instruction
    ...
    if (!check_register(&temp)) {
      tcc_error("'%s': Expected second argument to be a register",
                get_tok_str(token, NULL));
      return;
    }
    rs = temp.reg;
  ...
  case TOK_ASM_csrci:
    ...
    return;
  default:
    expect("control and status instruction");
    return;
  }
}
// support the Zicsr extension Takes in three arguments a dest register,
// immediate value, and a source register or a source immediate
static void asm_control_status_opcode(TCCState *s1, int token) {
  ...
  switch (token) {
  case TOK_ASM_csrrw:
    emit_CSRRW(rd.reg, csr.e.v, rs1.reg);
    return;
  case TOK_ASM_csrrs:
    emit_CSRRS(rd.reg, csr.e.v, rs1.reg);
    return;
  case TOK_ASM_csrrc:
    emit_CSRRC(rd.reg, csr.e.v, rs1.reg);
    return;
  case TOK_ASM_csrrwi:
    emit_CSRRC(rd.reg, csr.e.v, rs1.e.v);
    return;
  case TOK_ASM_csrrsi:
    emit_CSRRC(rd.reg, csr.e.v, uimm.e.v);
    return;
  case TOK_ASM_csrrci:
    emit_CSRRC(rd.reg, csr.e.v, uimm.e.v);
    return;
...
// Generate code for j, jal, jalr, and jr. Can take 1, 2, or 3 arguments (from
// TCCState)
static void asm_jump_opcode(TCCState *s1, int token) {
  ...
  if (ops[0].type != OP_REG) {
    ...
    case TOK_ASM_jal:
      ...
      break;
    // other than these two instructions, we expect to have a register, so issue
    // an error
    default:
      expect("register");
      return;
    ...
    if (!(ops[0].type & OP_IM21)) {
      tcc_error("'%s': Expected operand that is an immediate value between 0 "
                "and 0x1fffff",
                get_tok_str(token, NULL));
    }
    return;
  ...
  if (ops[0].type != OP_REG) {
    ...
    if (!(ops[1].type & OP_IM21)) {
      tcc_error("'%s': Expected second operand that is an immediate value "
                "between 0 and 0xfffff",
                get_tok_str(token, NULL));
      return;
    }
...
static void asm_call_opcode(TCCState *s1, int token) {
  ...
  switch (token) {
  case TOK_ASM_call:
    emit_CALL(symbol.e.v);
    return;
  case TOK_ASM_tail:
    emit_TAIL(symbol.e.v);
    return;
  default:
    // we didn't get the instruction we expected, send an error
    expect("jump or link instruction");
...
ST_FUNC void asm_opcode(TCCState *s1, int token) {
  ...
  case TOK_ASM_fence_i:
    tcc_error("zfenci instruction not implemented");
    return;
