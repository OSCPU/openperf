...
// // TODO add temporary and saved registers here once I figure out how TCC
// works #define NB_REGS 11 // a0-a7, sp (?), ra, sp #else #define NB_REGS 19 //
// a0-a7, fa0-fa7, xxx, ra, sp #endif
...
ST_DATA const int reg_classes[NB_REGS] = {
    ...
    RC_FLOAT | RC_F(0), RC_FLOAT | RC_F(1), RC_FLOAT | RC_F(2),
    RC_FLOAT | RC_F(3), RC_FLOAT | RC_F(4), RC_FLOAT | RC_F(5),
...
static void load_lvalue(int r, SValue *sv) {
  ...
  switch (size) {
  case 1:
    emit_LB(dest_reg, rs1, lvar_offset);
    break;
  case 2:
    emit_LH(dest_reg, rs1, lvar_offset);
    break;
  case 4:
    emit_LW(dest_reg, rs1, lvar_offset);
    break;
  default:
    tcc_error("unexpected load size: %d", size);
  }
}
  ...
  if (stack_reg & VT_LVAL) {
    ...
    if (is_freg(r) && is_freg(masked_stack_reg)) {
      ER(0x53, 0, dest_reg, freg(masked_stack_reg), freg(masked_stack_reg),
         stack_type == VT_DOUBLE
             ? 0x11
             : 0x10); // fsgnj.[sd] Rd, V, V == fmv.[sd] Rd, V
...
ST_FUNC void store(int r, SValue *sv) {
  ...
  switch (size) {
  case 1:
    emit_SB(loc_reg, src_reg, offset);
    break;
  case 2:
    emit_SH(loc_reg, src_reg, offset);
    break;
  case 4:
    emit_SW(loc_reg, src_reg, offset);
    break;
  default:
    tcc_error("unexpected store size: %d", size);
  }
}
...
static void reg_pass_rec(CType *type, int *rc, int *fieldofs, int ofs) {
  if ((type->t & VT_BTYPE) == VT_STRUCT) {
    ...
    fieldofs[rc[0]] =
        (ofs << 4) |
        ((type->t & VT_BTYPE) == VT_PTR ? VT_LLONG : type->t & VT_BTYPE);
  } else
    rc[0] = -1;
}
...
ST_FUNC void gsym_addr(int branch_list_offset, int target_offset) {
  ...
  nocode_wanted &=
      ~0x20000000; // copy code from the NO_CODE macro (move to tcc.h?)
