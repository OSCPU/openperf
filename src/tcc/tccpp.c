...
tail_call:
  if (!al)
    return;
  // #ifdef TAL_INFO
  //     fprintf(stderr, "limit=%5d, size=%5g MB, nb_peak=%6d, nb_total=%8d,
  //     nb_missed=%6d, usage=%5.1f%%\n",
  //             al->limit, al->size / 1024.0 / 1024.0, al->nb_peak,
  //             al->nb_total, al->nb_missed, (al->peak_p - al->buffer) * 100.0
  //             / al->size);
  // #endif
  // #ifdef TAL_DEBUG
  //     if (al->nb_allocs > 0) {
...
tail_call:
  if (al->buffer <= (uint8_t *)p && (uint8_t *)p < al->buffer + al->size) {
    ...
    //         if (header->line_num < 0) {
    //             fprintf(stderr, "%s:%d: TAL_DEBUG: double frees chunk
    //             from\n",
    //                     file, line);
...
static void *tal_realloc_impl(TinyAlloc **pal, void *p,
  ...
  unsigned adj_size = (size + sizeof(size_t) - 1) & ~(sizeof(size_t) - 1);
  TinyAlloc *al = *pal;
...
static int handle_eob(void) {
  ...
  if (bf->buf_ptr >= bf->buf_end) {
    if (bf->fd >= 0) {
      ...
      len = IO_BUF_SIZE;
#endif
      len = fs_read(bf->fd, bf->buffer, len);
      if (len < 0)
        len = 0;
    } else {
      len = 0;
...
static void pragma_parse(TCCState *s1) {
  ...
  if (tok == TOK_push_macro || tok == TOK_pop_macro) {
    ...
    tcc_warning_c(warn_unsupported)("#pragma %s ignored",
...
static int macro_subst_tok(TokenString *tok_str, Sym **nested_list, Sym *s) {
  ...
  if (tok == TOK___LINE__ || tok == TOK___COUNTER__) {
    ...
    // }
    if (tok == TOK___DATE__) {
      snprintf(buf, sizeof(buf), "%s %2d %d", ab_month_name[0], 0, 0 + 1900);
    } else {
